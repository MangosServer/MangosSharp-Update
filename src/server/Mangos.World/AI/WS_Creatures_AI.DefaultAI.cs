...
public partial class WS_Creatures_AI
{
    public class DefaultAI : TBaseAI
    {
            ...
            checked
            {
                try
                {
                    ...
                    if(nextAttack <= 0)
                    {
                        ...
                        var damageInfo = WorldServiceLocator.WSCombat
                            .CalculateDamage(ref Attacker, ref this.aiTarget, DualWield: false, Ranged: false);
                        ...
                        WorldServiceLocator.WSCombat
                            .SendAttackerStateUpdate(Attacker: ref Object2, Victim: ref Object, damageInfo, client);
                ...
                } catch(Exception ex)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.WARNING,
                            "WS_Creatures:DoAttack failed - Guid: {1} ID: {2}  {0}",
                            ex.Message);
                    Reset();
                }
            }
        ...
        {
            var distance = (!ResetRun)
                ? ((float)(3.0 * (aiCreature?.CreatureInfo?.WalkSpeed)))
                : ((float)(3.0 * (aiCreature?.CreatureInfo?.RunSpeed)));
            aiCreature?.SetToRealPosition(Forced: true);
            var angle = WorldServiceLocator.WSCombat
                .GetOrientation(aiCreature.positionX, ResetX, aiCreature.positionY, ResetY);
            ...
            var selectedZ = WorldServiceLocator.WSMaps
                .GetZCoord(selectedX, selectedY, aiCreature.positionZ, aiCreature.MapID);
        ...
        public override void DoMove()
        {
            switch(aiTarget)
            {
                ...
                default:
                    var distanceToLastHit = WorldServiceLocator.WSCombat
                        .GetDistance(
                            aiCreature.positionX,
                            LastHitX,
                            aiCreature.positionY,
            ...
            if(aiTarget == null)
            {
                ...
                while(true)
                {
                    ...
                    selectedZ2 = WorldServiceLocator.WSMaps
                        .GetZCoord(selectedX2, selectedY2, aiCreature.positionZ, aiCreature.MapID);
                    MoveTries = checked(MoveTries + 1);
                    if(!(Math.Abs(aiCreature.positionZ - selectedZ2) > 5f))
                    {
                        ref var reference = ref aiCreature;
                        WS_Base.BaseObject obj = reference;
                        var flag = WorldServiceLocator.WSMaps
                            .IsInLineOfSight(ref obj, selectedX2, selectedY2, selectedZ2 + 1f);
            ...
            if(distanceToTarget < distance)
            {
                ...
                NearX = (!((aiTarget?.positionX) > (aiCreature?.positionX)))
                    ? (NearX + destDist)
                    : (NearX - destDist);
                var NearY = aiTarget.positionY;
                NearY = (!((aiTarget?.positionY) > (aiCreature?.positionY)))
                    ? (NearY + destDist)
                    : (NearY - destDist);
                var NearZ = WorldServiceLocator.WSMaps
                    .GetZCoord(NearX, NearY, aiCreature.positionZ, aiCreature.MapID);
                ...
                {
                    aiCreature.orientation = WorldServiceLocator.WSCombat
                        .GetOrientation(aiCreature.positionX, NearX, aiCreature.positionY, NearY);
            ...
            State = AIState.AI_MOVE_FOR_ATTACK;
            var angle = WorldServiceLocator.WSCombat
                .GetOrientation(aiCreature.positionX, aiTarget.positionX, aiCreature.positionY, aiTarget.positionY);
            aiCreature.orientation = angle;
            var selectedX = (float)((aiCreature?.positionX) + (Math.Cos(angle) * distance));
            var selectedY = (float)((aiCreature?.positionY) + (Math.Sin(angle) * distance));
            var selectedZ = WorldServiceLocator.WSMaps
        ...
        public override void DoThink()
        {
            if(aiCreature == null)
            {
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "AI Creature {0} Null", aiCreature);
                return;
            }
            checked
            {
                ...
                switch(State)
                {
                    case AIState.AI_DEAD:
                        if((aiHateTable?.Count) > 0)
                        {
                            ...
                            aiTimer = WorldServiceLocator.WSCreatures.CorpseDecay[aiCreature.CreatureInfo.Elite] *
                                1000;
                            ignoreLoot = false;
                            break;
                        }
                    ...
                    case AIState.AI_WANDERING:
                        if(!AllowedMove)
                        {
                            ...
                            (WorldServiceLocator.WorldServer.Rnd.NextDouble() > 0.20000000298023224))
                        {
                            DoMove();
                        }
                        break;
            ...
            checked
            {
                    ...
                    (State != AIState.AI_MOVING_TO_SPAWN))
                {
                    ...
                    } else
                    {
                        ...
                        (aiHateTable = this.aiHateTable)[key = Attacker] = (int)Math.Round(
                            aiHateTable[key] + (HateValue * Attacker.Spell_ThreatModifier));
                    }
                }
            }
        }
        ...
        public override void Pause(int Time) { aiTimer = Time; }
        ...
        public override bool IsMoving => (checked(WorldServiceLocator.NativeMethods.timeGetTime(string.Empty) -
                    (aiCreature?.LastMove)) <
                aiTimer) &&
            (State switch
            {
