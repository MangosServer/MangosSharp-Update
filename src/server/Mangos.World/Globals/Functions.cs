...
public partial class Functions
{
    ...
    private static partial Regex AZ();

    [GeneratedRegex("^[a-z A-Z]+$")]
    private static partial Regex Guild();
            ...
            case > 0:
                ...
                switch(bannedAccount.Rows.Count)
                {
                    case > 0:
                        WorldServiceLocator.WorldServer.AccountDatabase
                            .Update(
                                $"UPDATE account_banned SET active = 1 WHERE id = '{Conversions.ToString(accID)}';");
                        break;
    ...
    public void Broadcast(string Message)
    {
        ...
        foreach(var Character in WorldServiceLocator.WorldServer.CHARACTERs)
        {
            ...
            {
                SendMessageSystem(
                    Character.Value.client,
                    $"System Message: {SetColor(Message, byte.MaxValue, 0, 0)}");
            }
        }
        WorldServiceLocator.WorldServer.CHARACTERs_Lock.ReleaseReaderLock();
    }

    public Packets.Packets.PacketClass BuildChatMessage(
        ulong SenderGUID,
        string Message,
        ChatMsg msgType,
        ...
        string msgChannel = "Global")
    {
        ...
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.FAILED, "failed chat message type - {0}!", msgType, ex);
        }
        return packet;
    }

    public Packets.Packets.PacketClass BuildPartyMemberStatsOffline(ulong GUID)
    {
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_PARTY_MEMBER_STATS_FULL);
    ...
    public DateTime GetNextDate(int Days, int Hours = 0) { return DateAndTime.Today.AddDays(Days).AddHours(Hours); }
    ...
    public bool IsContinentMap(int Map) { return ((uint)Map) <= 1u; }
    ...
    public void SendAccountMD5(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass SMSG_ACCOUNT_DATA_TIMES = new(Opcodes.SMSG_ACCOUNT_DATA_MD5);
        try
        {
            var i = 0;
            do
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "Send Account MD5 Exception  {0} : {1} : {2} : {3}",
                    client,
                    Character,
                    FoundData,
                    ex);
        ...
        int Seconds = 30)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_CORPSE_RECLAIM_DELAY);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "Send Corpse Reclaim Delay Exception  {0} : {1} : {2} : {3}",
                    client,
                    Character,
                    Seconds,
                    ex);
    ...
    public void SendGameTime(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass SMSG_LOGIN_SETTIMESPEED = new(Opcodes.SMSG_LOGIN_SETTIMESPEED);
        checked
        {
            try
            {
                ...
                SMSG_LOGIN_SETTIMESPEED.AddInt32(
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.WARNING, "Send Game Time Exception  {0} : {1} : {2}", client, Character, ex);
            } finally
            {
    ...
    public void SendMessageNotification(ref WS_Network.ClientClass client, string Message)
    {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "SMSG_NOTIFIATION Exception {0} : {0} : {1} : {2}",
                    client,
                    packet,
                    Message,
                    ex);
    ...
    public void SendMessageSystem(WS_Network.ClientClass objCharacter, string Message)
    {
        ...
        var packet = BuildChatMessage(0uL, Message, ChatMsg.CHAT_MSG_SYSTEM, LANGUAGES.LANG_GLOBAL, 0, string.Empty);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "Send Message Exception  {0} : {1} : {2} : {3}",
                    objCharacter,
                    packet,
                    Message,
                    ex);
    ...
    public void SendProficiency(ref WS_Network.ClientClass client, byte ProficiencyType, int ProficiencyFlags)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_SET_PROFICIENCY);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "Send Proficiency Exception  {0} : {1} : {2} : {3} : {4}",
                    client,
                    ProficiencyType,
                    ProficiencyFlags,
                    packet,
    ...
    public void SendTimeSyncReq(ref WS_Network.ClientClass client)
    {
        ...
        WorldServiceLocator.WorldServer.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SendTimeSyncReq", client.IP, client.Port);
    }
    ...
    public void SendTriggerCinematic(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_TRIGGER_CINEMATIC);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "Send Trigger Cinematic Exception  {0} : {1} : {2} : {3}",
                    client,
                    Character,
                    WorldServiceLocator.WSDBCDatabase.CharRaces[(int)Character.Race].CinematicID,
                    ex);
        } finally
        {
            packet.Dispose();
