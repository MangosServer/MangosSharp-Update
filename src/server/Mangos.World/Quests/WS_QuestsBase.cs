...
public class WS_QuestsBase : IDisposable
{
    ...
    public void AddCast(WS_PlayerData.CharacterObject objCharacter, byte index, ulong oGUID)
    {
        ...
        checked
        {
            ...
            WorldServiceLocator.WorldServer.ALLQUESTS
                .SendQuestMessageAddKill(
                    ref objCharacter.client,
                    ID,
                    oGUID,
                    ObjectivesObject[index],
                    Progress[index],
                    ObjectivesCount[index]);
        }
    }
    ...
    public void AddItem(WS_PlayerData.CharacterObject objCharacter, byte index, byte Count)
    {
        ...
        checked
        {
            ...
            WorldServiceLocator.WorldServer.ALLQUESTS
                .SendQuestMessageAddItem(ref objCharacter.client, ObjectivesItem[index], ItemCount);
        }
    }
    ...
    public void Initialize(ref WS_PlayerData.CharacterObject objCharacter)
    {
        ...
        if(ObjectivesDeliver > 0)
        {
            ...
            {
                tmpItem.Delete();
                Packets.Packets.PacketClass response = new(Opcodes.SMSG_QUESTGIVER_QUEST_FAILED);
    ...
    {
        Complete = (ObjectivesCount[0] <= ((uint)Progress[0])) &&
            (ObjectivesCount[1] <= ((uint)Progress[1])) &&
            (ObjectivesCount[2] <= ((uint)Progress[2])) &&
            (ObjectivesCount[3] <= ((uint)Progress[3])) &&
            (ObjectivesItemCount[0] <= ((uint)ProgressItem[0])) &&
            (ObjectivesItemCount[1] <= ((uint)ProgressItem[1])) &&
            (ObjectivesItemCount[2] <= ((uint)ProgressItem[2])) &&
            (ObjectivesItemCount[3] <= ((uint)ProgressItem[3])) &&
            Explored &&
            !Failed;
        return Complete;
    }
            ...
            checked
            {
                if(ObjectivesItem[i] != 0)
                {
                    ...
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "ITEM COUNT UPDATED TO: {0}", ProgressItem[i]);
                }
                i = (byte)unchecked((uint)(i + 1));
            }
        } while (i <= 3u);
