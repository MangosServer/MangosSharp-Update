    ...
    public class LootObject : IDisposable
    {
        ...
        public void GetLoot(ref WS_Network.ClientClass client, byte Slot)
        {
            ...
            try
            {
                if(Items[Slot] == null)
                {
                    Packets.Packets.PacketClass response4 = new(Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                ...
                {
                    Packets.Packets.PacketClass response3 = new(Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                ...
                var tmpItem = (ItemObject)(new(Items[Slot].ItemID, client.Character.GUID)
                {
                    StackCount = Items[Slot].ItemCount
                });
                var StackCount = Items[Slot].ItemCount;
                if (client.Character.ItemADD(ref tmpItem))
                {
                    ...
                    Packets.Packets.PacketClass response2 = new(Opcodes.SMSG_LOOT_REMOVED);
            ...
            } catch(Exception e)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.WARNING, "Error getting loot.{0}", $"{Environment.NewLine}{e}");
            }
        }
        ...
        public void SendLoot(ref WS_Network.ClientClass client)
        {
            ...
            {
                Packets.Packets.PacketClass notMy = new(Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                notMy.AddInt8(58);
                notMy.AddUInt64(0uL);
                notMy.AddUInt64(0uL);
                notMy.AddInt8(0);
            ...
            checked
            {
                    ...
                    switch(Items[j])
                    {
                        ...
                        default:
                            ...
                            if(client.Character.IsInGroup &&
                                (client.Character.Group.LootMethod == GroupLootMethod.LOOT_MASTER) &&
                                (client.Character.Group.LocalLootMaster != null) &&
                                (client.Character.Group.LocalLootMaster != client.Character))
                            {
                                response.AddInt8(2);
                            } else
                            {
                ...
                if(!client.Character.IsInGroup ||
                    !((client.Character.Group.LootMethod == GroupLootMethod.LOOT_NEED_BEFORE_GREED) ||
                        (client.Character.Group.LootMethod == GroupLootMethod.LOOT_GROUP)))
                {
                    return;
                }
            ...
            {
                if((Items[i] != null) &&
                    (WorldServiceLocator.WorldServer.ITEMDatabase[Items[i].ItemID].Quality >=
                        ((int)client.Character.Group.LootThreshold)))
                {
                    GroupLootInfo[i] = new GroupLootInfo { LootObject = this, LootSlot = i, Item = Items[i] };
