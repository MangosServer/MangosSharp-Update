...
public class WS_Commands
{
    ...
    private void PerformOverflow() { PerformOverflow(); }
    ...
    public bool cmdActivateGameObject(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].State ==
            GameObjectLootState.DOOR_CLOSED)
        {
            WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].State = GameObjectLootState.DOOR_OPEN;
            WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].SetState(
                GameObjectLootState.DOOR_OPEN);
        } else
        {
        ...
        objCharacter.CommandResponse(
            $"Activated game object [{WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].Name}] to state [{WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].State}].");
        return true;
    }
    ...
    public bool cmdAddCreature(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        WS_Creatures.CreatureObject tmpCr = new(
            Conversions.ToInteger(Message),
            objCharacter.positionX,
            objCharacter.positionY,
            objCharacter.positionZ,
            objCharacter.orientation,
            checked((int)objCharacter.MapID));
        tmpCr.AddToWorld();
        objCharacter.CommandResponse(
            $"Creature [{WorldServiceLocator.Functions.SetColor(tmpCr.Name, 4, 147, 11)}] spawned.");
        return true;
    }
    ...
    [ChatCommand(
        "additem",
        "additem #itemid #count (optional) - Add chosen items with item amount to selected character.")]
    public bool cmdAddItem(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        checked
        {
            ...
            if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID) &&
                WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
            {
                ItemObject newItem2 = new(id, objCharacter.TargetGUID) { StackCount = Count };
                if(WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].ItemADD(ref newItem2))
                {
                    WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].LogLootItem(
                        newItem2,
                        (byte)Count,
                        Recieved: true,
                        Created: false);
            ...
            {
                ItemObject newItem = new(id, objCharacter.GUID) { StackCount = Count };
    ...
    public bool cmdAddItemSet(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.WSDBCDatabase.ItemSet.ContainsKey(id))
        {
                ...
                WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
            {
                ...
                {
                    ItemObject newItem = new(item, objCharacter.TargetGUID) { StackCount = 1 };
            ...
            } else
            {
                ...
                {
                    ItemObject newItem2 = new(item2, objCharacter.GUID) { StackCount = 1 };
    ...
    {
        if(Operators.CompareString(tCopper, string.Empty, TextCompare: false) == 0)
        {
            return false;
        }
    ...
    [ChatCommand(
        "addrestedxp",
        "addrestedxp #amount - Add X rested bonus experience points to selected character.",
    ...
    public bool cmdAddRestedXP(ref WS_PlayerData.CharacterObject objCharacter, string tXP)
    {
        ...
        checked
        {
            if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
            {
                ...
                WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].SetUpdateFlag(
                    1175,
                    WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].RestBonus);
                WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].SetUpdateFlag(
                    194,
                    WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].cPlayerBytes2);
    ...
    //[ChatCommand("SetWeather", "SETWEATHER <TYPE> <INTENSITY> - Change weather in current zone. Intensity is float value!")]
    ...
    //    if (WorldServiceLocator.WSWeather.WeatherZones.ContainsKey(objCharacter.ZoneID) == false)
    //    {
    ...
    //        WorldServiceLocator.WSWeather.SendWeather();
    //    }
    //    return true;
    //}
    ...
    public bool cmdAI(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(Operators.CompareString(
                WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message),
                "ENABLE",
                TextCompare: false) ==
            0)
        {
    ...
    [ChatCommand(
        "npcaistate",
        "npcaistate - Shows debug information about AI state of selected creature.",
        AccessLevel.Developer)]
    public bool cmdAIState(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        objCharacter.CommandResponse(
            $"Information for creature [{creatureObject.Name}]:{Environment.NewLine}ai = {creatureObject.aiScript}{Environment.NewLine}state = {creatureObject.aiScript.State}{Environment.NewLine}maxdist = {creatureObject.MaxDistance}");
    ...
    [ChatCommand("anticheat", "gobjectnear - Tells you the near objects count.", AccessLevel.Developer)]
    public bool cmdAnticheat(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        objCharacter.CommandResponse("Anticheat:");
        objCharacter.CommandResponse("-------------------------------");
        objCharacter.CommandResponse($"Character: {AntiCheat.WS_Anticheat.SpeedHacks[0].Character}");
        objCharacter.CommandResponse($"LastMessage: {AntiCheat.WS_Anticheat.SpeedHacks[0].LastMessage}");
        objCharacter.CommandResponse($"LastClientTime: {AntiCheat.WS_Anticheat.SpeedHacks[0].LastClientTime}");
        objCharacter.CommandResponse($"LastServerTime: {AntiCheat.WS_Anticheat.SpeedHacks[0].LastServerTime}");
        objCharacter.CommandResponse($"TotalOffset: {AntiCheat.WS_Anticheat.SpeedHacks[0].TotalOffset}");
        objCharacter.CommandResponse($"LastViolation: {AntiCheat.WS_Anticheat.SpeedHacks[0].LastViolation}");
        objCharacter.CommandResponse($"Violations: {AntiCheat.WS_Anticheat.SpeedHacks[0].Violations}");
        return true;
    }
        ...
        checked
        {
            if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(GUID))
            {
                ...
                if(characterObject.OnTransport != null)
                {
                    ...
                    objCharacter.Transfer(
                        characterObject.positionX,
                        characterObject.positionY,
                        characterObject.positionZ,
                        characterObject.orientation,
                        (int)characterObject.MapID);
                } else
                {
                    objCharacter.Teleport(
                        characterObject.positionX,
                        characterObject.positionY,
                        characterObject.positionZ,
                        characterObject.orientation,
    ...
    public bool cmdBan(ref WS_PlayerData.CharacterObject objCharacter, string Name)
    {
        ...
        WorldServiceLocator.WorldServer.AccountDatabase
            .Query($"SELECT id, last_ip FROM account WHERE username = \"{Name}\";", ref account);
        var accountID = Conversions.ToULong(account.Rows[0]["id"]);
        var IP = Conversions.ToInteger(account.Rows[0]["last_ip"]);
        DataTable result = new();
        WorldServiceLocator.WorldServer.AccountDatabase
            .Query($"SELECT active FROM account_banned WHERE id = {Conversions.ToString(accountID)};", ref result);
        if(result.Rows.Count > 0)
        {
            ...
            } else
            {
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update(
                        $"INSERT INTO `account_banned` VALUES ('{accountID}', UNIX_TIMESTAMP({Strings.Format(DateAndTime.Now, "yyyy-MM-dd hh:mm:ss")}), UNIX_TIMESTAMP(0000-00-00 00:00:00), '{objCharacter.Name}', 'No Reason Specified.', active = 1);");
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update(
                        $"INSERT INTO `ip_banned` VALUES ('{IP}', UNIX_TIMESTAMP({Strings.Format(DateAndTime.Now, "yyyy-MM-dd hh:mm:ss")}), UNIX_TIMESTAMP(0000-00-00 00:00:00), '{objCharacter.Name}', 'No Reason Specified.');");
                objCharacter.CommandResponse($"Account [{Name}] banned.");
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.INFORMATION,
                        "[{0}:{1}] Account [{3}] banned by [{2}].",
                        objCharacter.client.IP,
                        objCharacter.client.Port,
                        objCharacter.Name,
                        Name);
            }
        } else
    ...
    [ChatCommand(
        "bancharacter",
        "bancharacter - Selected player won't be able to login next time with this character.")]
    public bool cmdBanChar(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Update(
                    $"UPDATE characters SET force_restrictions = 2 WHERE char_guid = {objCharacter.TargetGUID};");
    ...
    public bool cmdCastSpell(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.CommonGlobalFunctions.GuidIsCreature(objCharacter.TargetGUID) &&
            WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(objCharacter.TargetGUID))
        {
            WS_Spells.SpellTargets Targets2 = new();
            ulong targetGUID;
            Dictionary<ulong, WS_Creatures.CreatureObject> wORLD_CREATUREs;
            WS_Base.BaseUnit objCharacter2 = (wORLD_CREATUREs = WorldServiceLocator.WorldServer.WORLD_CREATUREs)[
                targetGUID = objCharacter.TargetGUID];
            ...
            ThreadPool.QueueUserWorkItem(new WS_Spells.CastSpellParameters(ref Targets2, ref Caster, SpellID).CastAsync);
            objCharacter.CommandResponse(
                $"You are now casting [{Conversions.ToString(SpellID)}] at [{WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID].Name}].");
        } else if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID) &&
            WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            WS_Spells.SpellTargets Targets = new();
            Dictionary<ulong, WS_PlayerData.CharacterObject> cHARACTERs;
            ulong targetGUID;
            WS_Base.BaseUnit objCharacter2 = (cHARACTERs = WorldServiceLocator.WorldServer.CHARACTERs)[
                targetGUID = objCharacter.TargetGUID];
            Targets.SetTarget_UNIT(ref objCharacter2);
            cHARACTERs[targetGUID] = (WS_PlayerData.CharacterObject)objCharacter2;
            WS_Base.BaseObject Caster = objCharacter;
            objCharacter = (WS_PlayerData.CharacterObject)Caster;
            ThreadPool.QueueUserWorkItem(new WS_Spells.CastSpellParameters(ref Targets, ref Caster, SpellID).CastAsync);
            objCharacter.CommandResponse(
                $"You are now casting [{Conversions.ToString(SpellID)}] at [{WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name}].");
        } else
        {
    ...
    [ChatCommand(
        "castspell",
        "castspell #spellid #target - Selected unit will start casting spell. Target can be ME or SELF.",
        AccessLevel.Developer)]
    public bool cmdCastSpellMe(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
            ...
            WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(objCharacter.TargetGUID))
        {
            if(Operators.CompareString(Target, "ME", TextCompare: false) != 0)
            {
                ...
                {
                    WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID].CastSpell(
                        SpellID,
                        WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID]);
                }
            } else
            {
                WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID].CastSpell(
            ...
            WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            } else
            {
                ...
                ulong targetGUID;
                Dictionary<ulong, WS_PlayerData.CharacterObject> cHARACTERs;
                WS_Base.BaseObject Caster = (cHARACTERs = WorldServiceLocator.WorldServer.CHARACTERs)[
                    targetGUID = objCharacter.TargetGUID];
                ...
                ThreadPool.QueueUserWorkItem(callBack: castParams.CastAsync);
            }
        } else
        {
            objCharacter.CommandResponse($"GUID=[{objCharacter.TargetGUID:X}] not found or unsupported.");
    ...
    [ChatCommand(
        "changepassword",
        "changepassword #account #password - Changes the password of an account.",
        AccessLevel.Admin)]
    public bool cmdChangePassword(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(Operators.CompareString(Message, string.Empty, TextCompare: false) == 0)
        {
            return false;
        }
        DataTable result = new();
        ...
        WorldServiceLocator.WorldServer.AccountDatabase
            .Query($"SELECT id, gmlevel FROM account WHERE username = \"{aName}\";", ref result);
        if(result.Rows.Count == 0)
        {
        ...
        } else
        {
            ...
            {
                objCharacter.CommandResponse(
                    "You cannot change password for accounts with the same or a higher access level than yourself.");
            } else
            {
                var passwordStr = Encoding.ASCII.GetBytes($"{aName.ToUpper()}:{aPassword.ToUpper()}");
                var passwordHash = SHA1.HashData(passwordStr);
                var hashStr = BitConverter.ToString(passwordHash).Replace("-", string.Empty);
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update(
                        $"UPDATE account SET password='{hashStr}' WHERE id={RuntimeHelpers.GetObjectValue(result.Rows[0]["id"])}");
    ...
    public bool cmdClearCooldowns(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else if(WorldServiceLocator.CommonGlobalFunctions.GuidIsCreature(objCharacter.TargetGUID) &&
            WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            case WS_PlayerData.CharacterObject _:
                ...
                foreach(var Spell in ((WS_PlayerData.CharacterObject)targetUnit).Spells)
                {
                    if(Spell.Value.CoolDown != 0)
                    {
                        ...
                        WorldServiceLocator.WorldServer.CharacterDatabase
                            .Update(
                                $"UPDATE characters_spells SET cooldown={0}, cooldownitem={0} WHERE guid = {objCharacter.GUID} AND spellid = {Spell.Key};");
                        cooldownSpells.Add(Spell.Key);
                    }
                }
                foreach(var SpellID in cooldownSpells)
                {
                    Packets.Packets.PacketClass packet = new(Opcodes.SMSG_CLEAR_COOLDOWN);
    ...
    {
        var combatList = ((decimal.Compare(new decimal(objCharacter.TargetGUID), 0m) == 0) ||
                !WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID))
            ? objCharacter.inCombatWith.ToArray()
            : WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].inCombatWith.ToArray();
    ...
    public bool cmdComeCreature(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if((creature.aiScript?.InCombat) == true)
        {
    ...
    public bool cmdControl(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(objCharacter.MindControl != null)
        {
            ...
            {
                Packets.Packets.PacketClass packet1 = new(Opcodes.SMSG_DEATH_NOTIFY_OBSOLETE);
        ...
        if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID) &&
            WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            Packets.Packets.PacketClass packet2 = new(Opcodes.SMSG_DEATH_NOTIFY_OBSOLETE);
    ...
    [ChatCommand(
        "createaccount",
        "createaccount #account #password #email - Add a New account using Name, Password, And Email.",
        AccessLevel.Admin)]
    public bool cmdCreateAccount(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        var aEmail = acct[2];
        WorldServiceLocator.WorldServer.AccountDatabase
            .Query($"SELECT username FROM account WHERE username = \"{aName}\";", ref result);
        if(result.Rows.Count > 0)
        {
        ...
        } else
        {
            ...
            WorldServiceLocator.WorldServer.AccountDatabase
                .Insert(
                    $"INSERT INTO account (username, sha_pass_hash, email, joindate, last_ip) VALUES ('{aName}', '{hashStr}', '{aEmail}', '{Strings.Format(DateAndTime.Now, "yyyy-MM-dd")}', '0.0.0.0')");
    ...
    public bool cmdCreateGuild(ref WS_PlayerData.CharacterObject objCharacter, string Message) { return true; }
    ...
    public bool cmdDeleteObject(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else if(WorldServiceLocator.CommonGlobalFunctions.GuidIsGameObject(objCharacter.TargetGUID))
        {
            ...
            WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].Destroy(
                WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID]);
            objCharacter.CommandResponse("Game object deleted.");
        }
        return true;
    }

    [ChatCommand("EditMovementFlag", "EDITPLAYERFLAG <PLAYERFLAG> - Change your playerflag.", AccessLevel.Developer)]
    public bool cmdEditMovementflag(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if((!Information.IsNumeric(Message)) && (Strings.InStr(Message, "0x") == 0))
        {
            return false;
        }

        if(Strings.InStr(Message, "0x") > 0)
        {
            objCharacter.charMovementFlags = (int)(MovementFlags)Conversion.Val(
                $"&H{Message.Replace("0x", string.Empty)}");
        } else
        {
            objCharacter.charMovementFlags = (int)(MovementFlags)Conversion.Val(Message);
        }
        objCharacter.SetUpdateFlag((int)(MovementFlags)Conversion.Val(Message), objCharacter.charMovementFlags);
        objCharacter.SendCharacterUpdate();

        return true;
    }

    [ChatCommand("EditPlayerFlag", "EDITPLAYERFLAG <PLAYERFLAG> - Change your playerflag.", AccessLevel.Developer)]
    public bool cmdEditPlayerflag(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if((!Information.IsNumeric(Message)) && (Strings.InStr(Message, "0x") == 0))
        {
            return false;
        ...
        if(Strings.InStr(Message, "0x") > 0)
        {
            objCharacter.cPlayerFlags = (PlayerFlags)Conversion.Val($"&H{Message.Replace("0x", string.Empty)}");
        } else
        {
            objCharacter.cPlayerFlags = (PlayerFlags)Conversion.Val(Message);
        }
    ...
    public bool cmdFlyOn(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(decimal.Compare(new decimal(objCharacter.TargetGUID), 0m) == 0)
        {
            objCharacter.CommandResponse("Select target first!");
    ...
    public bool cmdForceRename(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Update(
                    $"UPDATE characters SET force_restrictions = 1 WHERE char_guid = {objCharacter.TargetGUID};");
    ...
    [ChatCommand("gcinfo", "gobjectnear - Tells you the near objects count.", AccessLevel.Developer)]
    public bool cmdGarbageCollectionInfo(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        objCharacter.CommandResponse("Garbage Collection Info:");
        objCharacter.CommandResponse("-------------------------------");
        objCharacter.CommandResponse($"Max Generation: {GC.MaxGeneration}");
        objCharacter.CommandResponse($"Total Allocated Bytes: {GC.GetTotalAllocatedBytes()}");
        objCharacter.CommandResponse($"Total Memory: {GC.GetTotalMemory(true)}");
        objCharacter.CommandResponse($"Total Pause Duration: {GC.GetTotalPauseDuration()}");
        return true;
    }

    [ChatCommand(
        "skillmaster",
        "skillmaster - Get all spells and skills maxed out for your level.",
        AccessLevel.Developer)]
    public bool cmdGetMax(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        checked
        {
            foreach(var skill in objCharacter.Skills)
            {
                ...
                objCharacter.SetUpdateFlag(
                    718 + (objCharacter.SkillsPositions[skill.Key] * 3) + 1,
                    objCharacter.Skills[skill.Key].GetSkill);
            }
            objCharacter.SendCharacterUpdate(toNear: false);
            return true;
        }
    ...
    public bool cmdGoToGraveyard(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ulong targetGUID;
            Dictionary<ulong, WS_PlayerData.CharacterObject> cHARACTERs;
            var Character = (cHARACTERs = WorldServiceLocator.WorldServer.CHARACTERs)[
                targetGUID = objCharacter.TargetGUID];
    ...
    [ChatCommand(
        "tostart",
        "tostart #race - Instantly teleports selected character to specified race start location.")]
    public bool cmdGoToStart(ref WS_PlayerData.CharacterObject objCharacter, string StringRace)
    {
        ...
        if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            WorldServiceLocator.WorldServer.WorldDatabase
                .Query($"SELECT * FROM playercreateinfo WHERE race = {(int)Race};", ref Info);
            var Character = WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID];
            Character.Teleport(
                Conversions.ToSingle(Info.Rows[0]["position_x"]),
                Conversions.ToSingle(Info.Rows[0]["position_y"]),
                Conversions.ToSingle(Info.Rows[0]["position_z"]),
                Conversions.ToSingle(Info.Rows[0]["orientation"]),
                Conversions.ToInteger(Info.Rows[0]["map"]));
        }
        return true;
    }
    ...
    [ChatCommand("heal", "heal - Heals a selected character.")]
    public bool cmdHeal(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if (decimal.Compare(new decimal(objCharacter.TargetGUID), 0m) == 0)
        {
            objCharacter.CommandResponse("Select target first!");
        ...
        if (WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            (life = WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Life).Current = checked((int)Math.Round(
                life.Current +
                    (WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Life.Maximum * 0.25)));
            WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].SetUpdateFlag(
                22,
                WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Life.Current);
            WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].SendCharacterUpdate();
        }
        return true;
    }
    ...
    [ChatCommand("invulnerable", "set invulnerable #flag - Toggle invulnerability status. You can use values like On/Off.")]
    public bool cmdSetInvulnerable(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        var tmp = string.Join(Message, " ", 2);
        var value1 = tmp[0];

        switch (Operators.CompareString(
            WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(tmp),
            "on",
            TextCompare: false))
        {
            case 0:
                objCharacter.Invulnerable = true;
                objCharacter.CommandResponse("You're now Invulnerable.");
                break;
            ...
            default:
                if (Operators.CompareString(
                    ...
                    0)
                {
                    objCharacter.Invulnerable = false;
                    objCharacter.CommandResponse("You're now Vulnerable.");
                }
    ...
    public bool cmdKick(ref WS_PlayerData.CharacterObject objCharacter, string Name)
    {
        if(Operators.CompareString(Name, string.Empty, TextCompare: false) == 0)
        {
            if(objCharacter.client == null)
            {
                return false;
            }
            if (objCharacter.client.Character == null)
            {
                return false;
            }
            if (objCharacter.client.Character.Name == null)
            {
                return false;
            }
            ...
            {
                objCharacter.CommandResponse(
                    $"Character [{WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name}] kicked from server.");
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.INFORMATION,
                        "[{0}:{1}] Character [{3}] kicked by [{2}].",
                        objCharacter.client.IP,
                        objCharacter.client.Port,
                        objCharacter.client.Character.Name,
                        WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name);
        ...
        } else
        {
            ...
            {
                if(Operators.CompareString(
                        WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Character.Value.Name),
                        Name,
                        TextCompare: false) ==
                    0)
                {
    ...
    [ChatCommand(
        "learnskill",
        "learnskill #id #current #max - Add skill id X with value Y of Z to selected character.",
        AccessLevel.Developer)]
    public bool cmdLearnSkill(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            {
                WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].LearnSkill(
                    SkillID,
                    Current,
                    Maximum);
            }
    ...
    {
        if(Operators.CompareString(tID, string.Empty, TextCompare: false) == 0)
        {
            return false;
        }
        ...
        if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            {
                objCharacter.CommandResponse(
                    $"{WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name} has learned spell: {Conversions.ToString(ID)}");
            }
        } else
        {
            objCharacter.CommandResponse("Target not found or not character.");
    ...
    [ChatCommand("mapinfo", "gobjectnear - Tells you the near objects count.", AccessLevel.Developer)]
    public bool cmdMapInfo(
        ...
        string Message)
    {
        objCharacter.CommandResponse("Map Info:");
        objCharacter.CommandResponse("-------------------------------");
        //objCharacter.CommandResponse("Area Flag: " + WorldServiceLocator.WSMaps.GetAreaFlag(x, y, map));
        //objCharacter.CommandResponse("Area ID: " + WorldServiceLocator.WSMaps.GetAreaIDByMapandParent(mapid, parentID));
        //objCharacter.CommandResponse("Map Tile X: " + WorldServiceLocator.WSMaps.GetMapTileX(x));
        //objCharacter.CommandResponse("Map Tile Y: " + WorldServiceLocator.WSMaps.GetMapTileY(y));
        //objCharacter.CommandResponse("Sub Map Tile X: " + WorldServiceLocator.WSMaps.GetSubMapTileX(x));
        //objCharacter.CommandResponse("Sub Map Tile Y: " + WorldServiceLocator.WSMaps.GetSubMapTileY(y));
        //objCharacter.CommandResponse("Terrain Type: " + WorldServiceLocator.WSMaps.GetTerrainType(x, y, map));
        //objCharacter.CommandResponse("Vmap Height: " + WorldServiceLocator.WSMaps.GetVMapHeight(umapid, x, y, z));
        //objCharacter.CommandResponse("Outside of Map: " + WorldServiceLocator.WSMaps.IsOutsideOfMap(ref BaseObjCharacter));
        //objCharacter.CommandResponse("Map Name: " + WorldServiceLocator.WSMaps.Maps[0].Name);
        //objCharacter.CommandResponse("Map ID: " + WorldServiceLocator.WSMaps.Maps[0].ID);
        //objCharacter.CommandResponse("Map Tiles: " + WorldServiceLocator.WSMaps.Maps[0].Tiles);
        //objCharacter.CommandResponse("Map Tiles Used: " + WorldServiceLocator.WSMaps.Maps[0].TileUsed);
        //objCharacter.CommandResponse("Map Type: " + WorldServiceLocator.WSMaps.Maps[0].Type);
        //objCharacter.CommandResponse("Map Reset Time: " + WorldServiceLocator.WSMaps.Maps[0].ResetTime);
        //objCharacter.CommandResponse("Is in Dungeon: " + WorldServiceLocator.WSMaps.Maps[0].IsDungeon);
        //objCharacter.CommandResponse("Is in Raid: " + WorldServiceLocator.WSMaps.Maps[0].IsRaid);
        //objCharacter.CommandResponse("Is in BG: " + WorldServiceLocator.WSMaps.Maps[0].IsBattleGround);
        return true;
    }
    ...
    public bool cmdNear(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        objCharacter.CommandResponse(
            $"Dynamic Objects: {Conversions.ToString(objCharacter.dynamicObjectsNear.Count)}");
    ...
    public bool cmdNotificationMessage(ref WS_PlayerData.CharacterObject objCharacter, string Text)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_NOTIFICATION);
    ...
    [ChatCommand(
        "nullchar",
        " - Set's Character Object to Null, forcing an NullReferenceException to throw on actions peformed after issuing this command.",
        AccessLevel.Developer)]
    ...
    [ChatCommand(
        "playsound",
        "playsound - Plays a specific sound for every player around you.",
        AccessLevel.Developer)]
    ...
    public bool cmdPort(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        var tmp = Message.Split(new string[1] { " " }, StringSplitOptions.RemoveEmptyEntries);
        if(tmp.Length != 5)
        {
            return false;
        }
    ...
    {
        if(Operators.CompareString(location, string.Empty, TextCompare: false) == 0)
        {
            return false;
        }
        if(Operators.CompareString(
                WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(location),
                "LIST",
                TextCompare: false) ==
            0)
        {
            ...
            WorldServiceLocator.WorldServer.WorldDatabase
                .Query("SELECT * FROM game_tele order by name", ref listSqlQuery);
            ...
            try
            {
                ...
                while(enumerator.MoveNext())
                {
                    ...
                    cmdList = Conversions.ToString(
                        Operators.AddObject(cmdList, Operators.ConcatenateObject(row["name"], ", ")));
                }
            } finally
            {
        ...
        if(location.Contains('*'))
        {
            location = location.Replace("*", string.Empty);
            WorldServiceLocator.WorldServer.WorldDatabase
                .Query($"SELECT * FROM game_tele WHERE name like '{location}%' order by name;", ref mySqlQuery);
        } else
        {
            WorldServiceLocator.WorldServer.WorldDatabase
                .Query($"SELECT * FROM game_tele WHERE name = '{location}' order by name LIMIT 1;", ref mySqlQuery);
        }
            ...
            if(mySqlQuery.Rows.Count != 1)
            {
                ...
                try
                {
                    ...
                    while(enumerator2.MoveNext())
                    {
                        ...
                        cmdList2 = Conversions.ToString(
                            Operators.AddObject(cmdList2, Operators.ConcatenateObject(row["name"], ", ")));
                    }
                } finally
                {
    ...
    public bool cmdResetFactions(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
            ...
            WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ulong targetGUID;
            Dictionary<ulong, WS_PlayerData.CharacterObject> Characters;
            var objCharacter2 = (Characters = WorldServiceLocator.WorldServer.CHARACTERs)[
                targetGUID = objCharacter.TargetGUID];
    ...
    public bool cmdRespawnCreature(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        creature.Respawn();
        objCharacter.CommandResponse(
            $"Creature [{WorldServiceLocator.Functions.SetColor(creature.Name, 4, 147, 11)}] Respawned.");
        return true;
    }
    ...
    public bool cmdSave(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
            ...
            WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID))
        {
            ...
            WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].CommandResponse(
                $"Character {WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name} saved.");
        } else
        {
            objCharacter.Save();
    ...
    public bool cmdSay(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.CommonGlobalFunctions.GuidIsCreature(objCharacter.TargetGUID))
        {
            WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID].SendChatMessage(
                Message,
                ChatMsg.CHAT_MSG_MONSTER_SAY,
                LANGUAGES.LANG_GLOBAL,
                objCharacter.GUID);
            return true;
        }
        return false;
    ...
    [ChatCommand("serverinfo", "gobjectnear - Tells you the near objects count.", AccessLevel.Developer)]
    public bool cmdServerInfo(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        objCharacter.CommandResponse("Server Info:");
        objCharacter.CommandResponse("-------------------------------");
        objCharacter.CommandResponse($"Client Count: {WorldServiceLocator.WorldServer.CLIENTs.Count}");
        objCharacter.CommandResponse($"Cluster time: {WorldServiceLocator.WSNetwork.WC_MsTime}");
        objCharacter.CommandResponse($"World time: {WorldServiceLocator.WSNetwork.MsTime()}");
        objCharacter.CommandResponse($"Character Count: {WorldServiceLocator.WorldServer.CHARACTERs.Count}");
        objCharacter.CommandResponse($"Pending Threadpool Count: {ThreadPool.PendingWorkItemCount}");
        objCharacter.CommandResponse($"Completed Threadpool Count: {ThreadPool.CompletedWorkItemCount}");
        objCharacter.CommandResponse($"Thread Count: {ThreadPool.ThreadCount}");
        return true;
    }
    ...
    public bool cmdServerMessage(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        var Type = Conversions.ToInteger(tmp[0]);
        var Text = tmp[1];
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_SERVER_MESSAGE);
    ...
    [ChatCommand(
        "setaccess",
        "setaccess #account #level - Sets the account to a specific access level.",
        AccessLevel.Admin)]
    public bool cmdSetAccess(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else
        {
            ...
            {
                objCharacter.CommandResponse(
                    "You cannot set access levels to accounts with the same or a higher access level than yourself.");
            } else
            {
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update(
                        $"UPDATE account SET gmlevel={(byte)newLevel} WHERE id={RuntimeHelpers.GetObjectValue(result.Rows[0]["id"])}");
    ...
    public bool cmdSetCharacterSpeed(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        objCharacter.ChangeSpeedForced(ChangeSpeedType.RUNBACK, Conversions.ToSingle(Message));
        ...
        objCharacter.CommandResponse($"Your SwimBackSpeed is changed to {Message}");
        return true;
    }
    ...
    public bool cmdSetGM(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        var value2 = tmp[1];
        switch(Operators.CompareString(
            WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(value1),
            "on",
            TextCompare: false))
        {
            case 0:
        ...
        switch(Operators.CompareString(
            WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(value2),
            "on",
            TextCompare: false))
        {
            case 1:
    ...
    //[ChatCommand("gobjectnear", "gobjectnear - Tells you the near objects count.", AccessLevel.Developer)]
    //public bool cmdNear2(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    //{
    //    WorldServiceLocator.;
    //    objCharacter.CommandResponse("You are seen by: " + Conversions.ToString(objCharacter.SeenBy.Count));
    //    return true;
    ...
    [ChatCommand(
        "playercombat",
        "npcai #enable/disable - Enables/Disables  Creature AI updating.",
        AccessLevel.Developer)]
    public bool cmdSetPlayerCombat(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(Operators.CompareString(
                WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message),
                "ENABLE",
            ...
            0)
        {
            if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID) &&
                WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
            {
                WorldServiceLocator.WSCombat.SetPlayerInCombat(ref objCharacter.client.Character);
            }
            objCharacter.CommandResponse("Player combat is enabled.");
        } else
        {
            if(Operators.CompareString(
                    WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message),
            ...
            if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(objCharacter.TargetGUID) &&
                WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
            {
                WorldServiceLocator.WSCombat.SetPlayerOutOfCombat(ref objCharacter.client.Character);
            }
            objCharacter.CommandResponse("Player combat is disabled.");
        }
        return true;
    }
    ...
    public bool cmdSpawns(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        objCharacter.CommandResponse("-------------------------------");
        objCharacter.CommandResponse(
            $"Creatures: {Conversions.ToString(WorldServiceLocator.WorldServer.WORLD_CREATUREs.Count)}");
        objCharacter.CommandResponse(
            $"GameObjects: {Conversions.ToString(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs.Count)}");
        objCharacter.CommandResponse(
            $"Dynamic Objects: {Conversions.ToString(WorldServiceLocator.WorldServer.WORLD_DYNAMICOBJECTs.Count)}");
        objCharacter.CommandResponse(
            $"Items: {Conversions.ToString(WorldServiceLocator.WorldServer.WORLD_ITEMs.Count)}");
        objCharacter.CommandResponse(
            $"Transports: {Conversions.ToString(WorldServiceLocator.WorldServer.WORLD_TRANSPORTs.Count)}");
        return true;
    }

    [ChatCommand(
        "spellupdate",
        "spellupdate #enable/disable - Enables/Disables  Spell Manager updating.",
        AccessLevel.Developer)]
    public bool cmdSpellManager(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        if(Operators.CompareString(
                WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message),
                "ENABLE",
            ...
            0)
        {
            WorldServiceLocator.WSTimerBasedEvents.SpellManager.SpellManagerTimer.Change(1000, 1000);
            objCharacter.CommandResponse("Spell updating is enabled.");
        } else
        {
            if(Operators.CompareString(
                    WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message),
            ...
            WorldServiceLocator.WSTimerBasedEvents.SpellManager.SpellManagerTimer.Change(-1, -1);
            objCharacter.CommandResponse("Spell updating is disabled.");
        }
        return true;
    }
    ...
    [ChatCommand(
        "stresstest",
        " - Causes a huge CPU Spike, simulating very high load, leak situations.",
        AccessLevel.Developer)]
    public bool cmdStressTest(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        const string hexString = "XYZ";
        ...
        checked
        {
            ...
            if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(GUID))
            {
                if(objCharacter.OnTransport != null)
                {
                    ...
                    WorldServiceLocator.WorldServer.CHARACTERs[GUID].Transfer(
                        objCharacter.positionX,
                        objCharacter.positionY,
                        objCharacter.positionZ,
                        objCharacter.orientation,
                        (int)objCharacter.MapID);
                } else
                {
                    WorldServiceLocator.WorldServer.CHARACTERs[GUID].Teleport(
                        objCharacter.positionX,
                        objCharacter.positionY,
                        objCharacter.positionZ,
                        objCharacter.orientation,
    ...
    [ChatCommand("swimcheat", "control - Takes or removes control over the selected unit.", AccessLevel.GameMaster)]
    public bool cmdSwimCheat(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        Packets.Packets.PacketClass CMSG_MOVE_START_SWIM_CHEAT = new(Opcodes.MSG_MOVE_START_SWIM);
        try
        {
            CMSG_MOVE_START_SWIM_CHEAT.AddPackGUID(objCharacter.GUID);
            CMSG_MOVE_START_SWIM_CHEAT.AddInt8(1);
            objCharacter.client.Send(ref CMSG_MOVE_START_SWIM_CHEAT);
        } finally
        {
            CMSG_MOVE_START_SWIM_CHEAT.Dispose();
        }
        return true;
    }
    ...
    public bool cmdTargetGameObject(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        } else
        {
            ...
            objCharacter.CommandResponse(
                $"Selected [{targetGO.ID}][{targetGO.Name}] game object at distance {distance}.");
        }
        return true;
    }
    ...
    [ChatCommand(
        "turn",
        "turn - Selected creature or game object will turn to your position.",
        AccessLevel.Developer)]
    public bool cmdTurnObject(ref WS_PlayerData.CharacterObject objCharacter, string Message)
    {
        ...
        if(WorldServiceLocator.CommonGlobalFunctions.GuidIsCreature(objCharacter.TargetGUID))
        {
            ...
            WorldServiceLocator.WorldServer.WORLD_CREATUREs[objCharacter.TargetGUID].TurnTo(
                objCharacter.positionX,
                objCharacter.positionY);
        } else if(WorldServiceLocator.CommonGlobalFunctions.GuidIsGameObject(objCharacter.TargetGUID))
        {
            ...
            WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[objCharacter.TargetGUID].TurnTo(
                objCharacter.positionX,
                objCharacter.positionY);
            DataTable _ = new();
    ...
    public bool cmdUnBan(ref WS_PlayerData.CharacterObject objCharacter, string Name)
    {
        ...
        if(result.Rows.Count > 0)
        {
            ...
            {
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update($"UPDATE account_banned SET active = 0 WHERE id = '{Conversions.ToString(accountID)}';");
                WorldServiceLocator.WorldServer.AccountDatabase
                    .Update($"DELETE FROM `ip_banned` WHERE `ip` = '{IP}';");
                objCharacter.CommandResponse($"Account [{Name}] unbanned.");
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.INFORMATION,
                        "[{0}:{1}] Account [{3}] unbanned by [{2}].",
                        objCharacter.client.IP,
                        objCharacter.client.Port,
                        objCharacter.Name,
                        Name);
    ...
    [ChatCommand(
        "unlearnspell",
        "unlearnspell #id - Remove chosen spell from selected character.",
        AccessLevel.Developer)]
    public bool cmdUnlearnSpell(ref WS_PlayerData.CharacterObject objCharacter, string tID)
    {
        ...
        if(WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter.TargetGUID))
        {
            ...
            {
                objCharacter.CommandResponse(
                    $"{WorldServiceLocator.WorldServer.CHARACTERs[objCharacter.TargetGUID].Name} has unlearned spell: {Conversions.ToString(ID)}");
            }
        } else
        {
            objCharacter.CommandResponse("Target not found or not character.");
    ...
    {
        if(Operators.CompareString(Strings.Trim(Message), string.Empty, TextCompare: false) != 0)
        {
            var Command2 = ChatCommands[
                Strings.Trim(WorldServiceLocator.CommonFunctions.UppercaseFirstLetter(Message))];
            switch(Command2)
            {
                case null:
    ...
    public void OnCommand(ref WS_Network.ClientClass client, string Message)
    {
        try
        {
            ...
            if(Command.CommandDelegate == null) //Bind Failure
            {
                client.Character
                    .CommandResponse(
                        "This command has encountered an internal error and was unable to be registered.");
                return;
            }
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.USER, "[{0}:{1}] {2} used command: {3}", client.IP, client.Port, Name, Message);
        } catch(Exception err)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.FAILED,
                    "[{0}:{1}] Client command caused error! {3}{2}",
                    client.IP,
                    client.Port,
                    err.ToString(),
                    Environment.NewLine);
            client.Character
                .CommandResponse(
                    string.Format($"Your command caused error:{Environment.NewLine} [{{0}}]", err.Message));
        }
    }
    ...
    public void RegisterChatCommands()
    {
        try
        {
            foreach(var tmpModule in Assembly.GetExecutingAssembly().GetTypes())
            {
                foreach(var tmpMethod in tmpModule.GetMethods())
                {
                    var infos = (ChatCommandAttribute[])tmpMethod.GetCustomAttributes(
                    ...
                    if(infos.Length != 0)
                    {
                        foreach(var info in infos)
                        {
                            var cmd = (ChatCommand)(new()
                            {
                                ...
                                CommandDelegate = (ChatCommandDelegate)Delegate.CreateDelegate(typeof(ChatCommandDelegate), WorldServiceLocator.WSCommands, tmpMethod, false) //Set to True for quick debugging with Bind Failure
                            });
        ...
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.FAILED, "Command Register Failed", Environment.StackTrace, Environment.NewLine);
        }
    }
    ...
    public bool SetUpdateValue(ulong GUID, int Index, int Value, WS_Network.ClientClass client)
    {
        ...
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.WARNING, "WS_Commands: SetUpdateValue DataException", ex);
            noErrors = false;
        }
        return noErrors;
    }
    ...
    public class ChatCommand : IComparable, IComparable<ChatCommand>, IEquatable<ChatCommand>
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            if (obj is not ChatCommand other)
            {
                throw new ArgumentException(nameof(obj) + " is not a " + nameof(ChatCommand));
            }
        ...
        public int CompareTo(ChatCommand other)
        {
            ...
            var result = 0;
            result = CommandAccess.CompareTo(other.CommandAccess);
            if (result != 0)
            {
                return result;
            }

            result = CommandHelp.CompareTo(other.CommandHelp);
            if (result != 0)
            {
                return result;
            }
        ...
        {
            if (obj is ChatCommand chatCommand)
            {
                return Equals(chatCommand);
            }

            return base.Equals(obj);
        }

        public static bool operator ==(ChatCommand first, ChatCommand second)
        {
            if (first is null)
            {
        ...
        public bool Equals(ChatCommand other)
        {
            ...
            return CommandAccess.Equals(other.CommandAccess) && Equals(CommandDelegate, other.CommandDelegate) && Equals(CommandHelp, other.CommandHelp);
        }
        ...
        public override int GetHashCode()
        {
            ...
            {
                var hashCode = 47;
                hashCode = (hashCode * 53) ^ (int)CommandAccess;
                if (CommandDelegate != null)
                {
                    hashCode = (hashCode * 53) ^ EqualityComparer<ChatCommandDelegate>.Default.GetHashCode(CommandDelegate);
                }

                if (CommandHelp != null)
                {
                    hashCode = (hashCode * 53) ^ EqualityComparer<string>.Default.GetHashCode(CommandHelp);
                }
