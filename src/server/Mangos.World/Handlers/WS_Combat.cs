...
public class WS_Combat
{
    public DamageInfo CalculateDamage(
        ref WS_Base.BaseUnit Attacker,
        ref WS_Base.BaseUnit Victim,
        bool DualWield,
        bool Ranged,
        WS_Spells.SpellInfo Ability = null,
        WS_Spells.SpellEffect Effect = null)
    {
        if(Attacker is null)
        {
        ...
        if((Victim is WS_Creatures.CreatureObject @object) &&
            ((@object.aiScript?.State) == AIState.AI_MOVING_TO_SPAWN))
        {
            result.HitInfo |= 16;
            return result;
        }
        ...
        checked
        {
            ...
            if((Attacker is WS_PlayerData.CharacterObject) &&
                (Victim is WS_Creatures.CreatureObject) &&
                (Attacker.Level > (Victim.Level + 2)) &&
                (skillDiference <= (-15)))
            {
                chanceToGlancingBlow = (short)((Victim.Level - Attacker.Level) * 10);
            }
            short chanceToCrushingBlow = 0;
            if((Attacker is WS_Creatures.CreatureObject) &&
                (Victim is WS_PlayerData.CharacterObject) &&
                (Ability == null) &&
                (Attacker.Level > (Victim.Level + 2)))
            {
            ...
            var num = (float)(WorldServiceLocator.WorldServer.Rnd.Next(0, 10000) / 100.0);
            if(num < chanceToMiss)
            {
                result.Damage = 0;
                result.HitInfo |= 16;
            ...
            } else if(num < (chanceToMiss + chanceToDodge + chanceToParry + chanceToGlancingBlow + chanceToBlock))
            {
                ...
                (chanceToMiss + chanceToDodge + chanceToParry + chanceToGlancingBlow + chanceToBlock + chanceToCrit))
            {
                result.Damage *= 2;
                result.HitInfo |= 2;
                result.HitInfo |= 512;
                ...
                (chanceToMiss +
                    chanceToDodge +
                    chanceToParry +
                    chanceToGlancingBlow +
                    chanceToBlock +
                    chanceToCrit +
                    chanceToCrushingBlow))
            {
    ...
    public void DoEmote(int AnimationID, ref WS_Base.BaseObject Unit)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_EMOTE);
    ...
    public float GetAPMultiplier(ref WS_Base.BaseUnit objCharacter, WeaponAttackType AttackType, bool Normalized)
    {
        ...
        if(!Normalized || (objCharacter is not WS_PlayerData.CharacterObject))
        {
            ...
            {
                WeaponAttackType.BASE_ATTACK => ((WS_Creatures.CreatureObject)objCharacter).CreatureInfo.BaseAttackTime /
                    1000f,
                WeaponAttackType.RANGED_ATTACK => ((WS_Creatures.CreatureObject)objCharacter).CreatureInfo.BaseRangedAttackTime /
                    1000f,
                _ => 0f,
            };
        }
    ...
    public float GetBasePercentBlock(ref WS_Base.BaseUnit objCharacter, int skillDiference)
    {
        ...
        {
            WS_PlayerData.CharacterObject _ when ((WS_PlayerData.CharacterObject)objCharacter).combatBlock > 0 => ((WS_PlayerData.CharacterObject)objCharacter).combatBlock -
                (skillDiference * 0.04f),
            _ => 0f
        };
    }
    ...
    public float GetBasePercentCrit(ref WS_Base.BaseUnit objCharacter, int skillDiference)
    {
        ...
        switch(objCharacter)
        {
            case WS_PlayerData.CharacterObject _:
                var baseCrit = 0f;
                switch(objCharacter.Class)
                {
                    case Classes.CLASS_ROGUE:
                        baseCrit = (float)(0.0 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 29.0));
                        break;

                    case Classes.CLASS_DRUID:
                        baseCrit = (float)(0.92000001668930054 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 20.0));
                        break;

                    case Classes.CLASS_HUNTER:
                        baseCrit = (float)(0.0 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 33.0));
                        break;

                    case Classes.CLASS_MAGE:
                        baseCrit = (float)(3.2000000476837158 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 19.44));
                        break;

                    case Classes.CLASS_PALADIN:
                        baseCrit = (float)(0.699999988079071 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 19.77));
                        break;
                    ...
                    case Classes.CLASS_SHAMAN:
                        baseCrit = (float)(1.7000000476837158 +
                            (((WS_PlayerData.CharacterObject)objCharacter).Agility.Base / 19.7));
                        break;
    ...
    public float GetBasePercentDodge(ref WS_Base.BaseUnit objCharacter, int skillDiference)
    {
        ...
        if(objCharacter is WS_PlayerData.CharacterObject @object)
        {
            ...
            checked
            {
                ...
                {
                    var combatDodgeAgilityBonus = @object.Class switch
                    {
                        Classes.CLASS_HUNTER => (int)(@object.Agility.Base / 26.5f),
                        Classes.CLASS_ROGUE => (int)(@object.Agility.Base / 14.5f),
                        Classes.CLASS_PALADIN or Classes.CLASS_MAGE or Classes.CLASS_WARLOCK => (int)(@object.Agility.Base /
                            19.5f),
    ...
    public float GetBasePercentMiss(ref WS_Base.BaseUnit objCharacter, int skillDiference)
    {
        ...
        if (objCharacter is WS_PlayerData.CharacterObject characterObject && characterObject.attackSheathState == SHEATHE_SLOT.SHEATHE_WEAPON)
        {
            if (characterObject.Items.ContainsKey(16) &&
                (characterObject.Items[16].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_WEAPON))
            {
    ...
    public float GetBasePercentParry(ref WS_Base.BaseUnit objCharacter, int skillDiference)
    {
        ...
        return objCharacter switch
        {
            WS_PlayerData.CharacterObject _ when ((WS_PlayerData.CharacterObject)objCharacter).combatParry > 0 => ((WS_PlayerData.CharacterObject)objCharacter).combatParry -
                (skillDiference * 0.04f),
            _ => 0f
        };
    }
        ...
        checked
        {
            if(objCharacter is WS_PlayerData.CharacterObject characterObject)
            {
                switch(characterObject.attackSheathState)
                {
                    case SHEATHE_SLOT.SHEATHE_NONE:
                        result.HitInfo = 0;
                        result.DamageType = DamageTypes.DMG_PHYSICAL;
                        result.Damage = WorldServiceLocator.WorldServer.Rnd
                            .Next(characterObject.BaseUnarmedDamage, characterObject.BaseUnarmedDamage + 1);
                        break;
                    ...
                    case SHEATHE_SLOT.SHEATHE_WEAPON:
                        if(DualWield)
                        {
                            ...
                            result.Damage = WorldServiceLocator.WorldServer.Rnd
                                    .Next(
                                        (int)Math.Round(characterObject.OffHandDamage.Minimum / 2f),
                                        (int)Math.Round((characterObject.OffHandDamage.Maximum / 2f) + 1f)) +
                                characterObject.BaseUnarmedDamage;
                        } else
                        {
                            result.HitInfo = 2;
                            result.DamageType = DamageTypes.DMG_PHYSICAL;
                            result.Damage = WorldServiceLocator.WorldServer.Rnd
                                    .Next(
                                        (int)Math.Round(characterObject.Damage.Minimum),
                                        (int)Math.Round(characterObject.Damage.Maximum + 1f)) +
                                characterObject.BaseUnarmedDamage;
                        }
                        break;
                    ...
                    case SHEATHE_SLOT.SHEATHE_RANGED:
                        ...
                        result.Damage = WorldServiceLocator.WorldServer.Rnd
                                .Next(
                                    (int)Math.Round(characterObject.RangedDamage.Minimum),
                                    (int)Math.Round(characterObject.RangedDamage.Maximum + 1f)) +
                            characterObject.BaseRangedDamage;
                        break;
            ...
            } else
            {
                ...
                result.Damage = WorldServiceLocator.WorldServer.Rnd
                    .Next(
                        (int)Math.Round(
                            WorldServiceLocator.WorldServer.CREATURESDatabase[creatureObject.ID].Damage.Minimum),
                        (int)Math.Round(
                            WorldServiceLocator.WorldServer.CREATURESDatabase[creatureObject.ID].Damage.Maximum + 1f));
            }
        }
    }
            ...
            if(objCharacter is WS_PlayerData.CharacterObject characterObject)
            {
                ...
                switch(characterObject.attackSheathState)
                {
                    ...
                    case SHEATHE_SLOT.SHEATHE_WEAPON:
                        ...
                        {
                            tmpSkill = WorldServiceLocator.WorldServer.ITEMDatabase[
                                characterObject.Items[16].ItemEntry].GetReqSkill;
                        } else if(characterObject.Items.ContainsKey(15))
                        {
                            tmpSkill = WorldServiceLocator.WorldServer.ITEMDatabase[
                                characterObject.Items[15].ItemEntry].GetReqSkill;
                        }
                        break;
                    ...
                    case SHEATHE_SLOT.SHEATHE_RANGED:
                        ...
                        {
                            tmpSkill = WorldServiceLocator.WorldServer.ITEMDatabase[
                                characterObject.Items[17].ItemEntry].GetReqSkill;
                        }
                        break;
                }
                if(tmpSkill == 0)
        ...
        bool MaxDmg)
    {
        ...
        if(!objCharacter.Items.ContainsKey(WepSlot) ||
            (objCharacter.Items[WepSlot].ItemInfo.ObjectClass != ITEM_CLASS.ITEM_CLASS_WEAPON) ||
            objCharacter.Items[WepSlot].IsBroken())
        {
            return 0f;
        }
        var Dmg = 0f;
    ...
    public bool IsInBackOf(ref WS_Base.BaseObject Object1, float x2, float y2)
    {
        ...
        return (lowAngle < 0f)
            ? (((angle2 >= ((Math.PI * 2.0) + lowAngle)) && (angle2 <= (Math.PI * 2.0))) ||
                ((angle2 >= 0f) && (angle2 <= hiAngle)))
    ...
    public void On_CMSG_ATTACKSTOP(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
        ...
        try
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ATTACKSTOP", client.IP, client.Port);
        ...
        } catch(Exception ex)
        {
            ...
            var e = ex;
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.FAILED, "Error stopping attack: {0}", e.ToString());
            ProjectData.ClearProjectError();
        }
    }

    public void On_CMSG_ATTACKSWING(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        if((checked(packet.Data.Length - 1) >= 13) && (client.Character != null))
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ATTACKSWING [GUID={2:X}]", client.IP, client.Port, GUID);
            if(client.Character.Spell_Pacified)
            {
                Packets.Packets.PacketClass SMSG_ATTACKSWING_CANT_ATTACK2 = new(
                    Opcodes.SMSG_ATTACKSWING_CANT_ATTACK);
            ...
            {
                Packets.Packets.PacketClass SMSG_ATTACKSWING_CANT_ATTACK = new(Opcodes.SMSG_ATTACKSWING_CANT_ATTACK);
    ...
    public void On_CMSG_SET_AMMO(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        WorldServiceLocator.WorldServer.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_SET_AMMO [{2}]", client.IP, client.Port, AmmoID);
        ...
        } else if(AmmoID != 0)
        {
            ...
            var currentDPS = 0f;
            if((WorldServiceLocator.WorldServer.ITEMDatabase.ContainsKey(AmmoID) &&
                    (WorldServiceLocator.WorldServer.ITEMDatabase[AmmoID].ObjectClass ==
                        ITEM_CLASS.ITEM_CLASS_PROJECTILE)) ||
                WorldServiceLocator.CharManagementHandler.CheckAmmoCompatibility(ref client.Character, AmmoID))
            {
    ...
    public void On_CMSG_SET_SELECTION(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
    ...
    public void On_CMSG_SETSHEATHED(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        if(checked(packet.Data.Length - 1) >= 9)
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_SETSHEATHED [{2}]", client.IP, client.Port, sheathed);
            SetSheath(ref client.Character, sheathed);
        }
    }

    public void SendAttackerStateUpdate(
        ref WS_Base.BaseObject Attacker,
        ref WS_Base.BaseObject Victim,
        ...
        WS_Network.ClientClass client = null)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_ATTACKERSTATEUPDATE);
    ...
    public void SendAttackStart(ulong attackerGUID, ulong victimGUID, WS_Network.ClientClass client)
    {
        ...
        Packets.Packets.PacketClass SMSG_ATTACKSTART = new(Opcodes.SMSG_ATTACKSTART);
    ...
    public void SendAttackStop(ulong attackerGUID, ulong victimGUID, ref WS_Network.ClientClass client)
    {
        ...
        Packets.Packets.PacketClass SMSG_ATTACKSTOP = new(Opcodes.SMSG_ATTACKSTOP);
        ...
        if(objCharacter is null)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    ...
                    objCharacter.client.Account,
                    objCharacter.client.Character.UnitName,
                    objCharacter.client.Character.GUID);
            return;
        }
        switch(State)
        {
            ...
            case SHEATHE_SLOT.SHEATHE_WEAPON:
            {
                ...
                if(objCharacter.Items.ContainsKey(16) && !objCharacter.Items[16].IsBroken())
                {
                    ...
                    items[16] = Item;
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "spellCanDualWeild = {0}", objCharacter.spellCanDualWield);
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "objectClass = {0}", objCharacter.Items[16].ItemInfo.ObjectClass);
                    if(objCharacter.spellCanDualWield &&
                        (objCharacter.Items[16].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_WEAPON))
                    {
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.WARNING, "Unhandled sheathe state [{0}]", State);
    ...
    public void SetVirtualItemInfo(WS_PlayerData.CharacterObject objChar, byte Slot, ref ItemObject Item)
    {
        if(objChar is null)
        {
            throw new ArgumentNullException(nameof(objChar));
        }
    ...
    public class TAttackTimer : IDisposable
    {
        ...
        public TAttackTimer(ref WS_PlayerData.CharacterObject Character_)
        {
            ...
            Character = Character_ ?? throw new ArgumentNullException(nameof(Character_));
            Victim = null;
        }
        ...
        public TAttackTimer(ref WS_Base.BaseObject Victim_, ref WS_PlayerData.CharacterObject Character_)
        {
            if(Victim_ is null)
            {
                throw new ArgumentNullException(nameof(Victim_));
            }
        ...
        public void AttackStart(WS_Base.BaseUnit Victim_ = null)
        {
            ...
            checked
            {
                if((WorldServiceLocator.NativeMethods.timeGetTime(string.Empty) - LastAttack) >= AttackSpeed)
                {
                    DoAttack(null);
                } else
                {
                    NextAttackTimer.Change(
                        WorldServiceLocator.NativeMethods.timeGetTime(string.Empty) - LastAttack,
                        -1);
                }
            }
        }
        ...
        public void DoAttack(object Status)
        {
            ...
            LastAttack = WorldServiceLocator.NativeMethods.timeGetTime(string.Empty);
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.CRITICAL, "Error doing attack.{0}{1}", Environment.NewLine, ex.ToString());
            }
        }
        ...
        public void DoMeleeAttack(object Status)
        {
            ...
            checked
            {
                ...
                {
                    bool flag;
                    if((Character.spellCasted[1]?.Finished) == false)
                    {
                    ...
                    if(Victim.IsDead)
                    {
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_DEADTARGET = new(
                            Opcodes.SMSG_ATTACKSWING_DEADTARGET);
                    ...
                    if(Character.IsDead)
                    {
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_DEADTARGET2 = new(
                            Opcodes.SMSG_ATTACKSWING_DEADTARGET);
                    ...
                    {
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_NOTSTANDING = new(
                            Opcodes.SMSG_ATTACKSWING_NOTSTANDING);
                    ...
                    if(tmpDist > (8f + Victim.CombatReach))
                    {
                        ...
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_NOTINRANGE2 = new(
                            Opcodes.SMSG_ATTACKSWING_NOTINRANGE);
                    ...
                    {
                        NextAttackTimer.Change(2000, -1);
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_NOTINRANGE = new(
                            Opcodes.SMSG_ATTACKSWING_NOTINRANGE);
                    ...
                    {
                        NextAttackTimer.Change(2000, -1);
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_BADFACING = new(
                            Opcodes.SMSG_ATTACKSWING_BADFACING);
                        Character.client.Send(ref SMSG_ATTACKSWING_BADFACING);
                        SMSG_ATTACKSWING_BADFACING.Dispose();
                        return;
                    }
                    var HaveMainHand = (Character.GetAttackTime(WeaponAttackType.BASE_ATTACK) > 0) &&
                        Character.Items.ContainsKey(15);
                    var HaveOffHand = (Character.GetAttackTime(WeaponAttackType.OFF_ATTACK) > 0) &&
                        Character.Items.ContainsKey(16);
                    ...
                    if(HaveMainHand && HaveOffHand)
                    {
                        if(combatDualWield)
                        {
                            ...
                            {
                                TimeLeftMainHand = (int)Math.Round(
                                    Character.GetAttackTime(WeaponAttackType.OFF_ATTACK) / 2.0);
                            }
                        ...
                        } else
                        {
                            if(TimeLeftMainHand == (-1))
                            {
                                TimeLeftOffHand = (int)Math.Round(
                                    Character.GetAttackTime(WeaponAttackType.BASE_ATTACK) / 2.0);
                            }
                ...
                } catch(Exception ex)
                {
                    ...
                    {
                        Packets.Packets.PacketClass SMSG_ATTACKSWING_CANT_ATTACK = new(
                            Opcodes.SMSG_ATTACKSWING_CANT_ATTACK);
                    ...
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.DEBUG,
                            "Error while doing melee attack.{0}",
        ...
        public void DoMeleeDamage()
        {
            ...
            checked
            {
                ...
                while(i <= ((uint)b))
                {
                    if((Victim.ActiveSpells[i] != null) &&
                        ((unchecked((uint)Victim.ActiveSpells[i].GetSpellInfo.procFlags) & (true ? 1u : 0u)) != 0))
                    {
                        byte j = 0;
                        do
                        {
                            if(((Victim.ActiveSpells[i].Aura_Info[j]?.ApplyAuraIndex) == 42) &&
                                WorldServiceLocator.Functions
                                    .RollChance(Victim.ActiveSpells[i].GetSpellInfo.procChance))
                            {
                                ...
                                WS_Spells.CastSpellParameters castSpellParameters = new(
                                    ref Target,
                                    ref baseObject,
                                    Victim.ActiveSpells[i].Aura_Info[j].TriggerSpell,
                                    Instant: true);
                                reference2 = (WS_Base.BaseUnit)baseObject;
                ...
                if((Character.Class == Classes.CLASS_WARRIOR) ||
                    ((Character.Class == Classes.CLASS_DRUID) &&
                        ((Character.ShapeshiftForm == ShapeshiftForm.FORM_BEAR) ||
                            (Character.ShapeshiftForm == ShapeshiftForm.FORM_DIREBEAR))))
                {
                    Character.Rage
                        .Increment(
                            (int)(((7.5 * damageInfo.Damage / Character.GetRageConversion) +
                                    (Character.GetHitFactor(
                                            (damageInfo.HitInfo & 4) == 0,
                                            (damageInfo.HitInfo & 0x200) != 0) *
                                        WorldServiceLocator.WSCombat
                                            .GetAttackTime(ref Character, ref combatDualWield))) /
                                2.0));
        ...
        public void DoMeleeDamageBySpell(
            ref WS_PlayerData.CharacterObject Character,
            ref WS_Base.BaseObject Victim2,
            int BonusDamage,
            int SpellID)
        {
            ...
            if(Victim2 is null)
            {
                throw new ArgumentNullException(nameof(Victim2));
            }

            var wS_Combat = WorldServiceLocator.WSCombat;
            WS_Base.BaseUnit Attacker = Character;
            var baseUnit = (WS_Base.BaseUnit)Victim2;
            var damageInfo2 = wS_Combat.CalculateDamage(
                ref Attacker,
                ref baseUnit,
                DualWield: false,
                Ranged: false,
                WorldServiceLocator.WSSpells.SPELLs[SpellID]);
            Victim2 = baseUnit;
            ...
            checked
            {
                ...
                wS_Spells.SendNonMeleeDamageLog(
                    ref baseUnit,
                    ref Attacker,
                    SpellID,
                ...
                } else if(Victim2 is WS_PlayerData.CharacterObject obj2)
                {
                    ...
                    Character = (WS_PlayerData.CharacterObject)Attacker;
                    if(((WS_PlayerData.CharacterObject)Victim2).Class == Classes.CLASS_WARRIOR)
                    {
                        ((WS_PlayerData.CharacterObject)Victim2).Rage
                            .Increment(
                                (int)((damageInfo.Damage /
                                            ((double)(((WS_PlayerData.CharacterObject)Victim2).Level * 4)) *
                                        25.0) +
                                    10.0));
                        ((WS_PlayerData.CharacterObject)Victim2).SetUpdateFlag(
                            24,
                            ((WS_PlayerData.CharacterObject)Victim2).Rage.Current);
                        Character.SendCharacterUpdate();
                    }
                }
                if((Character.Class == Classes.CLASS_WARRIOR) ||
                ...
                {
                    Character.Rage
                        .Increment((int)((damageInfo.Damage / ((double)(Character.Level * 4)) * 75.0) + 10.0));
                    Character.SetUpdateFlag(24, Character.Rage.Current);
                    Character.SendCharacterUpdate();
                }
            }
        ...
        public void DoRangedAttack(object Status)
        {
            ...
            {
                Packets.Packets.PacketClass SMSG_ATTACKSWING_DEADTARGET2 = new(Opcodes.SMSG_ATTACKSWING_DEADTARGET);
                Character.client.Send(ref SMSG_ATTACKSWING_DEADTARGET2);
                SMSG_ATTACKSWING_DEADTARGET2.Dispose();
                Character.AutoShotSpell = 0;
                AttackStop();
            ...
            if(Character.DEAD)
            {
                Packets.Packets.PacketClass SMSG_ATTACKSWING_DEADTARGET = new(Opcodes.SMSG_ATTACKSWING_DEADTARGET);
                Character.client.Send(ref SMSG_ATTACKSWING_DEADTARGET);
                SMSG_ATTACKSWING_DEADTARGET.Dispose();
                Character.AutoShotSpell = 0;
                AttackStop();
            ...
            if(Character.StandState > 0)
            {
                Packets.Packets.PacketClass SMSG_ATTACKSWING_NOTSTANDING = new(Opcodes.SMSG_ATTACKSWING_NOTSTANDING);
                Character.client.Send(ref SMSG_ATTACKSWING_NOTSTANDING);
                SMSG_ATTACKSWING_NOTSTANDING.Dispose();
                Character.AutoShotSpell = 0;
                AttackStop();
            ...
            {
                NextAttackTimer.Change(2000, -1);
                Packets.Packets.PacketClass SMSG_ATTACKSWING_NOTINRANGE = new(Opcodes.SMSG_ATTACKSWING_NOTINRANGE);
                Character.client.Send(ref SMSG_ATTACKSWING_NOTINRANGE);
                SMSG_ATTACKSWING_NOTINRANGE.Dispose();
                return;
            }
            ...
            {
                NextAttackTimer.Change(2000, -1);
                Packets.Packets.PacketClass SMSG_ATTACKSWING_BADFACING = new(Opcodes.SMSG_ATTACKSWING_BADFACING);
                Character.client.Send(ref SMSG_ATTACKSWING_BADFACING);
                SMSG_ATTACKSWING_BADFACING.Dispose();
            } else
            {
