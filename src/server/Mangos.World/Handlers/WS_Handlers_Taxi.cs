...
public class WS_Handlers_Taxi
{
    private void SendActivateTaxiReply(ref WS_Network.ClientClass client, ActivateTaxiReplies reply)
    {
        ...
        Packets.Packets.PacketClass taxiFailed = new(Opcodes.SMSG_ACTIVATETAXIREPLY);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "SMSG_ACTIVATETAXIREPLY Exception  {0} : {1} : {2} : {3} : {4}",
                    client.IP,
                    client.Port,
                    reply,
                    taxiFailed,
    ...
    private void SendTaxiStatus(ref WS_PlayerData.CharacterObject objCharacter, ulong cGuid)
    {
        ...
        var currentTaxi = WorldServiceLocator.WSDBCDatabase
            .GetNearestTaxi(
                ...
                checked((int)WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGuid].MapID));
        Packets.Packets.PacketClass SMSG_TAXINODE_STATUS = new(Opcodes.SMSG_TAXINODE_STATUS);
        try
        {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.WARNING,
                    "SMSG_TAXINODE_STATUS Exception  {0} : {1} : {2} : {3} : {4}",
                    objCharacter,
                    cGuid,
                    currentTaxi,
        ...
        checked
        {
            try
            {
                ...
                foreach(var path in waypointPaths)
                {
                    ...
                    {
                        var price = (int)Math.Round(
                            WorldServiceLocator.WSDBCDatabase.TaxiPaths[path].Price * discountMod);
                    ...
                    foreach(var taxiPathNode in WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[path])
                    {
                        ...
                        totalDistance += WorldServiceLocator.WSCombat
                            .GetDistance(
                                lastX,
                                taxiPathNode.Value.x,
                                lastY,
                                taxiPathNode.Value.y,
                                lastZ,
                                taxiPathNode.Value.z);
                    ...
                    try
                    {
                        ...
                        SMSG_MONSTER_MOVE.AddInt32(
                            (int)(totalDistance / WorldServiceLocator.GlobalConstants.UNIT_NORMAL_TAXI_SPEED *
                                1000f));
                    ...
                    } catch(Exception ex)
                    {
                        WorldServiceLocator.WorldServer.Log
                            .WriteLine(
                                LogType.WARNING,
                                "SMSG_MONSTER_MOVE Exception  {0} : {1} : {2} : {3} : {4}",
                                character.GUID,
                                character.positionX,
                                character.positionY,
                                character.positionZ,
                    ...
                    for(var i = 0; i <= (waypointNodes.Count - 1); i++)
                    {
                        ...
                        Packets.Packets.PacketClass WP_SMSG_MONSTER_MOVE = new(Opcodes.SMSG_MONSTER_MOVE);
                        try
                        {
                        ...
                        var moveDistance = WorldServiceLocator.WSCombat
                            .GetDistance(
                                lastX,
                                waypointNodes[i].x,
                                lastY,
                                ...
                                waypointNodes[i].z);
                        Thread.Sleep(
                            (int)(moveDistance / WorldServiceLocator.GlobalConstants.UNIT_NORMAL_TAXI_SPEED *
                                1000f));
    ...
    public void On_CMSG_ACTIVATETAXI(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
        ...
        WorldServiceLocator.WorldServer.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_ACTIVATETAXI [taxiGUID={2:X} srcNode={3} dstNode={4}]",
                client.IP,
                client.Port,
                guid,
                srcNode,
        ...
        if(!WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(guid) ||
            ((WorldServiceLocator.WorldServer.WORLD_CREATUREs[guid].CreatureInfo.cNpcFlags & 8) == 0))
        {
        ...
        if((((int)client.Character.ShapeshiftForm) > 0) &&
            (client.Character.ShapeshiftForm != ShapeshiftForm.FORM_BERSERKERSTANCE) &&
            (client.Character.ShapeshiftForm != ShapeshiftForm.FORM_BATTLESTANCE) &&
            (client.Character.ShapeshiftForm != ShapeshiftForm.FORM_DEFENSIVESTANCE) &&
            (client.Character.ShapeshiftForm != ShapeshiftForm.FORM_SHADOW))
        {
        ...
        if(!WorldServiceLocator.WSDBCDatabase.TaxiNodes.ContainsKey(srcNode) ||
            !WorldServiceLocator.WSDBCDatabase.TaxiNodes.ContainsKey(dstNode))
        {
        ...
        if(client.Character.IsHorde)
        {
            if(!WorldServiceLocator.WorldServer.CREATURESDatabase
                .ContainsKey(WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].HordeMount))
            {
                mount = new CreatureInfo(WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].HordeMount).GetFirstModel;
            } else
            {
                mount = WorldServiceLocator.WorldServer.CREATURESDatabase[
                    WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].HordeMount].ModelA1;
            }
        } else if(!WorldServiceLocator.WorldServer.CREATURESDatabase
            .ContainsKey(WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].AllianceMount))
        {
            mount = new CreatureInfo(WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].AllianceMount).GetFirstModel;
        } else
        {
            mount = WorldServiceLocator.WorldServer.CREATURESDatabase[
                WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].AllianceMount].ModelA2;
        }
        if(mount == 0)
        {
        ...
        checked
        {
            ...
            var discountMod = client.Character
                .GetDiscountMod(WorldServiceLocator.WorldServer.WORLD_CREATUREs[guid].Faction);
            foreach(var taxiPath in WorldServiceLocator.WSDBCDatabase.TaxiPaths)
            {
                if((taxiPath.Value.TFrom == srcNode) && (taxiPath.Value.TTo == dstNode))
                {
    ...
    public void On_CMSG_ACTIVATETAXI_FAR(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        checked
        {
            ...
            try
            {
                ...
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.DEBUG,
                        "[{0}:{1}] CMSG_ACTIVATETAXI_FAR [taxiGUID={2:X} TotalCost={3} NodeCount={4}]",
                        client.IP,
                        client.Port,
                        guid,
                        totalCost,
                ...
                if(client.Character.IsHorde)
                {
                    ...
                    {
                        mount = WorldServiceLocator.WorldServer.CREATURESDatabase[
                            WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].HordeMount].GetFirstModel;
                    }
                } else if(!WorldServiceLocator.WorldServer.CREATURESDatabase
                    .ContainsKey(WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].AllianceMount))
                {
                ...
                {
                    mount = WorldServiceLocator.WorldServer.CREATURESDatabase[
                        WorldServiceLocator.WSDBCDatabase.TaxiNodes[srcNode].AllianceMount].GetFirstModel;
                }
                if(mount == 0)
                {
                    SendActivateTaxiReply(ref client, ActivateTaxiReplies.ERR_TAXIUNSPECIFIEDSERVERERROR);
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.CRITICAL, "Error when taking a long taxi.{0}", $"{Environment.NewLine}{e}");
            }
        }
    }

    public void On_CMSG_MOVE_SPLINE_DONE(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        WorldServiceLocator.WorldServer.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_MOVE_SPLINE_DONE", client.IP, client.Port);
    }

    public void On_CMSG_TAXINODE_STATUS_QUERY(
        ...
        ref WS_Network.ClientClass client)
    {
        ...
        if(checked(packet.Data.Length - 1) >= 13)
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.DEBUG,
                    "[{0}:{1}] CMSG_TAXINODE_STATUS_QUERY [taxiGUID={2:X}]",
                    client.IP,
                    client.Port,
                    guid);
            if(WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(guid))
    ...
    public void On_CMSG_TAXIQUERYAVAILABLENODES(
        ref Packets.Packets.PacketClass packet,
        ref WS_Network.ClientClass client)
    {
        if(packet is null)
        ...
        if(checked(packet.Data.Length - 1) >= 13)
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.DEBUG,
                    "[{0}:{1}] CMSG_TAXIQUERYAVAILABLENODES [taxiGUID={2:X}]",
                    client.IP,
                    client.Port,
                    guid);
            if(WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(guid) &&
                ((((uint)WorldServiceLocator.WorldServer.WORLD_CREATUREs[guid].CreatureInfo.cNpcFlags) & 8u) != 0))
            {
    ...
    public void SendTaxiMenu(ref WS_PlayerData.CharacterObject objCharacter, ulong cGuid)
    {
        ...
        if(!objCharacter.TaxiZones[currentTaxi])
        {
            ...
            Packets.Packets.PacketClass SMSG_NEW_TAXI_PATH = new(Opcodes.SMSG_NEW_TAXI_PATH);
            try
            {
            ...
            } catch(Exception ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.WARNING,
                        "SMSG_NEW_TAXI_PATH Exception  {0} : {1} : {2} : {3} : {4}",
                        objCharacter,
                        cGuid,
                        currentTaxi,
                        SMSG_NEW_TAXI_PATH,
        ...
        {
            Packets.Packets.PacketClass SMSG_SHOWTAXINODES = new(Opcodes.SMSG_SHOWTAXINODES);
            try
            {
            ...
            } catch(Exception ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.WARNING,
                        "SMSG_SHOWTAXINODES Exception  {0} : {1} : {2} : {3} : {4}",
                        objCharacter,
                        cGuid,
                        currentTaxi,
                        SMSG_SHOWTAXINODES,
