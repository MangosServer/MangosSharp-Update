...
public class WS_Base
{
    public class BaseObject
    {
        ...
        public virtual bool CanSee(ref BaseObject objCharacter)
        {
            ...
            if((objCharacter.Invisibility == InvisibilityLevel.STEALTH) &&
                (Math.Sqrt(
                        Math.Pow(objCharacter.positionX - positionX, 2.0) +
                            Math.Pow(objCharacter.positionY - positionY, 2.0)) <
                    WorldServiceLocator.GlobalConstants.DEFAULT_DISTANCE_DETECTION))
            {
                return true;
            }
            ...
            if((objCharacter.Invisibility == InvisibilityLevel.STEALTH) &&
                (objCharacter.Invisibility_Value > CanSeeInvisibility_Stealth))
            {
                return false;
            }
            return Math.Sqrt(
        ...
        {
            Packets.Packets.PacketClass packet = new(Opcodes.SMSG_PLAY_OBJECT_SOUND);
            try
            {
        ...
        public void SendToNearPlayers(ref Packets.Packets.PacketClass packet, ulong NotTo = 0uL, bool ToSelf = true)
        {
            if(packet is null)
            {
            ...
            foreach(var objCharacter in SeenBy.ToArray())
            {
                if((objCharacter != NotTo) &&
                    WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(objCharacter) &&
                    (WorldServiceLocator.WorldServer.CHARACTERs[objCharacter].client != null))
                {
    ...
    public class BaseUnit : BaseObject
    {
            ...
            checked
            {
                ...
                try
                {
                    if(!WorldServiceLocator.WSSpells.SPELLs[SpellID].IsPassive)
                    {
                        ...
                        {
                            if((ActiveSpells[slot2] != null) &&
                                (ActiveSpells[slot2].GetSpellInfo.Target == SpellInfo.Target) &&
                                (ActiveSpells[slot2].GetSpellInfo.Category == SpellInfo.Category) &&
                                (ActiveSpells[slot2].GetSpellInfo.SpellIconID == SpellInfo.SpellIconID) &&
                                (ActiveSpells[slot2].GetSpellInfo.SpellVisual == SpellInfo.SpellVisual) &&
                                (ActiveSpells[slot2].GetSpellInfo.Attributes == SpellInfo.Attributes) &&
                                (ActiveSpells[slot2].GetSpellInfo.AttributesEx == SpellInfo.AttributesEx) &&
                                (ActiveSpells[slot2].GetSpellInfo.AttributesEx2 == SpellInfo.AttributesEx2))
                            {
                ...
                } catch(Exception ex)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.CRITICAL, "ERROR ADDING AURA!{0}{1}", Environment.NewLine, ex.ToString());
                }
                var num2 = AuraStart;
                var num3 = AuraEnd;
                ...
                switch(this)
                {
                    case WS_PlayerData.CharacterObject _:
                        ((WS_PlayerData.CharacterObject)this).GroupUpdateFlag |= 0x200u;
                        break;
        ...
        public void DealSpellDamage(
            ref BaseUnit Caster,
            ref WS_Spells.SpellEffect EffectInfo,
            int SpellID,
            int Damage,
            ...
            SpellType SpellType)
        {
            ...
            checked
            {
                if(Caster is WS_PlayerData.CharacterObject @object)
                {
                    ...
                    var SpellDamage = (!IsHeal)
                        ? @object.spellDamage[(uint)DamageType].Value
                        : @object.healing.Value;
                    if(IsDot)
                    {
                        var TickAmount = (int)Math.Round(
                            WorldServiceLocator.WSSpells.SPELLs[SpellID].GetDuration /
                                ((double)EffectInfo.Amplitude));
                        if(TickAmount < 5)
                        {
                            TickAmount = 5;
                        }
                    ...
                    } else
                    {
                        ...
                        SpellDamageBenefit = (int)(SpellDamage *
                                (CastTime / 1000f) *
                                ((100 - PenaltyFactor) / 100.0) /
                            3.5);
                    }
                ...
                if(!IsDot &&
                    ...
                    WorldServiceLocator.Functions.RollChance(object1.GetCriticalWithSpells))
                {
            ...
            switch(SpellType)
            {
                case SpellType.SPELL_TYPE_NONMELEE:
                {
                    ...
                    var Target = this;
                    wS_Spells4.SendNonMeleeDamageLog(
                        ref Caster,
                        ref Target,
                        SpellID,
                        (int)DamageType,
                ...
                case SpellType.SPELL_TYPE_DOT:
                {
                    ...
                    var Target = this;
                    wS_Spells3.SendPeriodicAuraLog(
                        ref Caster,
                        ref Target,
                        SpellID,
                        (int)DamageType,
        ...
        public int GetAbsorb(DamageTypes School, int Damage)
        {
            ...
            checked
            {
                foreach(var tmpSpell in AbsorbSpellLeft)
                {
                    ...
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "Spell: {0} [{1}]", AbsorbDamage, Schools);
                ...
                for(var i = 0; i <= num; i++)
                {
                    ...
                    do
                    {
                        if((ActiveSpells[i].Aura_Info[j] != null) &&
                            (ActiveSpells[i].Aura_Info[j].ApplyAuraIndex == ((int)AuraIndex)))
                        {
            ...
            checked
            {
                ...
                for(var i = 0; i <= num; i++)
                {
                    ...
                    do
                    {
                        if((ActiveSpells[i].Aura_Info[j] != null) &&
                            (ActiveSpells[i].Aura_Info[j].ApplyAuraIndex == ((int)AuraIndex)) &&
                            ((ActiveSpells[i].Aura_Info[j].MiscValue & Mask) == Mask))
                        {
                            Modifier += ActiveSpells[i].Aura_Info[j].GetValue(Level, 0);
                        }
                        j = (byte)unchecked((uint)(j + 1));
        ...
        public float GetDamageReduction(ref BaseUnit t, DamageTypes School, int Damage)
        {
            ...
            checked
            {
                ...
                {
                    DamageReduction = (float)(Resistances[0].Base /
                        ((double)(Resistances[0].Base + 400 + (85 * Level))));
                } else
                {
                    var effectiveResistanceRating = t.Resistances[(uint)School].Base +
                        Math.Max((t.Level - Level) * 5, 0);
        ...
        public int GetDefenceSkill(ref BaseUnit Attacker)
        {
            ...
            if(this is WS_PlayerData.CharacterObject characterObject)
            {
                return (!Attacker.IsPlayer)
                    ? characterObject.Skills[95].CurrentWithBonus
                    : characterObject.Skills[95].MaximumWithBonus;
            }
            checked
            {
                return Level * 5;
        ...
        public SpellMissInfo GetMagicSpellHitResult(ref BaseUnit Caster, WS_Spells.SpellInfo Spell, int rand)
        {
            if(Caster is null)
            {
                throw new ArgumentNullException(nameof(Caster));
            ...
            checked
            {
                ...
                modHitChance += Caster.GetAuraModifierByMiscMask(
                    AuraEffects_Names.SPELL_AURA_MOD_INCREASES_SPELL_PCT_TO_HIT,
                    (int)Spell.SchoolMask);
                modHitChance += GetAuraModifierByMiscMask(
                    AuraEffects_Names.SPELL_AURA_MOD_ATTACKER_SPELL_HIT_CHANCE,
                    (int)Spell.SchoolMask);
                if(Spell.IsAOE)
                {
                ...
                if(Spell.Mechanic > 0)
                {
                    resist_mech = GetAuraModifierByMiscMask(
                        AuraEffects_Names.SPELL_AURA_MOD_MECHANIC_RESISTANCE,
                        Spell.Mechanic);
                }
                var i = 0;
                do
                {
                    if((Spell.SpellEffects[i]?.Mechanic) > 0)
                    {
                        var temp = GetAuraModifierByMiscMask(
                            AuraEffects_Names.SPELL_AURA_MOD_MECHANIC_RESISTANCE,
                            Spell.SpellEffects[i].Mechanic);
                        if(resist_mech < temp)
                        {
                            resist_mech = temp;
                        }
                ...
                modHitChance -= GetAuraModifierByMiscMask(
                    AuraEffects_Names.SPELL_AURA_MOD_DEBUFF_RESISTANCE,
                    Spell.DispellType);
                ...
                if(WorldServiceLocator.WorldServer.Rnd == null)
                {
                    return SpellMissInfo.SPELL_MISS_NONE;
                }
        ...
        public SpellMissInfo GetMeleeSpellHitResult(ref BaseUnit Caster, WS_Spells.SpellInfo Spell, int roll)
        {
            if(Caster is null)
            {
                throw new ArgumentNullException(nameof(Caster));
            ...
            return ((((uint)Spell.Attributes) & 0x200000u) != 0)
                ? SpellMissInfo.SPELL_MISS_NONE
                : SpellMissInfo.SPELL_MISS_NONE;
        }
        ...
        public float GetResist(ref BaseUnit t, DamageTypes School, int Damage)
        {
            ...
            var partialChances = (damageReduction < 0.15f)
                ? (new int[4] { 33, 11, 2, 0 })
                : ((damageReduction < 0.3f)
                    ? (new int[4] { 49, 24, 6, 1 })
                    : ((damageReduction < 0.45f)
                        ? (new int[4] { 26, 48, 18, 1 })
                        : ((!(damageReduction < 0.6f))
                            ? (new int[4] { 3, 16, 55, 25 })
                            : (new int[4] { 14, 40, 34, 11 }))));
            ...
            checked
            {
                if(this is WS_PlayerData.CharacterObject characterObject)
                {
                    ...
                    return (!Victim.IsPlayer)
                        ? characterObject.Skills[skill].CurrentWithBonus
                        : characterObject.Skills[skill].MaximumWithBonus;
                }
                return Level * 5;
            }
        }
                    ...
                    checked
                    {
                        if(ActiveSpells[Slot].Aura[i] != null)
                        {
                            ...
                            obj(
                                ref Target,
                                ref Caster2,
                                ref ActiveSpells[Slot].Aura_Info[i],
                ...
                for(var i = 0; i <= num; i++)
                {
                    if((ActiveSpells[i] != null) && (ActiveSpells[i].SpellID == SpellID))
                    {
                        ...
                        if((this is WS_PlayerData.CharacterObject @object) &&
                            (decimal.Compare(new decimal(@object.DuelArbiter), 0m) != 0) &&
                            (@object.DuelPartner == null))
                        {
                            WorldServiceLocator.WorldServer.WORLD_CREATUREs[@object.DuelArbiter].RemoveAuraBySpell(
                                SpellID);
        ...
        public void RemoveAurasByDispellType(int DispellType, int Amount)
        {
            checked
            {
                ...
                {
                    if((ActiveSpells[i] != null) &&
                        (WorldServiceLocator.WSSpells.SPELLs[ActiveSpells[i].SpellID].DispellType == DispellType))
                    {
                        RemoveAura(i, ref ActiveSpells[i].SpellCaster);
                        Amount--;
                        if(Amount <= 0)
        ...
        public void RemoveAurasByInterruptFlag(int AuraInterruptFlag)
        {
            checked
            {
                ...
                {
                    if((ActiveSpells[i] != null) &&
                        WorldServiceLocator.WSSpells.SPELLs.ContainsKey(ActiveSpells[i].SpellID) &&
                        ((WorldServiceLocator.WSSpells.SPELLs[ActiveSpells[i].SpellID].auraInterruptFlags &
                                AuraInterruptFlag) !=
                            0) &&
                        ((WorldServiceLocator.WSSpells.SPELLs[ActiveSpells[i].SpellID].procFlags & 0x8000000) == 0))
                    {
                        RemoveAura(i, ref ActiveSpells[i].SpellCaster);
                    }
                }
                if(this is WS_PlayerData.CharacterObject characterObject)
                {
                    if(((characterObject.spellCasted[3]?.Finished) == false) &&
                        ((characterObject.spellCasted[3].SpellInfo.channelInterruptFlags & AuraInterruptFlag) != 0))
                    {
                ...
                {
                    if((creatureObject.SpellCasted != null) &&
                        ((creatureObject.SpellCasted.SpellInfo.channelInterruptFlags & AuraInterruptFlag) != 0))
                    {
        ...
        public void RemoveAurasByMechanic(int Mechanic)
        {
            checked
            {
                ...
                {
                    if((ActiveSpells[i] != null) &&
                        (WorldServiceLocator.WSSpells.SPELLs[ActiveSpells[i].SpellID].Mechanic == Mechanic))
                    {
                        RemoveAura(i, ref ActiveSpells[i].SpellCaster);
                    }
                }
        ...
        public void SetAura(int SpellID, int Slot, int Duration, bool SendUpdate = true)
        {
            if((ActiveSpells[Slot] == null) ||
                ((SpellID != 0) &&
                    WorldServiceLocator.WSSpells.SPELLs.ContainsKey(SpellID) &&
                    WorldServiceLocator.WSSpells.SPELLs[SpellID].IsPassive))
            {
                return;
            }
            ...
            checked
            {
                ...
                if(this is not WS_PlayerData.CharacterObject @object)
                {
                    ...
                    try
                    {
                        ...
                        Packets.Packets.PacketClass packet = tmpPacket;
                        ...
                        tmpPacket = (Packets.Packets.UpdatePacketClass)packet;
                        packet = tmpPacket;
                        SendToNearPlayers(ref packet);
                        tmpPacket = (Packets.Packets.UpdatePacketClass)packet;
                    } finally
                ...
                } else
                {
                    ...
                    Packets.Packets.PacketClass SMSG_UPDATE_AURA_DURATION = new(Opcodes.SMSG_UPDATE_AURA_DURATION);
                    try
                    {
        ...
        {
            if((ActiveSpells[Slot] == null) ||
                (Slot >= WorldServiceLocator.GlobalConstants.MAX_AURA_EFFECTs_VISIBLE))
            {
                return;
            }
            var AuraFlag_Slot = Slot / 4;
        ...
        public virtual bool Exist
        {
            get
            {
                ...
                return (this is WS_Creatures.CreatureObject) &&
        ...
        public bool IsSwimming => (cUnitFlags & 0x200000) != 0;
