...
public class WS_GameObjects
{
    ...
    public GameObject GetClosestGameobject(ref WS_Base.BaseUnit unit, int GameObjectEntry = 0)
    {
        ...
        {
            foreach(var GUID2 in @object.gameObjectsNear.ToArray())
            {
                if(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs.ContainsKey(GUID2) &&
                    ((GameObjectEntry == 0) ||
                        (WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GUID2].ID == GameObjectEntry)))
                {
                    var tmpDistance = WorldServiceLocator.WSCombat
                        .GetDistance(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GUID2], unit);
            ...
            do
            {
                ...
                do
                {
                    if(((x + cellX) > (-1)) &&
                        ...
                        ((y + cellY) > (-1)) &&
                        ((y + cellY) < 64) &&
                        (WorldServiceLocator.WSMaps.Maps[unit.MapID].Tiles[x + cellX, y + cellY] != null))
                    {
                        foreach(var GUID in WorldServiceLocator.WSMaps.Maps[unit.MapID].Tiles[x + cellX, y + cellY].GameObjectsHere
                            .ToArray())
                        {
                            if(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs.ContainsKey(GUID) &&
                                ((GameObjectEntry == 0) ||
                                    (WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GUID].ID == GameObjectEntry)))
                            {
                                var tmpDistance = WorldServiceLocator.WSCombat
                                    .GetDistance(WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GUID], unit);
                                if(tmpDistance < minDistance)
                                {
                                    minDistance = tmpDistance;
    ...
    public void On_CMSG_GAMEOBJ_USE(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
        ...
        checked
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.DEBUG,
                    "GameObjectType: {0}",
                    WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GameObjectGUID].Type);
            var type = GO.Type;
            switch(type)
            {
                case GameObjectType.GAMEOBJECT_TYPE_QUESTGIVER:
                    ...
                    {
                        var qm = WorldServiceLocator.WorldServer.ALLQUESTS
                            .GetQuestMenuGO(ref client.Character, GameObjectGUID);
                        WorldServiceLocator.WorldServer.ALLQUESTS
                            .SendQuestMenu(ref client.Character, GameObjectGUID, "Available quests", qm);
                    }
                    break;
                ...
                case GameObjectType.GAMEOBJECT_TYPE_CHAIR:
                    Packets.Packets.PacketClass StandState = new(Opcodes.CMSG_STANDSTATECHANGE);
                    try
                    {
                        StandState.AddInt8(
                            (byte)(4L +
                                WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GameObjectGUID].GetSound(1)));
                        client.Character
                            .Teleport(GO.positionX, GO.positionY, GO.positionZ, GO.orientation, (int)GO.MapID);
                ...
                case GameObjectType.GAMEOBJECT_TYPE_CAMERA:
                    Packets.Packets.PacketClass cinematicPacket = new(Opcodes.SMSG_TRIGGER_CINEMATIC);
                ...
                case GameObjectType.GAMEOBJECT_TYPE_RITUAL:
                    ...
                    if(((GO.Owner == client.Character.GUID) || client.Character.IsInGroup) &&
                        ((GO.Owner == client.Character.GUID) ||
                            (WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(GO.Owner) &&
                                WorldServiceLocator.WorldServer.CHARACTERs[GO.Owner].IsInGroup &&
                                (WorldServiceLocator.WorldServer.CHARACTERs[GO.Owner].Group ==
                                    client.Character.Group))))
                    {
                ...
                case GameObjectType.GAMEOBJECT_TYPE_SPELLCASTER:
                    ...
                    {
                        WorldServiceLocator.WorldServer.Log
                            .WriteLine(LogType.DEBUG, "Spellcaster requires same group.");
                        WorldServiceLocator.WorldServer.Log
                            .WriteLine(
                                LogType.DEBUG,
                                "Owner: {0:X}  You: {1:X}",
                                WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GameObjectGUID].Owner,
                ...
                case GameObjectType.GAMEOBJECT_TYPE_MEETINGSTONE:
                    if(client.Character.Level < GO.GetSound(0))
                    {
                        WorldServiceLocator.WSSpells
                            .SendCastResult(SpellFailedReason.SPELL_FAILED_LEVEL_REQUIREMENT, ref client, 23598);
                    } else if(client.Character.Level >
                        WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[GameObjectGUID].GetSound(1))
                    {
                        WorldServiceLocator.WSSpells
                            .SendCastResult(SpellFailedReason.SPELL_FAILED_LEVEL_REQUIREMENT, ref client, 23598);
                ...
                case GameObjectType.GAMEOBJECT_TYPE_FISHINGNODE:
                    ...
                    if(GO.Loot == null)
                    {
                        if(GO.State == GameObjectLootState.DOOR_CLOSED)
                        {
                            ...
                            Packets.Packets.PacketClass fishNotHookedPacket = new(Opcodes.SMSG_FISH_NOT_HOOKED);
                            client.Send(ref fishNotHookedPacket);
                            fishNotHookedPacket.Dispose();
                        }
                    } else
                    {
                        var AreaFlag = WorldServiceLocator.WSMaps
                            .GetAreaFlag(GO.positionX, GO.positionY, (int)GO.MapID);
                        var AreaID = WorldServiceLocator.WSMaps.AreaTable[AreaFlag].ID;
                        DataTable MySQLQuery = new();
                        WorldServiceLocator.WorldServer.WorldDatabase
                            .Query(
                                $"SELECT * FROM skill_fishing_base_level WHERE entry = {AreaID};",
                                ref MySQLQuery);
                        ...
                        } else
                        {
                            WorldServiceLocator.WorldServer.Log
                                .WriteLine(
                                    LogType.CRITICAL,
                                    "No fishing entry in 'skill_fishing_base_level' for area [{0}] in zone [{1}]",
                                    WorldServiceLocator.WSMaps.AreaTable[AreaFlag].ID,
                                    WorldServiceLocator.WSMaps.AreaTable[AreaFlag].Zone);
                        }
                        ...
                        {
                            GO.State = GameObjectLootState.DOOR_CLOSED;
                            Packets.Packets.PacketClass fishEscaped = new(Opcodes.SMSG_FISH_ESCAPED);
    ...
    public void On_CMSG_GAMEOBJECT_QUERY(ref Packets.Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        if(packet is null)
        {
            throw new ArgumentNullException(nameof(packet));
        ...
        checked
        {
            ...
            Packets.Packets.PacketClass response = new(Opcodes.SMSG_GAMEOBJECT_QUERY_RESPONSE);
            packet.GetInt16();
            ...
            try
            {
                if(!WorldServiceLocator.WorldServer.GAMEOBJECTSDatabase.ContainsKey(GameObjectID))
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.DEBUG,
                            "[{0}:{1}] CMSG_GAMEOBJECT_QUERY [GameObject {2} not loaded.]",
                            client.IP,
                            client.Port,
                            GameObjectID);
            ...
            } catch(Exception ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.FAILED,
                        "Unknown Error: Unable to find GameObjectID={0} in database. {1}",
                        GameObjectID,
                        ex);
            }
        }
    ...
    public class GameObjectInfo : IDisposable
    {
        ...
        public GameObjectInfo(int ID_)
        {
            ...
            DataTable MySQLQuery = new();
            WorldServiceLocator.WorldServer.WorldDatabase
                .Query($"SELECT * FROM gameobject_template WHERE entry = {ID_};", ref MySQLQuery);
            if(MySQLQuery.Rows.Count == 0)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.FAILED, "gameobject_template {0} not found in SQL database!", ID_);
                found_ = false;
                return;
            }
            found_ = true;
    ...
    public class GameObject : WS_Base.BaseObject, IDisposable
    {
        ...
        public GameObject(ulong cGUID, DataRow Info = null)
        {
            ...
            if(Info == null)
            {
                ...
                WorldServiceLocator.WorldServer.WorldDatabase
                    .Query(
                        $"SELECT * FROM gameobject LEFT OUTER JOIN game_event_gameobject ON gameobject.guid = game_event_gameobject.guid WHERE gameobject.guid = {cGUID};",
                        ref MySQLQuery);
                if(MySQLQuery.Rows.Count <= 0)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.FAILED, "GameObject Spawn not found in database. [cGUID={0:X}]", cGUID);
                    return;
                }
        ...
        public void AddToWorld()
        {
            ...
            } catch(Exception ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.FAILED,
                        "Could not add GameObject [{0}] to World with Exception of {1}",
                        GUID,
                        ex);
                return;
            }
        ...
        public void CalculateMineRemaning(bool Force = false)
        {
            if((Type != GameObjectType.GAMEOBJECT_TYPE_CHEST) ||
                !WorldServiceLocator.WSLoot.Locks.ContainsKey(LockID))
            {
                return;
            }
            var i = 0;
            checked
            {
                do
                {
                    if(WorldServiceLocator.WSLoot.Locks[LockID].KeyType[i] is 2 and (3 or 2))
                    {
                        ...
                        {
                            MineRemaining = WorldServiceLocator.WorldServer.Rnd
                                .Next((int)GetSound(4), (int)(GetSound(5) + 1L));
                        }
                        break;
                    }
                    i++;
        ...
        public void CloseDoor(object ClosedState, bool timedOut)
        {
            if(ClosedState is null)
            {
                throw new ArgumentNullException(nameof(ClosedState));
            }

            Flags &= 254;
            ClosedState = GameObjectLootState.DOOR_CLOSED;
            Packets.Packets.PacketClass packet = new(Opcodes.SMSG_UPDATE_OBJECT);
            packet.AddInt32(1);
            packet.AddInt8(0);
            Packets.Packets.UpdateClass tmpUpdate = new(
            ...
            tmpUpdate.SetUpdateFlag(14, 0, Conversions.ToByte(ClosedState));
            var updateObject = this;
        ...
        public void Destroy(object state)
        {
            ...
            if((CreatedBySpell > 0) &&
                (decimal.Compare(new decimal(Owner), 0m) > 0) &&
                WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(Owner) &&
                WorldServiceLocator.WorldServer.CHARACTERs[Owner].gameObjects.Contains(this))
            {
            ...
            if(ToDespawn)
            {
                ToDespawn = false;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "Gameobject {0:X} despawning.", GUID);
                Packets.Packets.PacketClass despawnPacket = new(Opcodes.SMSG_GAMEOBJECT_DESPAWN_ANIM);
        ...
        public virtual void FillAllUpdateFlags(
            ref Packets.Packets.UpdateClass Update,
        ...
        public uint GetSound(int Index) { return ObjectInfo.Fields[Index]; }
        ...
        public void LootObject(ref WS_PlayerData.CharacterObject Character, LootType LootingType)
        {
            ...
            if(Loot == null)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.WARNING,
                        "Null Loot ID [{0}] Loot [{1}] LootingType [{2}]",
                        LootID,
                        Loot,
                        LootingType);
                return;
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.WARNING, "Null Loot SpellCasted [{0}]", Character.spellCasted);
                return;
            }
        ...
        public void OpenDoor()
        {
            ...
            if(AutoCloseTime > 0)
            {
                ThreadPool.RegisterWaitForSingleObject(
                    waitObject: new AutoResetEvent(initialState: false),
                    callBack: CloseDoor,
                    state: null,
                    millisecondsTimeOutInterval: AutoCloseTime,
        ...
        public void SetFishEscaped(object state, bool timedOut)
        {
            ...
            if(!Operators.ConditionalCompareObjectNotEqual(state, GameObjectLootState.DOOR_OPEN, TextCompare: false))
            {
                ...
                if((decimal.Compare(new decimal(Owner), 0m) > 0) &&
                    WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(Owner) &&
                    WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(Owner))
                {
                    Packets.Packets.PacketClass fishEscaped = new(Opcodes.SMSG_FISH_ESCAPED);
                    WorldServiceLocator.WorldServer.CHARACTERs[Owner].client.Send(ref fishEscaped);
                    fishEscaped.Dispose();
                    WorldServiceLocator.WorldServer.CHARACTERs[Owner].FinishSpell(
                        CurrentSpellTypes.CURRENT_CHANNELED_SPELL,
                        OK: true);
                }
            }
        }
        ...
        public void SetFishHooked(object state, bool timedOut)
        {
                ...
                TextCompare: false))
            {
                ...
                SendToNearPlayers(ref packet);
                packet.Dispose();
                Packets.Packets.PacketClass packetAnim = new(Opcodes.SMSG_GAMEOBJECT_CUSTOM_ANIM);
                packetAnim.AddUInt64(GUID);
                packetAnim.AddInt32(0);
                SendToNearPlayers(ref packetAnim);
                packetAnim.Dispose();
                const int FishEscapeTime = 2000;
                ThreadPool.RegisterWaitForSingleObject(
