...
public sealed class ItemObject : IDisposable
{
    ...
    public ItemObject(int itemId, ulong owner)
    {
        ...
        try
        {
            ...
            do
            {
                if(((WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Spells[i].SpellTrigger ==
                            ITEM_SPELLTRIGGER_TYPE.USE) ||
                        (WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Spells[i].SpellTrigger ==
                            ITEM_SPELLTRIGGER_TYPE.NO_DELAY_USE)) &&
                    (WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Spells[i].SpellCharges != 0))
                {
        ...
        } catch(Exception ex)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.WARNING, "Duplicate Key Warning {0} ITEMID:{1} OWNERGUID:{2}", ex, itemId, owner);
        }
    }
    ...
    public ItemObject(ulong guidVal, WS_PlayerData.CharacterObject owner = null, bool equipped = false)
    {
        ...
        WorldServiceLocator.WorldServer.CharacterDatabase
            .Query($"SELECT * FROM characters_inventory WHERE item_guid = \"{guidVal}\";", ref mySqlQuery);
        if(mySqlQuery.Rows.Count == 0)
        {
            Information.Err().Raise(1, "ItemObject.New", $"itemGuid {guidVal} not found in SQL database!");
        }
        GUID = Conversions.ToULong(
            Operators.AddObject(mySqlQuery.Rows[0]["item_guid"], WorldServiceLocator.GlobalConstants.GUID_ITEM));
        ...
        checked
        {
            if(tmp.Length > 0)
            {
                ...
                for(var i = 0; i <= num; i++)
                {
                    if(Operators.CompareString(Strings.Trim(tmp[i]), string.Empty, TextCompare: false) != 0)
                    {
                        var tmp2 = Strings.Split(tmp[i], ":");
                        Enchantments.Add(
                            Conversions.ToByte(tmp2[0]),
                            new WS_Items.TEnchantmentInfo(
                                Conversions.ToInteger(tmp2[1]),
                                Conversions.ToInteger(tmp2[2]),
                                Conversions.ToInteger(tmp2[3])));
                        if(equipped)
                        {
            ...
            try
            {
                ...
                while(enumerator.MoveNext())
                {
                    ...
                    if(Operators.ConditionalCompareObjectNotEqual(
                        ...
                        TextCompare: false))
                    {
                        Items[row.As<byte>("item_slot")] = (new(row.As<long, ulong>("item_guid")));
                    }
                }
            } finally
            {
    ...
    {
        Items = WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].IsContainer
            ? new Dictionary<byte, ItemObject>()
            : null;
    }
    ...
    private void SaveAsNew()
    {
        ...
        {
            temp.Add(
                $"{enchantment.Key}:{enchantment.Value.ID}:{enchantment.Value.Duration}:{enchantment.Value.Charges}");
        }
    ...
    public void AddEnchantBonus(byte slot, WS_PlayerData.CharacterObject objCharacter = null)
    {
        ...
        if((objCharacter == null) ||
            !WorldServiceLocator.WSDBCDatabase.SpellItemEnchantments.ContainsKey(Enchantments[slot].ID))
        {
            return;
        }
        byte i = 0;
        do
        {
            checked
            {
                if((WorldServiceLocator.WSDBCDatabase.SpellItemEnchantments[Enchantments[slot].ID].SpellID[i] != 0) &&
                    WorldServiceLocator.WSSpells.SPELLs
                        .ContainsKey(
                            WorldServiceLocator.WSDBCDatabase.SpellItemEnchantments[Enchantments[slot].ID].SpellID[
                                    i]))
                {
                    var spellInfo = WorldServiceLocator.WSSpells.SPELLs[
                        WorldServiceLocator.WSDBCDatabase.SpellItemEnchantments[Enchantments[slot].ID].SpellID[i]];
                    byte j = 0;
                    do
                    {
                        if(spellInfo.SpellEffects[j] != null)
                        {
                            ...
                            {
                                var obj = WorldServiceLocator.WSSpells.AURAs[
                                    spellInfo.SpellEffects[j].ApplyAuraIndex];
                                WS_Base.BaseUnit Target = objCharacter;
                                WS_Base.BaseObject Caster = objCharacter;
                                obj(
                                    ref Target,
                                    ref Caster,
                                    ref spellInfo.SpellEffects[j],
                                    spellInfo.ID,
                                    1,
                                    AuraAction.AURA_ADD);
    ...
    public void Delete()
    {
        checked
        {
            if(ItemEntry == WorldServiceLocator.GlobalConstants.PETITION_GUILD)
            {
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update(
                        $"DELETE FROM petitions WHERE petition_itemGuid = {Conversions.ToString(GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM)};");
            }
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Update(
                    $"DELETE FROM characters_inventory WHERE item_guid = {GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM}");
            if(WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].IsContainer)
            {
                foreach(var item in Items)
                {
    ...
    public bool GenerateLoot()
    {
        ...
        WorldServiceLocator.WorldServer.WorldDatabase
            .Query($"SELECT * FROM item_loot WHERE entry = {ItemEntry};", ref mySqlQuery);
        if(mySqlQuery.Rows.Count == 0)
        {
            return false;
        }
    ...
    public bool IsBroken() { return (Durability == 0) && (ItemInfo.Durability > 0); }
    ...
    public void ModifyDurability(float percent, ref WS_Network.ClientClass client)
    {
        ...
        if(WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Durability > 0)
        {
            ...
            durability = checked((int)Math.Round(
                durability -
                    Conversion.Fix(WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Durability * percent)));
            if(Durability < 0)
            {
                Durability = 0;
            }
                    ...
                    do
                    {
                        if(spellInfo.SpellEffects[j] != null)
                        {
                            ...
                            if(iD == SpellEffects_Names.SPELL_EFFECT_APPLY_AURA)
                            {
                                ...
                                ulong ownerGUID;
                                WS_Base.BaseUnit Target = (cHARACTERs = WorldServiceLocator.WorldServer.CHARACTERs)[
                                    ownerGUID = OwnerGUID];
                                Dictionary<ulong, WS_PlayerData.CharacterObject> cHARACTERs2;
                                ulong ownerGUID2;
                                WS_Base.BaseObject Caster = (cHARACTERs2 =
                                    WorldServiceLocator.WorldServer.CHARACTERs)[ownerGUID2 = OwnerGUID];
                                obj(
                                    ref Target,
                                    ref Caster,
                                    ref spellInfo.SpellEffects[j],
            ...
            do
            {
                checked
                {
                    if(characterObject.Items.ContainsKey(i))
                    {
                        ...
                        while(j <= ((uint)b))
                        {
                            if(characterObject.Items[i].Items.ContainsKey(j) &&
                                (characterObject.Items[i].Items[j] == this))
                            {
                                return i;
                            }
                            j = (byte)unchecked((uint)(j + 1));
    ...
    public uint GetDurabilityCost
    {
        get
        {
            checked
            {
                try
                {
                    var lostDurability = WorldServiceLocator.WorldServer.ITEMDatabase[ItemEntry].Durability -
                        Durability;
                    if(lostDurability > 300)
                    {
                        lostDurability = 300;
                    }
                    var subClass = (ItemInfo.ObjectClass != ITEM_CLASS.ITEM_CLASS_WEAPON)
                        ? (((int)ItemInfo.SubClass) + 21)
                        : ((int)ItemInfo.SubClass);
                    var durabilityCost = (uint)Math.Round(
                        lostDurability *
                            (WorldServiceLocator.WSDBCDatabase.DurabilityCosts[ItemInfo.Level, subClass] / 40.0 *
                                100.0));
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "Durability cost: {0}", durabilityCost);
                    return durabilityCost;
                } catch(Exception ex)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.WARNING, "Exception at Durbatility Cost: {0}", ex);
                    return 0u;
                }
            }
        }
            ...
            do
            {
                checked
                {
                    if(characterObject.Items.ContainsKey(j))
                    {
                        ...
                        while(l <= ((uint)b))
                        {
                            if(characterObject.Items[j].Items.ContainsKey(l) &&
                                (characterObject.Items[j].Items[l] == this))
                            {
                                return l;
                            }
    ...
    public bool IsRanged => (ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_WEAPON) &&
        ((ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) ||
            (ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_CROSSBOW) ||
            (ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION));
