...
public class WS_Transports
{
    ...
    public void LoadTransports()
    {
        try
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.INFORMATION, "Database: {0} Transports initialized.", TransportQuery.Rows.Count);
        } catch(DirectoryNotFoundException ex)
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("Database : TransportQuery missing.", ex);
    ...
    public class TransportObject : WS_GameObjects.GameObject
    {
        ...
        public TransportObject(int ID_, string Name, int Period_) : base(
            ID_,
            WorldServiceLocator.WSTransports.GetNewGUID())
        {
        ...
        public void CheckCell(bool Teleported = false)
        {
            ...
            if(WorldServiceLocator.WSMaps.Maps[MapID].Tiles[CellX, CellY] != null)
            {
                ...
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.WARNING, $"Failed to remove Transport {GUID} {ex}", ex);
                }
            }
            CellX = TileX;
            CellY = TileY;
            if(WorldServiceLocator.WSMaps.Maps[MapID].Tiles[CellX, CellY] != null)
            {
                ...
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.WARNING, $"Failed to add Transport {GUID} {ex}", ex);
                }
            }
            NotifyEnter();
        }
        ...
        public void CreateEveryoneOnTransport(ref WS_PlayerData.CharacterObject Character)
        {
            ...
            Packets.Packets.PacketClass mePacket = new(Opcodes.SMSG_UPDATE_OBJECT);
            mePacket.AddInt32(1);
            mePacket.AddInt8(0);
            Packets.Packets.UpdateClass meTmpUpdate = new(
            ...
            for(var i = 0; i < array.Length; i = checked(i + 1))
            {
                ...
                switch(tmpUnit)
                {
                    case WS_PlayerData.CharacterObject _:
                    {
                        ...
                        if(flag)
                        {
                            Packets.Packets.PacketClass myPacket2 = new(Opcodes.SMSG_UPDATE_OBJECT);
                            try
                            {
                                myPacket2.AddInt32(1);
                                myPacket2.AddInt8(0);
                                Packets.Packets.UpdateClass myTmpUpdate2 = new(
                                    WorldServiceLocator.GlobalConstants.FIELD_MASK_SIZE_PLAYER);
                            ...
                            tmpUnit.SeenBy.Add(Character.GUID);
        ...
        public override void FillAllUpdateFlags(
            ...
            checked
            {
                ...
                if(!WorldServiceLocator.WSDBCDatabase.TaxiPaths.ContainsKey(PathID))
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.CRITICAL,
                            "An transport [{0} - {1}] is created with an invalid TaxiPath.",
                            ID,
                            TransportName);
                    return false;
                }
                ...
                if(WorldServiceLocator.WSDBCDatabase.TaxiPathNodes.ContainsKey(PathID))
                {
                    ...
                    for(var i = 0; i <= num; i++)
                    {
                        if(MapChange == 0)
                        {
                            if(WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID].ContainsKey(i) &&
                                WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID].ContainsKey(i + 1))
                            {
                                if(WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].MapID ==
                                    WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i + 1].MapID)
                                {
                                    PathPoints.Add(
                                        new TransportMove(
                                            WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].x,
                                            WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].y,
                                            WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].z,
                                            (uint)WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].MapID,
                                            WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].action,
                                            WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].waitTime));
                                    if(WorldServiceLocator.WSMaps.Maps
                                        .ContainsKey(
                                            (uint)WorldServiceLocator.WSDBCDatabase.TaxiPathNodes[PathID][i].MapID))
                                    {
                                        MapsUsed++;
                    ...
                    {
                        PathPoints[j].DistFromPrev = ((PathPoints[j].ActionFlag == 1) ||
                                (PathPoints[j].MapID != PathPoints[j - 1].MapID))
                            ? 0f
                    ...
                    for(var n = 0; n <= num3; n++)
                    {
                        unchecked
                        {
                            ...
                            if(j2 >= 0)
                            {
                                tmpDist = (PathPoints[j2].ActionFlag != 2)
                                    ? (tmpDist + PathPoints[j2].DistFromPrev)
                                    : 0f;
                    ...
                    for(var k = 0; k <= num6; k++)
                    {
                        ...
                        if((d < PathPoints[k + 1].DistFromPrev) && (tTo > 0f))
                        {
                            while((d < PathPoints[k + 1].DistFromPrev) && (tTo > 0f))
                            {
                                ...
                                if(d > 0f)
                                {
                                    var newX = PathPoints[k].X +
                                        ((PathPoints[k + 1].X - PathPoints[k].X) * d /
                                            PathPoints[k + 1].DistFromPrev);
                                    var newY = PathPoints[k].Y +
                                        ((PathPoints[k + 1].Y - PathPoints[k].Y) * d /
                                            PathPoints[k + 1].DistFromPrev);
                                    var newZ = PathPoints[k].Z +
                                        ((PathPoints[k + 1].Z - PathPoints[k].Z) * d /
                                            PathPoints[k + 1].DistFromPrev);
                                    teleport = false;
                                    ...
                                    {
                                        Waypoints.Add(
                                            new TransportWP(k, t, newX, newY, newZ, PathPoints[k].MapID, teleport));
                                    }
                                }
                                if(tFrom < tTo)
                                {
                                    d = (!(tFrom <= 30000f))
                                        ? (450f + (30f * ((tFrom - 30000f) / 1000f)))
                                        : (0.5f * (tFrom / 1000f) * (tFrom / 1000f));
                                    d -= PathPoints[k].DistSinceStop;
                                } else
                                {
                                    d = (!(tTo <= 30000f))
                                        ? (450f + (30f * ((tTo - 30000f) / 1000f)))
                                        : (0.5f * (tTo / 1000f) * (tTo / 1000f));
                        ...
                        t = (!(PathPoints[k + 1].tFrom > PathPoints[k + 1].tTo))
                            ? ((int)(t + (((long)PathPoints[k + 1].tTo) % 100)))
                            : ((int)(t + (100 - (((long)PathPoints[k + 1].tTo) % 100))));
                        teleport = false;
                            ...
                            foreach(var plGUID in tMapTile.PlayersHere.ToArray())
                            {
                                ...
                                try
                                {
                                    ...
                                    if(WorldServiceLocator.WorldServer.CHARACTERs
                                        .TryGetValue(plGUID, out var _character))
                                    {
                                    ...
                                    {
                                        WorldServiceLocator.WorldServer.Log
                                            .WriteLine(LogType.WARNING, $"Failed to retrieve character {plGUID}");
                                    }
                                } catch(Exception ex)
                                {
                                    WorldServiceLocator.WorldServer.Log
                                        .WriteLine(LogType.FAILED, ex.Message + Environment.NewLine);
                                }
                            }
                        }
        ...
        {
            var oldMap = MapID;
            foreach(var tmpUnit in Passengers.ToArray())
            {
                try
                {
                    if(tmpUnit == null)
                    ...
                    {
                        case WS_PlayerData.CharacterObject _:
                            if((((WS_PlayerData.CharacterObject)tmpUnit).OnTransport != null) &&
                                (((WS_PlayerData.CharacterObject)tmpUnit).OnTransport == this))
                            {
                                ((WS_PlayerData.CharacterObject)tmpUnit).Teleport(
                                    PosX,
                                    PosY,
                                    PosZ,
                                    ((WS_PlayerData.CharacterObject)tmpUnit).orientation,
                                    checked((int)NewMap));
                                continue;
                            }
                            lock(Passengers)
                ...
                } catch(Exception ex)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.CRITICAL,
                            "Failed to transfer player [0x{0:X}].{1}{2}",
                            tmpUnit.GUID,
                            Environment.NewLine,
                            ex.ToString());
                }
        ...
        public void Update()
        {
            ...
            while((Math.Abs(checked(Timer - Waypoints[CurrentWaypoint].Time)) % PathTime) >
                (Math.Abs(checked(Waypoints[NextWaypoint].Time - Waypoints[CurrentWaypoint].Time)) % PathTime))
            {
                CurrentWaypoint = GetNextWaypoint();
                NextWaypoint = GetNextWaypoint();
                ...
                {
                    TeleportTransport(
                        Waypoints[CurrentWaypoint].MapID,
                        Waypoints[CurrentWaypoint].X,
                        Waypoints[CurrentWaypoint].Y,
                        Waypoints[CurrentWaypoint].Z);
                } else
                {
