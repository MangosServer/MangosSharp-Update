...
public class WS_PlayerData
{
    public class CharacterObject : WS_Base.BaseUnit, IDisposable
    {
        ...
        private Hashtable UpdateData;
        private BitArray UpdateMask;
        ...
        public CharacterObject()
        {
            ...
            AttackTimeBase = (new short[3] { 2000, 0, 0 });
            AttackTimeMods = (new float[3] { 1f, 1f, 1f });
        ...
        {
            if(ClientVal is null)
            {
                throw new ArgumentNullException(nameof(ClientVal));
            }
            ...
            checked
            {
                ActiveSpells = (new WS_Base.BaseActiveSpell[WorldServiceLocator.GlobalConstants.MAX_AURA_EFFECTs -
                        1 +
                    1]);
                client = ClientVal;
                GUID = GuidVal;
                ...
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Query(
                        $"SELECT * FROM characters WHERE char_guid = {GUID}; UPDATE characters SET char_online = 1 WHERE char_guid = {GUID};",
                        ref MySQLQuery);
                if(MySQLQuery.Rows.Count == 0)
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.DEBUG,
                            "[{0}:{1}] Unable to get SQLDataBase info for character [GUID={2:X}]",
                            client.IP,
                            client.Port,
                            GUID);
                    Dispose();
            ...
            Class = (Classes)MySQLQuery.Rows[0].As<byte>("char_class");
            ...
            if(WorldServiceLocator.WSDBCDatabase.CharRaces.TryGetValue(MySQLQuery.Rows[0].As<int>("char_race"), out var value))

            {
                Faction = value.FactionID;
            ...
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Query(
                    $"UPDATE characters_spells SET cooldown = 0, cooldownitem = 0 WHERE guid = {GUID} AND cooldown > 0 AND cooldown < {WorldServiceLocator.Functions.GetTimestamp(DateAndTime.Now)}; \r\n                SELECT * FROM characters_spells WHERE guid = {GUID}; \r\n                UPDATE characters_spells SET cooldown = 0, cooldownitem = 0 WHERE guid = {GUID} AND cooldown > 0 AND cooldown < {WorldServiceLocator.Functions.GetTimestamp(DateAndTime.Now)};",
                    ref SpellQuery);
            ...
            try
            {
                ...
                while(enumerator.MoveNext())
                {
                    ...
                    Spells.Add(
                        row.As<int>("spellid"),
                        new WS_Spells.CharacterSpell(
                            row.As<int>("spellid"),
                            row.As<byte>("active"),
                            row.As<uint>("cooldown"),
                            row.As<int>("cooldownitem")));
                }
            } finally
            {
            ...
            checked
            {
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[i2]), string.Empty, TextCompare: false) != 0)
                        {
                            var tmp5 = Strings.Split(tmp[i2], ":");
                            if(tmp5.Length == 3)
                            {
                                Skills[Conversions.ToInteger(tmp5[0])] = new WS_PlayerHelper.TSkill(
                                    Conversions.ToShort(tmp5[1]),
                                    Conversions.ToShort(tmp5[2]));
                ...
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[i3]), string.Empty, TextCompare: false) == 0)
                        {
                            continue;
                        }
                        ...
                        if((AuraSlot < 0) ||
                            (AuraSlot >= WorldServiceLocator.GlobalConstants.MAX_AURA_EFFECTs_VISIBLE) ||
                            !WorldServiceLocator.WSSpells.SPELLs.ContainsKey(AuraSpellID) ||
                            (ActiveSpells[AuraSlot] != null))
                        {
                            continue;
                        }
                        int duration;
                ...
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[n]), string.Empty, TextCompare: false) != 0)
                        {
                ...
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[l]), string.Empty, TextCompare: false) == 0)
                        {
                            continue;
                        }
                        byte j2 = 0;
                ...
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[k]), string.Empty, TextCompare: false) != 0)
                        {
                ...
                if(tmp.Length > 0)
                {
                    ...
                    {
                        if(Operators.CompareString(Strings.Trim(tmp[j]), string.Empty, TextCompare: false) != 0)
                        {
                            var tmp3 = Strings.Split(tmp[j], ":");
                            ActionButtons[Conversions.ToByte(tmp3[0])] = new WS_PlayerHelper.TActionButton(
                                Conversions.ToInteger(tmp3[1]),
                                Conversions.ToByte(tmp3[2]),
                                Conversions.ToByte(tmp3[3]));
                        }
                    }
                }
                ...
                MySQLQuery.Clear();
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Query($"SELECT * FROM characters_inventory WHERE item_bag = {GUID};", ref MySQLQuery);
                ...
                try
                {
                    ...
                    while(enumerator2.MoveNext())
                    {
                        ...
                        if(Operators.ConditionalCompareObjectNotEqual(
                            row["item_slot"],
                            WorldServiceLocator.GlobalConstants.ITEM_SLOT_NULL,
                            TextCompare: false))
                        {
                            var tmpItem = WorldServiceLocator.WSItems
                                .LoadItemByGUID(
                                    row.As<long, ulong>("item_guid"),
                                    this,
                                    row.As<byte>("item_slot") < 19u);
                ...
                MySQLQuery.Clear();
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Query($"SELECT * FROM corpse WHERE player = {GUID};", ref MySQLQuery);
                if(MySQLQuery.Rows.Count > 0)
                {
                    corpseGUID = Conversions.ToULong(
                        Operators.AddObject(
                            MySQLQuery.Rows[0]["guid"],
                    ...
                    if((positionX == corpsePositionX) &&
                        (positionY == corpsePositionY) &&
                        (positionZ == corpsePositionZ) &&
                        (MapID == corpseMapID))
                    {
        ...
        protected virtual void Dispose(bool disposing)
        {
            if(!_disposedValue)
            {
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update(
                        $"DELETE FROM characters_inventory WHERE item_bag = {GUID} AND item_slot >= {69} AND item_slot <= {79}");
        ...
        public void AddToCombat(WS_Base.BaseUnit Unit)
        {
            ...
            {
                lastPvpAction = WorldServiceLocator.NativeMethods.timeGetTime(string.Empty);
            } else
            {
        ...
        public void AddXP(int Ammount, int RestedBonus, ulong VictimGUID = 0uL, bool LogIt = true)
        {
            ...
            checked
            {
                ...
                do
                {
                    if(XP >= WorldServiceLocator.WSPlayerInitializator.XPTable[Level])
                    {
                        ...
                        WorldServiceLocator.WorldServer.ClsWorldServer.Cluster
                            .ClientUpdate(client.Index, (uint)ZoneID, Level);
                        ...
                        Packets.Packets.PacketClass SMSG_LEVELUP_INFO = new(Opcodes.SMSG_LEVELUP_INFO);
                        try
                        {
                        ...
                        SetUpdateFlag(
                            135,
                            (float)(Damage.Maximum + ((AttackPower + AttackPowerMods) * 0.071428571428571425)));
                ...
                } while ((XP >= WorldServiceLocator.WSPlayerInitializator.XPTable[Level]) &&
        ...
        public void BindPlayer(ulong cGUID)
        {
            ...
            SaveCharacter();
            Packets.Packets.PacketClass SMSG_BINDPOINTUPDATE = new(Opcodes.SMSG_BINDPOINTUPDATE);
            try
            {
            ...
            Packets.Packets.PacketClass SMSG_PLAYERBOUND = new(Opcodes.SMSG_PLAYERBOUND);
            try
            {
        ...
        public override bool CanSee(ref WS_Base.BaseObject objCharacter)
        {
            ...
            if(DEAD && (corpseGUID != 0))
            {
                ...
                if(WorldServiceLocator.WSCombat
                        .GetDistance(objCharacter, corpsePositionX, corpsePositionY, corpsePositionZ) <
                    objCharacter.VisibleDistance)
                {
                    ...
                    {
                        var objCharacter2 = (WS_Base.BaseUnit)objCharacter;
                    ...
                    if((objCharacter.Invisibility == InvisibilityLevel.INIVISIBILITY) &&
                        (objCharacter.Invisibility_Value > CanSeeInvisibility_Invisibility))
                    {
                        return false;
                    }
        ...
        public void ChangeSpeedForced(ChangeSpeedType Type, float NewSpeed)
        {
            checked
            {
                ...
                Packets.Packets.PacketClass packet = null;
                try
                {
                    switch(Type)
                    {
                        ...
                        case ChangeSpeedType.RUN:
                            packet = new Packets.Packets.PacketClass(Opcodes.SMSG_FORCE_RUN_SPEED_CHANGE);
                            RunSpeed = NewSpeed;
                            break;

                        case ChangeSpeedType.RUNBACK:
                            packet = new Packets.Packets.PacketClass(Opcodes.SMSG_FORCE_RUN_BACK_SPEED_CHANGE);
                            RunBackSpeed = NewSpeed;
                            break;

                        case ChangeSpeedType.SWIM:
                            packet = new Packets.Packets.PacketClass(Opcodes.SMSG_FORCE_SWIM_SPEED_CHANGE);
                            SwimSpeed = NewSpeed;
                            break;

                        case ChangeSpeedType.SWIMBACK:
                            packet = new Packets.Packets.PacketClass(Opcodes.SMSG_FORCE_SWIM_BACK_SPEED_CHANGE);
                            SwimBackSpeed = NewSpeed;
                            break;

                        case ChangeSpeedType.TURNRATE:
                            packet = new Packets.Packets.PacketClass(Opcodes.SMSG_FORCE_TURN_RATE_CHANGE);
                            TurnRate = NewSpeed;
                            break;
                    }
        ...
        public void CommandResponse(string Message)
        {
            ...
            {
                var packet = WorldServiceLocator.Functions
                    .BuildChatMessage(2147483647uL, Msg, ChatMsg.CHAT_MSG_SYSTEM, LANGUAGES.LANG_GLOBAL);
                client.Send(ref packet);
                packet.Dispose();
            }
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_MESSAGECHAT", client.IP, client.Port);
        }
        ...
        public override void DealDamage(int Damage, WS_Base.BaseUnit Attacker = null)
        {
            ...
            if(Attacker != null)
            {
                ...
                foreach(var cGUID in creaturesNear.ToArray())
                {
                    if(WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(cGUID) &&
                        ...
                        WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGUID].IsGuard &&
                        ...
                        !inCombatWith.Contains(cGUID) &&
                        (GetReaction(WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGUID].Faction) ==
                            TReaction.FIGHT_SUPPORT) &&
                        (WorldServiceLocator.WSCombat
                                .GetDistance(WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGUID], this) <=
                            WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGUID].AggroRange(this)))
                    {
                        WorldServiceLocator.WorldServer.WORLD_CREATUREs[cGUID].aiScript
                            .OnGenerateHate(ref Attacker, Damage);
                    }
                }
            }
            GroupUpdateFlag |= 2u;
            checked
            {
                ...
                SetUpdateFlag(22, Life.Current);
                SendCharacterUpdate();
                if((Class == Classes.CLASS_WARRIOR) ||
                    ((Class == Classes.CLASS_DRUID) &&
                        ((ShapeshiftForm == ShapeshiftForm.FORM_BEAR) ||
                            (ShapeshiftForm == ShapeshiftForm.FORM_DIREBEAR))))
                {
        ...
        public override void Die(ref WS_Base.BaseUnit Attacker)
        {
            ...
            {
                if(WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(uGuid) &&
                    WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(uGuid))
                {
            ...
            checked
            {
                ...
                if(Attacker is null or WS_Creatures.CreatureObject)
                {
                    ...
                    Packets.Packets.PacketClass SMSG_DURABILITY_DAMAGE_DEATH = new(
                        Opcodes.SMSG_DURABILITY_DAMAGE_DEATH);
                    try
                    {
            ...
            checked
            {
                ...
                do
                {
                    if(Items.ContainsKey(l))
                    {
                        ...
                        {
                            SetUpdateFlag(
                                260 + (l * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                Items[l].ItemEntry);
                            foreach(var Enchant in Items[l].Enchantments)
                            {
                                SetUpdateFlag(
                                    261 +
                                        (Enchant.Key * 3) +
                                        (l * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Enchant.Value.ID);
                                SetUpdateFlag(
                                    262 +
                                        (Enchant.Key * 3) +
                                        (l * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Enchant.Value.Charges);
                                SetUpdateFlag(
                                    263 +
                                        (Enchant.Key * 3) +
                                        (l * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Enchant.Value.Duration);
                            }
                            SetUpdateFlag(
                                268 + (l * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                Items[l].RandomProperties);
                        }
            ...
            } else
            {
                ...
                if(!OK)
                {
                    WorldServiceLocator.WSSpells
                        .SendCastResult(SpellFailedReason.SPELL_FAILED_INTERRUPTED, ref client, SpellID);
                }
                client.Character.RemoveAuraBySpell(SpellID);
                foreach(var tmpDO in client.Character.dynamicObjects.ToArray())
                {
                ...
                foreach(var tmpGO in client.Character.gameObjects.ToArray())
                {
        ...
        {
            if(!WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo.ContainsKey(FactionID) ||
                !WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo.ContainsKey(Faction))
            {
                return TReaction.NEUTRAL;
            }
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyMask == 0L) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendMask == 0L) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction1 == 0) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction2 == 0) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction3 == 0) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction4 == 0))
            {
                return TReaction.NEUTRAL;
            }
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyMask == 0L) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendMask == 0L) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction1 != Faction) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction2 != Faction) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction3 != Faction) &&
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction4 != Faction))
            {
                return TReaction.NEUTRAL;
            }
            ...
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendFaction1 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendFaction2 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendFaction3 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendFaction4 == Faction))
            {
                return TReaction.FIGHT_SUPPORT;
            }
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].friendMask &
                    WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[Faction].ourMask) !=
                0)
            {
                return TReaction.FIGHT_SUPPORT;
            }
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction1 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction2 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction3 == Faction) ||
                (WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyFaction4 == Faction))
            {
            ...
            if((WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[FactionID].enemyMask &
                    WorldServiceLocator.WSDBCDatabase.FactionTemplatesInfo[Faction].ourMask) !=
                0)
            {
                return TReaction.HOSTILE;
        ...
        public ReputationRank GetReputation(int FactionTemplateID)
        {
            ...
            checked
            {
                var points = WorldServiceLocator.Functions
                        .HaveFlag(
                            (uint)WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].flags[0],
                            (byte)(((int)Race) - 1))
                    ? WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].rep_stats[0]
                    : (WorldServiceLocator.Functions
                            .HaveFlag(
                                (uint)WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].flags[1],
                                (byte)(((int)Race) - 1))
                        ? WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].rep_stats[1]
                        : (WorldServiceLocator.Functions
                                .HaveFlag(
                                    (uint)WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].flags[2],
                                    (byte)(((int)Race) - 1))
                            ? WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].rep_stats[2]
                            : (WorldServiceLocator.Functions
                                    .HaveFlag(
                                        (uint)WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].flags[3],
                                        (byte)(((int)Race) - 1))
                                ? WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].rep_stats[3]
                                : 0)));
                ...
                return num switch
                {
                    ...
                    > 9000 => ReputationRank.Revered,
                    > 3000 => ReputationRank.Honored,
                    > 0 => ReputationRank.Friendly,
                    > -3000 => ReputationRank.Neutral,
                    > -6000 => ReputationRank.Unfriendly,
                    _ => (num > (-42000)) ? ReputationRank.Hostile : ReputationRank.Hated,
                };
            }
        }
        ...
        public float GetStealthDistance(ref WS_Base.BaseUnit objCharacter)
        {
            ...
            checked
            {
                ...
                return (float)(VisibleDistance +
                    ((objCharacter.CanSeeInvisibility_Stealth - Invisibility_Bonus) / 5.0));
            }
        }
        ...
        public void HandleDrowning(object state)
        {
            checked
            {
                ...
                {
                    if(positionZ >
                        (WorldServiceLocator.WSMaps.GetWaterLevel(positionX, positionY, (int)MapID) - 1.6))
                    {
                    ...
                    if(underWaterTimer.DrowningValue < 0)
                    {
                        ...
                        LogEnvironmentalDamage(
                            DamageTypes.DMG_HOLY,
                            (int)(0.1f * Life.Maximum * underWaterTimer.DrowningDamage));
                ...
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.FAILED, "Error at HandleDrowning():", ex.ToString());
                    underWaterTimer?.Dispose();
                    underWaterTimer = null;
                }
            }
        ...
        public bool HaveSpell(int SpellID) { return Spells.ContainsKey(SpellID); }
        ...
        public void HonorLoad()
        {
            ...
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Query($"SELECT * FROM characters_honor WHERE char_guid = {GUID};", ref MySQLQuery);
            if(MySQLQuery.Rows.Count == 0)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.FAILED,
                        "Unable to get SQLDataBase honor info for character [GUID={0:X}]",
                        GUID);
                return;
            }
        ...
        {
            Packets.Packets.PacketClass packet = new(Opcodes.SMSG_PVP_CREDIT);
            try
            {
        ...
        {
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Update($"INSERT INTO characters_honor (char_guid)  VALUES ({Conversions.ToString(GUID)});");
        }
        ...
        public void Initialize()
        {
            ...
            checked
            {
                ...
                Damage.Minimum++;
                RangedDamage.Type = 0;
                RangedDamage.Minimum++;
                if(Access >= AccessLevel.GameMaster)
                {
                    GM = true;
                }
                if(!Items.ContainsKey(17) ||
                    (Items[17].ItemInfo.ObjectClass != ITEM_CLASS.ITEM_CLASS_WEAPON) ||
                    ((Items[17].ItemInfo.SubClass != ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                        (Items[17].ItemInfo.SubClass != ITEM_SUBCLASS.ITEM_SUBCLASS_CROSSBOW) &&
                        (Items[17].ItemInfo.SubClass != ITEM_SUBCLASS.ITEM_SUBCLASS_POTION)))
                {
                    return;
                }
                ...
                do
                {
                    if(Items.ContainsKey(i) && (Items[i].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_QUIVER))
                    {
                        ...
                        {
                            if((slot.Value.ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_PROJECTILE) &&
                                (slot.Value.ItemInfo.SubClass == AmmoType))
                            {
                                var charManagementHandler = WorldServiceLocator.CharManagementHandler;
                                var objCharacter = this;
                                if(charManagementHandler.CanUseAmmo(ref objCharacter, slot.Value.ItemEntry) ==
                                    InventoryChangeFailure.EQUIP_ERR_OK)
                                {
                                    ...
                                    var wS_Combat = WorldServiceLocator.WSCombat;
                                    objCharacter = this;
                                    wS_Combat.CalculateMinMaxDamage(
                                        ref objCharacter,
        ...
        public override bool IsEnemyTo(ref WS_Base.BaseUnit Unit)
        {
            ...
            if(Unit is CharacterObject characterObject)
            {
                ...
                if(WorldServiceLocator.Functions.HaveFlags((int)cPlayerFlags, 128) &&
                    WorldServiceLocator.Functions.HaveFlags((int)characterObject.cPlayerFlags, 128))
                {
                    return true;
                }
        ...
        public bool IsQuestCompleted(int QuestID)
        {
            ...
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Query(
                    $"SELECT quest_id FROM characters_quests WHERE char_guid = {GUID} AND quest_status = -1 AND quest_id = {QuestID};",
                    ref q);
            ...
            checked
            {
                ...
                if(dstBag == 0)
                {
                    if(Item.ItemInfo.Stackable > 1)
                    {
                        ...
                        do
                        {
                            if((Items[slot].ItemEntry == Item.ItemEntry) &&
                                (Items[slot].StackCount < Items[slot].ItemInfo.Stackable))
                            {
                ...
                {
                    if(((Items[dstBag].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                            (Items[dstBag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_FOOD) &&
                            (Item.ItemInfo.BagFamily != ITEM_BAG.SOUL_SHARD)) ||
                        ((Items[dstBag].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                            (Items[dstBag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                            (Item.ItemInfo.BagFamily != ITEM_BAG.HERB)) ||
                        ((Items[dstBag].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                            (Items[dstBag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
                            (Item.ItemInfo.BagFamily != ITEM_BAG.ENCHANTING)) ||
                        ((Items[dstBag].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_QUIVER) &&
                            (Items[dstBag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                            (Item.ItemInfo.BagFamily != ITEM_BAG.ARROW)) ||
                        ((Items[dstBag].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_QUIVER) &&
                            (Items[dstBag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
                            (Item.ItemInfo.BagFamily != ITEM_BAG.BULLET)))
                    {
                        WorldServiceLocator.WorldServer.Log
                            .WriteLine(
                                LogType.DEBUG,
                                "{0} - {1} - {2}",
                                Items[dstBag].ItemInfo.ObjectClass,
                                Items[dstBag].ItemInfo.SubClass,
                                Item.ItemInfo.BagFamily);
                        var wS_Items = WorldServiceLocator.WSItems;
                        objCharacter = this;
                        wS_Items.SendInventoryChangeFailure(
                            ref objCharacter,
                    ...
                    if(Item.ItemInfo.Stackable > 1)
                    {
                        ...
                        {
                            if((i.Value.ItemEntry == Item.ItemEntry) &&
                                (i.Value.StackCount < i.Value.ItemInfo.Stackable))
                            {
                    ...
                    while(slot2 <= ((uint)b))
                    {
                        if(!Items[dstBag].Items.ContainsKey(slot2) &&
                            (ItemCANEQUIP(Item, dstBag, slot2) == InventoryChangeFailure.EQUIP_ERR_OK))
                        {
                ...
                objCharacter = this;
                wS_Items2.SendInventoryChangeFailure(
                    ref objCharacter,
                    InventoryChangeFailure.EQUIP_ERR_BAG_FULL,
                    Item.GUID,
                    0uL);
                return false;
            }
        ...
        public bool ItemADD_AutoSlot(ref ItemObject Item)
        {
            ...
            {
                if((Item.ItemInfo.BagFamily == ITEM_BAG.KEYRING) ||
                    (Item.ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_KEY))
                {
                    byte slot = 81;
                    do
                    {
                        checked
                        {
                            if(Items.ContainsKey(slot) &&
                                (Items[slot].ItemEntry == Item.ItemEntry) &&
                                (Items[slot].StackCount < Items[slot].ItemInfo.Stackable))
                            {
                ...
                } else if(Item.ItemInfo.BagFamily != 0)
                {
                    ...
                    do
                    {
                        checked
                        {
                            if(Items.ContainsKey(bag2) &&
                                (Items[bag2].ItemInfo.SubClass != 0) &&
                                (((Items[bag2].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_FOOD) &&
                                        (Item.ItemInfo.BagFamily == ITEM_BAG.SOUL_SHARD)) ||
                                    ((Items[bag2].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                                        (Item.ItemInfo.BagFamily == ITEM_BAG.HERB)) ||
                                    ((Items[bag2].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
                                        (Item.ItemInfo.BagFamily == ITEM_BAG.ENCHANTING)) ||
                                    ((Items[bag2].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                                        (Item.ItemInfo.BagFamily == ITEM_BAG.ARROW)) ||
                                    ((Items[bag2].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
                                        (Item.ItemInfo.BagFamily == ITEM_BAG.BULLET))))
                            {
                                ...
                                {
                                    if((slot5.Value.ItemEntry == Item.ItemEntry) &&
                                        (slot5.Value.StackCount < slot5.Value.ItemInfo.Stackable))
                                    {
                ...
                do
                {
                    checked
                    {
                        if(Items.ContainsKey(slot6) &&
                            (Items[slot6].ItemEntry == Item.ItemEntry) &&
                            (Items[slot6].StackCount < Items[slot6].ItemInfo.Stackable))
                        {
                ...
                do
                {
                    checked
                    {
                        if(Items.ContainsKey(bag4))
                        {
                            ...
                            {
                                if((slot7.Value.ItemEntry == Item.ItemEntry) &&
                                    (slot7.Value.StackCount < slot7.Value.ItemInfo.Stackable))
                                {
            ...
            } else if(Item.ItemInfo.BagFamily != 0)
            {
                ...
                do
                {
                    checked
                    {
                        if(Items.ContainsKey(bag3) &&
                            (Items[bag3].ItemInfo.SubClass != 0) &&
                            (((Items[bag3].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                                    (Items[bag3].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_FOOD) &&
                                    (Item.ItemInfo.BagFamily == ITEM_BAG.SOUL_SHARD)) ||
                                ((Items[bag3].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                                    (Items[bag3].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                                    (Item.ItemInfo.BagFamily == ITEM_BAG.HERB)) ||
                                ((Items[bag3].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_CONTAINER) &&
                                    (Items[bag3].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
                                    (Item.ItemInfo.BagFamily == ITEM_BAG.ENCHANTING)) ||
                                ((Items[bag3].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_QUIVER) &&
                                    (Items[bag3].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_LIQUID) &&
                                    (Item.ItemInfo.BagFamily == ITEM_BAG.ARROW)) ||
                                ((Items[bag3].ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_QUIVER) &&
                                    (Items[bag3].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_POTION) &&
            ...
            do
            {
                checked
                {
                    if(Items.ContainsKey(bag) &&
                        (Items[bag].ItemInfo.SubClass == ITEM_SUBCLASS.ITEM_SUBCLASS_CONSUMABLE))
                    {
                        var b2 = (byte)(Items[bag].ItemInfo.ContainerSlots - 1);
                        byte slot2 = 0;
                        while(slot2 <= ((uint)b2))
                        {
                            if(!Items[bag].Items.ContainsKey(slot2) &&
                                (ItemCANEQUIP(Item, bag, slot2) == InventoryChangeFailure.EQUIP_ERR_OK))
                            {
                    ...
                    switch(dstSlot)
                    {
                        ...
                        case 18:
                            ...
                            checked
                            {
                                if(!WorldServiceLocator.Functions
                                    .HaveFlag(ItemInfo.AvailableClasses, (byte)(((int)Class) - 1)))
                                {
                                    return InventoryChangeFailure.EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;
                                }
                                if(!WorldServiceLocator.Functions
                                    .HaveFlag(ItemInfo.AvailableRaces, (byte)(((int)Race) - 1)))
                                {
                                ...
                                var tmp = false;
                                foreach(var SlotVal in ItemInfo.GetSlots)
                                {
                                    if(dstSlot == SlotVal)
                                    {
                                        tmp = true;
                                ...
                                if((dstSlot == 15) &&
                                    (ItemInfo.InventoryType == INVENTORY_TYPES.INVTYPE_TWOHAND_WEAPON) &&
                                    Items.ContainsKey(16))
                                {
                                    return InventoryChangeFailure.EQUIP_ERR_CANT_EQUIP_WITH_TWOHANDED;
                                }
                                if((dstSlot == 16) &&
                                    Items.ContainsKey(15) &&
                                    (Items[15].ItemInfo.InventoryType == INVENTORY_TYPES.INVTYPE_TWOHAND_WEAPON))
                                {
                                    return InventoryChangeFailure.EQUIP_ERR_CANT_EQUIP_WITH_TWOHANDED;
                                }
                                if((dstSlot == 16) &&
                                    (ItemInfo.InventoryType == INVENTORY_TYPES.INVTYPE_WEAPON) &&
                                    !Skills.ContainsKey(118))
                                {
                            ...
                            if((ItemInfo.ReqFaction != 0) &&
                                (((int)client.Character.GetReputation(ItemInfo.ReqFaction)) <=
                                    ItemInfo.ReqFactionLevel))
                            {
                        ...
                        case 38:
                            if(ItemInfo.IsContainer)
                            {
                                return Item.IsFree
                                    ? InventoryChangeFailure.EQUIP_ERR_OK
                                    : InventoryChangeFailure.EQUIP_ERR_CAN_ONLY_DO_WITH_EMPTY_BAGS;
                            }
                            return InventoryChangeFailure.EQUIP_ERR_OK;
                        ...
                        case 111:
                        case 112:
                            if((ItemInfo.BagFamily != ITEM_BAG.KEYRING) &&
                                (ItemInfo.ObjectClass != ITEM_CLASS.ITEM_CLASS_KEY))
                            {
                                return InventoryChangeFailure.EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT;
                            }
                            return InventoryChangeFailure.EQUIP_ERR_OK;
            ...
            do
            {
                checked
                {
                    if(Items.ContainsKey(bag))
                    {
                        ...
                        while(slot3 <= ((uint)b))
                        {
                            if(Items[bag].Items.ContainsKey(slot3) &&
                                (Items[bag].Items[slot3].ItemEntry == ItemEntry))
                            {
        ...
        public ItemObject ItemGET(byte srcBag, byte srcSlot)
        {
            ...
            } else if(Items.ContainsKey(srcBag) && ((Items[srcBag].Items?.ContainsKey(srcSlot)) == true))
            {
        ...
        public void ItemREMOVE(ulong itemGuid, bool Destroy, bool Update)
        {
            ...
            do
            {
                checked
                {
                    if(Items.ContainsKey(slot) && (Items[slot].GUID == itemGuid))
                    {
                        WorldServiceLocator.WorldServer.CharacterDatabase
                            .Update(
                                $"UPDATE characters_inventory SET item_slot = {WorldServiceLocator.GlobalConstants.ITEM_SLOT_NULL}, item_bag = {WorldServiceLocator.GlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[slot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                        if(slot < 23u)
                        {
                            if(slot < 19u)
                            {
                                SetUpdateFlag(
                                    260 + (slot * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    0);
                            }
                    ...
                    if(Items.ContainsKey(bag))
                    {
                        ...
                        while(slot2 <= ((uint)b))
                        {
                            if(Items[bag].Items.ContainsKey(slot2) && (Items[bag].Items[slot2].GUID == itemGuid))
                            {
                                WorldServiceLocator.WorldServer.CharacterDatabase
                                    .Update(
                                        $"UPDATE characters_inventory SET item_slot = {WorldServiceLocator.GlobalConstants.ITEM_SLOT_NULL}, item_bag = {WorldServiceLocator.GlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[bag].Items[slot2].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                                if(Destroy)
                                {
            ...
            throw new ApplicationException(
                "Unable to remove item because character doesn't have it in inventory or bags.");
        }
            ...
            checked
            {
                if(srcBag == 0)
                {
                    if(srcSlot < 23u)
                    {
                        ...
                        {
                            SetUpdateFlag(
                                260 + (srcSlot * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                0);
                        }
                        Dictionary<byte, ItemObject> items;
                        byte key;
                    ...
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {WorldServiceLocator.GlobalConstants.ITEM_SLOT_NULL}, item_bag = {WorldServiceLocator.GlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[srcSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    if(Destroy)
                    {
                ...
                } else
                {
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {WorldServiceLocator.GlobalConstants.ITEM_SLOT_NULL}, item_bag = {WorldServiceLocator.GlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[srcBag].Items[srcSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    if(Destroy)
                    {
        ...
        public bool ItemSETSLOT(ref ItemObject Item, byte dstBag, byte dstSlot)
        {
            ...
            checked
            {
                if(dstBag == 0)
                {
                    ...
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {GUID}, item_stackCount = {Item.StackCount} WHERE item_guid = {Item.GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    SetUpdateFlag(486 + (dstSlot * 2), Item.GUID);
                    if(dstSlot < 19u)
                    {
                        SetUpdateFlag(
                            260 + (dstSlot * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                            Item.ItemEntry);
                        SetUpdateFlag(
                            268 + (dstSlot * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                            Item.RandomProperties);
                ...
                } else
                {
                    ...
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {Items[dstBag].GUID}, item_stackCount = {Item.StackCount} WHERE item_guid = {Item.GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                }
                if(client != null)
                {
        ...
        public void ItemSPLIT(byte srcBag, byte srcSlot, byte dstBag, byte dstSlot, int Count)
        {
            ...
            if(srcBag == 0)
            {
                ...
                {
                    Packets.Packets.PacketClass EQUIP_ERR_ITEM_NOT_FOUND = new(
                        Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                    try
                    {
            ...
            } else
            {
                ...
                {
                    Packets.Packets.PacketClass EQUIP_ERR_ITEM_NOT_FOUND2 = new(
                        Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                    try
                    {
            ...
            checked
            {
                ...
                {
                    Packets.Packets.PacketClass notHandledYet = new(Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                    try
                    {
                ...
                {
                    Packets.Packets.PacketClass EQUIP_ERR_TRIED_TO_SPLIT_MORE_THAN_COUNT = new(
                        Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                    try
                    {
                ...
                } else if(dstItem == null)
                {
                    ...
                    ItemObject tmpItem = new(srcItem.ItemEntry, GUID) { StackCount = Count };
                    dstItem = tmpItem;
                    tmpItem.Save();
                    ItemSETSLOT(ref tmpItem, dstBag, dstSlot);
                    Packets.Packets.UpdatePacketClass SMSG_UPDATE_OBJECT = new();
                    Packets.Packets.UpdateClass tmpUpdate = new(
                        WorldServiceLocator.GlobalConstants.FIELD_MASK_SIZE_ITEM);
                    try
                    {
                        tmpItem.FillAllUpdateFlags(ref tmpUpdate);
                        var updateClass = tmpUpdate;
                        Packets.Packets.PacketClass packet = SMSG_UPDATE_OBJECT;
                ...
                } else if((srcItem.ItemEntry == dstItem.ItemEntry) &&
                    ((dstItem.StackCount + Count) <= dstItem.ItemInfo.Stackable))
                {
                    srcItem.StackCount -= Count;
                    ...
                    srcItem.Save(saveAll: false);
                    dstItem.Save(saveAll: false);
                    Packets.Packets.PacketClass EQUIP_ERR_OK = new(Opcodes.SMSG_INVENTORY_CHANGE_FAILURE);
                    try
                    {
        ...
        public bool ItemSTACK(byte srcBag, byte srcSlot, byte dstBag, byte dstSlot)
        {
            ...
            if((srcItem.StackCount == dstItem.ItemInfo.Stackable) ||
            ...
            checked
            {
                if((srcItem.ItemEntry == dstItem.ItemEntry) &&
                    ((dstItem.StackCount + srcItem.StackCount) <= dstItem.ItemInfo.Stackable))
                {
        ...
        public void ItemSWAP(byte srcBag, byte srcSlot, byte dstBag, byte dstSlot)
        {
            if(DEAD)
            {
                ...
                wS_Items.SendInventoryChangeFailure(
                    ref objCharacter,
                    InventoryChangeFailure.EQUIP_ERR_YOU_ARE_DEAD,
                    ItemGetGUID(srcBag, srcSlot),
                    ItemGetGUID(dstBag, dstSlot));
                return;
            }
            byte errCode = 21;
            if(((srcBag == 0) && (srcSlot == dstBag) && (dstBag > 0)) ||
                ((dstBag == 0) && (dstSlot == srcBag) && (srcBag > 0)))
            {
                var wS_Items2 = WorldServiceLocator.WSItems;
                var objCharacter = this;
                wS_Items2.SendInventoryChangeFailure(
                    ref objCharacter,
                    (InventoryChangeFailure)errCode,
                    Items[srcSlot].GUID,
                    0uL);
                return;
            }
            checked
            {
                try
                {
                    unchecked
                    {
                        if((srcBag > 0) && (dstBag > 0))
                        {
                            ...
                            {
                                if(!Items[dstBag].Items.Remove(dstSlot))
                                {
                            ...
                            (Items[srcBag].Items[srcSlot], Items[dstBag].Items[dstSlot]) = (Items[dstBag].Items[
                                dstSlot], Items[srcBag].Items[srcSlot]);
                            goto IL_02e9;
                        }
                        if(srcBag > 0)
                        {
                            ...
                            (Items[srcBag].Items[srcSlot], Items[dstSlot]) = (Items[dstSlot], Items[srcBag].Items[
                                srcSlot]);
                            if(dstSlot < 23u)
                            {
                                Dictionary<byte, ItemObject> items;
                        ...
                        if(dstBag > 0)
                        {
                            ...
                            (Items[srcSlot], Items[dstBag].Items[dstSlot]) = (Items[dstBag].Items[dstSlot], Items[
                                srcSlot]);
                            if(srcSlot < 23u)
                            {
                                Dictionary<byte, ItemObject> items;
                        ...
                        (Items[srcSlot], Items[dstSlot]) = (Items[dstSlot], Items[srcSlot]);
                        if(dstSlot < 23u)
                        {
                            Dictionary<byte, ItemObject> items;
                            byte key;
                    ...
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {GUID} WHERE item_guid = {Items[dstSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    if(Items[srcBag].Items.ContainsKey(srcSlot))
                    {
                        WorldServiceLocator.WorldServer.CharacterDatabase
                            .Update(
                                $"UPDATE characters_inventory SET item_slot = {srcSlot}, item_bag = {Items[srcBag].GUID} WHERE item_guid = {Items[srcBag].Items[srcSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    }
                    goto end_IL_0080;
                    IL_0ab8:
                    ...
                    WorldServiceLocator.WorldServer.CharacterDatabase
                        .Update(
                            $"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {Items[dstBag].GUID} WHERE item_guid = {Items[dstBag].Items[dstSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    if(Items.ContainsKey(srcSlot))
                    {
                        WorldServiceLocator.WorldServer.CharacterDatabase
                            .Update(
                                $"UPDATE characters_inventory SET item_slot = {srcSlot}, item_bag = {GUID} WHERE item_guid = {Items[srcSlot].GUID - WorldServiceLocator.GlobalConstants.GUID_ITEM};");
                    }
                    goto end_IL_0080;
                    IL_0ec5:
                ...
                } finally
                {
                    if(errCode != 0)
                    {
                        ...
                        var objCharacter = this;
                        wS_Items3.SendInventoryChangeFailure(
                            ref objCharacter,
                            (InventoryChangeFailure)errCode,
                            ItemGetGUID(srcBag, srcSlot),
                            ItemGetGUID(dstBag, dstSlot));
        ...
        public void LearnSpell(int SpellID)
        {
            ...
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Update(
                    $"INSERT INTO characters_spells (guid, spellid, active, cooldown, cooldownitem) VALUES ({GUID},{SpellID},{1},{0},{0});");
            if(client == null)
            {
                return;
            }
            Packets.Packets.PacketClass SMSG_LEARNED_SPELL = new(Opcodes.SMSG_LEARNED_SPELL);
            try
            {
            ...
            if(WorldServiceLocator.WSSpells.SPELLs[SpellID].IsPassive)
            {
                ...
                if(!HavePassiveAura(SpellID))
                {
                    var spellInfo = WorldServiceLocator.WSSpells.SPELLs[SpellID];
                    var Character = this;
                    num = (spellInfo.CanCast(ref Character, t, FirstCheck: false) ==
            ...
            if(!WorldServiceLocator.WSSpells.SPELLs[SpellID].CanStackSpellRank &&
                WorldServiceLocator.WSSpells.SpellChains.ContainsKey(SpellID) &&
                Spells.ContainsKey(WorldServiceLocator.WSSpells.SpellChains[SpellID]))
            {
                Spells[WorldServiceLocator.WSSpells.SpellChains[SpellID]].Active = 0;
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update($"UPDATE characters_spells SET active = 0 WHERE guid = {GUID} AND spellid = {SpellID};");
                Packets.Packets.PacketClass packet = new(Opcodes.SMSG_SUPERCEDED_SPELL);
                try
                {
            ...
            checked
            {
                var maxSkill = (Level > WorldServiceLocator.WSPlayerInitializator.DEFAULT_MAX_LEVEL)
                    ? (WorldServiceLocator.WSPlayerInitializator.DEFAULT_MAX_LEVEL * 5)
                    : (Level * 5);
                switch(SpellID)
                {
                    case 4036:
        ...
        {
            Packets.Packets.PacketClass SMSG_ENVIRONMENTALDAMAGELOG = new(Opcodes.SMSG_ENVIRONMENTALDAMAGELOG);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_PVP_CREDIT = new(Opcodes.SMSG_PVP_CREDIT);
            try
            {
        ...
        public void Login()
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.DEBUG, "Spawned on wrong map [{0}], transferring to [{1}].", LoginMap, MapID);
            ...
            catch (IndexOutOfRangeException ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.WARNING,
                        "Failed loading Character, Login Coordinates Out Of Range.{0}{1}",
                        Environment.NewLine,
                        ex.ToString());
                WorldServiceLocator.WorldServer.AllGraveYards
                .GoToNearestGraveyard(ref Character, Alive: false, Teleport: true);
            }
            catch (Exception ex)
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(
                        LogType.CRITICAL,
                        "Failed loading maps at character logging in.{0}{1}",
                        Environment.NewLine,
                        ex.ToString());
            }
        ...
        public void LogLootItem(ItemObject Item, byte ItemCount, bool Recieved, bool Created)
        {
            ...
            Packets.Packets.PacketClass response = new(Opcodes.SMSG_ITEM_PUSH_RESULT);
            try
            {
        ...
        public void Logout(object StateObj = null)
        {
            ...
            if(this is CharacterObject _character)
            {
                ...
                if(DuelPartner != null)
                {
                    ...
                    {
                        WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[DuelArbiter].Destroy(
                            WorldServiceLocator.WorldServer.WORLD_GAMEOBJECTs[DuelArbiter]);
                    }
                }
                Packets.Packets.PacketClass SMSG_LOGOUT_COMPLETE = new(Opcodes.SMSG_LOGOUT_COMPLETE);
                try
                {
                    client?.Send(ref SMSG_LOGOUT_COMPLETE);
                    SMSG_LOGOUT_COMPLETE?.Dispose();
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_LOGOUT_COMPLETE", client.IP, client.Port);
                    client.Character = null;
        ...
        {
            Packets.Packets.PacketClass SMSG_LOG_XPGAIN = new(Opcodes.SMSG_LOG_XPGAIN);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_START_MIRROR_TIMER = new(Opcodes.SMSG_START_MIRROR_TIMER);
            try
            {
        ...
        public void PrepareUpdate(ref Packets.Packets.PacketClass packet, int UPDATETYPE = 2)
        {
            if(packet is null)
            {
            ...
            checked
            {
                try
                {
                    ...
                    foreach(var Spell in Spells)
                    {
                        ...
                        if((SpellInfo.School == School) &&
                            ((Spell.Value.CoolDown < curTime) || ((Spell.Value.CoolDown - curTime) < Time)))
                        {
        ...
        public void SaveAsNewCharacter(int Account_ID)
        {
            ...
            tmpCMD += ", char_class";
            tmpValues = $"{tmpValues}, {Conversions.ToString((byte)Class)}";
            tmpCMD += ", char_gender";
            ...
            checked
            {
                ...
                {
                    temp.Add(
                        $"{Conversions.ToString(Reputation_Point.Flags)}:{Conversions.ToString(Reputation_Point.Value)}");
                }
                ...
                {
                    temp.Add(
                        $"{ActionButton.Key}:{ActionButton.Value.Action}:{ActionButton.Value.ActionType}:{ActionButton.Value.ActionMisc}");
                }
                ...
                DataTable MySQLQuery = new();
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Query($"SELECT char_guid FROM characters WHERE char_name = '{Name}';", ref MySQLQuery);
        ...
        public void SaveCharacter()
        {
            ...
            tmp = $"{tmp}, char_class={Conversions.ToString((byte)Class)}";
            ...
            checked
            {
                ...
                for(var i = 0; i <= num; i++)
                {
                    if((ActiveSpells[i] != null) &&
                        ((ActiveSpells[i].SpellDuration ==
                            ...
                            (ActiveSpells[i].SpellDuration > 10000)))
                    {
                        var expire = 0L;
                        if(ActiveSpells[i].SpellDuration !=
                            WorldServiceLocator.GlobalConstants.SPELL_DURATION_INFINITE)
                        {
                            expire = WorldServiceLocator.Functions.GetTimestamp(DateAndTime.Now) +
                ...
                temp.AddRange(TmpArray);
                tmp = $"{tmp}, char_taxiFlags=\"{Strings.Join(temp.ToArray())}\"";
                temp.Clear();
                temp.AddRange(ZonesExplored);
        ...
        public void SendCharacterUpdate(bool toNear = true, bool notMe = false)
        {
            ...
            {
                Packets.Packets.UpdateClass forOthers = new(WorldServiceLocator.GlobalConstants.FIELD_MASK_SIZE_PLAYER);
                {
                    UpdateData = (Hashtable)UpdateData.Clone();
                    UpdateMask = (BitArray)UpdateMask.Clone();
                }
                Packets.Packets.PacketClass packetForOthers = new(Opcodes.SMSG_UPDATE_OBJECT);
                try
                {
            ...
            bool SendToMe = false)
        {
            ...
            var packet = WorldServiceLocator.Functions
                .BuildChatMessage(
                    ...
                    WorldServiceLocator.WSHandlersChat.GetChatFlag(Sender),
                    ChannelName);
        ...
        {
            Packets.Packets.PacketClass p = new(Opcodes.CMSG_REPOP_REQUEST);
            try
            {
                p.AddInt32(MapID);
                p.AddSingle(x);
        ...
        {
            Packets.Packets.PacketClass SMSG_GOSSIP_MESSAGE = new(Opcodes.SMSG_GOSSIP_MESSAGE);
            checked
            {
                try
                {
        ...
        {
            Packets.Packets.PacketClass SMSG_GOSSIP_COMPLETE = new(Opcodes.SMSG_GOSSIP_COMPLETE);
            try
            {
                ...
                do
                {
                    checked
                    {
                        if(Items.ContainsKey(i))
                        {
                            ...
                            {
                                SetUpdateFlag(
                                    260 + (i * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Items[i].ItemEntry);
                                SetUpdateFlag(
                                    268 + (i * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    0);
                            }
                        } else
                        {
        ...
        public void SendItemAndCharacterUpdate(ItemObject Item, int UPDATETYPE = 0)
        {
            ...
            Packets.Packets.PacketClass packet = new(Opcodes.SMSG_UPDATE_OBJECT);
            Packets.Packets.UpdateClass tmpUpdate = new(WorldServiceLocator.GlobalConstants.FIELD_MASK_SIZE_ITEM);
            try
            {
                packet.AddInt32(2);
                packet.AddInt8(0);
                ...
                do
                {
                    checked
                    {
                        if(Items.ContainsKey(j))
                        {
                            ...
                            {
                                SetUpdateFlag(
                                    260 + (j * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Items[j].ItemEntry);
                                SetUpdateFlag(
                                    268 + (j * WorldServiceLocator.GlobalConstants.PLAYER_VISIBLE_ITEM_SIZE),
                                    Items[j].RandomProperties);
                            }
                        } else
                        {
        ...
        public void SendPointOfInterest(float x, float y, int icon, int flags, int data, string name)
        {
            ...
            Packets.Packets.PacketClass SMSG_GOSSIP_POI = new(Opcodes.SMSG_GOSSIP_POI);
            try
            {
        ...
        public void SendTalking(int TextID)
        {
            ...
            Packets.Packets.PacketClass response = new(Opcodes.SMSG_NPC_TEXT_UPDATE);
            try
            {
                ...
                } else
                {
                    ...
                    do
                    {
                        ...
                        if(Operators.CompareString(
                                WorldServiceLocator.WSCreatures.NPCTexts[TextID].TextLine2[i],
                                string.Empty,
                                TextCompare: false) ==
                            0)
                        {
        ...
        {
            Packets.Packets.PacketClass SMSG_MOVE_SET_HOVER = new(Opcodes.SMSG_MOVE_SET_HOVER);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_MOVE_LAND_WALK = new(Opcodes.SMSG_MOVE_LAND_WALK);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_FORCE_MOVE_ROOT = new(Opcodes.SMSG_FORCE_MOVE_ROOT);
            try
            {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_FORCE_MOVE_ROOT", client.IP, client.Port);
        }

        public void SetMoveUnroot()
        {
            Packets.Packets.PacketClass SMSG_FORCE_MOVE_UNROOT = new(Opcodes.SMSG_FORCE_MOVE_UNROOT);
            try
            {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_FORCE_MOVE_UNROOT", client.IP, client.Port);
        }
            ...
            checked
            {
                if(decimal.Compare(new decimal(TransportGUID), 0m) > 0)
                {
                    ...
                    } else if(WorldServiceLocator.CommonGlobalFunctions.GuidIsTransport(TransportGUID))
                    {
                        ...
                        {
                            WorldServiceLocator.WorldServer.Log
                                .WriteLine(LogType.CRITICAL, "Spawning new transport!");
                    ...
                    } else
                    {
                        WorldServiceLocator.WorldServer.Log
                            .WriteLine(
                                LogType.CRITICAL,
                                "Character logging in on non-existant transport [{0}].",
                                TransportGUID - WorldServiceLocator.GlobalConstants.GUID_MO_TRANSPORT);
                        var allGraveYards = WorldServiceLocator.WorldServer.AllGraveYards;
                        var Character = this;
        ...
        public void SetReputation(int FactionID, int Value)
        {
            ...
            checked
            {
                ...
                if(client != null)
                {
                    Packets.Packets.PacketClass packet = new(Opcodes.SMSG_SET_FACTION_STANDING);
                    try
                    {
                        packet.AddInt32(
                            Reputation[WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].VisibleID].Flags);
                        packet.AddInt32(WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].VisibleID);
                        packet.AddInt32(
                            Reputation[WorldServiceLocator.WSDBCDatabase.FactionInfo[FactionID].VisibleID].Value);
                        client.Send(ref packet);
                    } finally
                    {
                        packet.Dispose();
        ...
        {
            Packets.Packets.PacketClass SMSG_MOVE_WATER_WALK = new(Opcodes.SMSG_MOVE_WATER_WALK);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_SHOW_BANK = new(Opcodes.SMSG_SHOW_BANK);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_SPLINE_MOVE_START_SWIM = new(Opcodes.SMSG_SPLINE_MOVE_START_SWIM);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_SPLINE_MOVE_STOP_SWIM = new(Opcodes.SMSG_SPLINE_MOVE_STOP_SWIM);
            try
            {
        ...
        {
            Packets.Packets.PacketClass SMSG_STOP_MIRROR_TIMER = new(Opcodes.SMSG_STOP_MIRROR_TIMER);
            try
            {
            ...
            checked
            {
                do
                {
                    if(TalkQuests[i] == null)
                    {
                        ...
                        WorldServiceLocator.WorldServer.CharacterDatabase
                            .Update(
                                $"INSERT INTO characters_quests (char_guid, quest_id, quest_status) VALUES ({GUID}, {TalkQuests[i].ID}, {questState});");
        ...
        public bool TalkCanAccept(ref WS_QuestInfo Quest)
        {
            ...
            WorldServiceLocator.WorldServer.CharacterDatabase
                .Query(
                    $"SELECT quest_status FROM characters_quests WHERE char_guid = {GUID} AND quest_id = {Quest.ID} LIMIT 1;",
                    ref DBResult);
            if(DBResult.Rows.Count > 0)
            {
                ...
                if(status == (-1))
                {
                    Packets.Packets.PacketClass packet = new(Opcodes.SMSG_QUESTGIVER_QUEST_INVALID);
                    try
                    {
                        packet.AddInt32(7);
                        client.Send(ref packet);
                ...
                {
                    Packets.Packets.PacketClass packet2 = new(Opcodes.SMSG_QUESTGIVER_QUEST_INVALID);
                    try
                    {
                        packet2.AddInt32(13);
            ...
            checked
            {
                ...
                {
                    Packets.Packets.PacketClass packet4 = new(Opcodes.SMSG_QUESTGIVER_QUEST_INVALID);
                    try
                    {
                        packet4.AddInt32(6);
                ...
                if((Quest.RequiredClass != 0) && ((Quest.RequiredClass & (1 << (((int)Class) - 1))) == 0))
                {
                    Packets.Packets.PacketClass packet5 = new(Opcodes.SMSG_QUESTGIVER_QUEST_INVALID);
                    try
                    {
                        packet5.AddInt32(0);
                ...
                {
                    Packets.Packets.PacketClass packet3 = new(Opcodes.SMSG_QUESTGIVER_QUEST_INVALID);
                    try
                    {
                        packet3.AddInt32(0);
        ...
        public bool TalkCompleteQuest(byte QuestSlot)
        {
            ...
            checked
            {
                ...
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update(
                        $"UPDATE characters_quests SET quest_status = -1 WHERE char_guid = {GUID} AND quest_id = {TalkQuests[QuestSlot].ID};");
        ...
        public bool TalkDeleteQuest(byte QuestSlot)
        {
            ...
            checked
            {
                ...
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update(
                        $"DELETE  FROM characters_quests WHERE char_guid = {GUID} AND quest_id = {TalkQuests[QuestSlot].ID};");
                TalkQuests[QuestSlot] = null;
                SendCharacterUpdate(updateDataCount != 0);
                return true;
            }
        ...
        public bool TalkUpdateQuest(byte QuestSlot)
        {
            ...
            checked
            {
                ...
                {
                    var tmpTimer = (int)(TalkQuests[QuestSlot].TimeEnd -
                        WorldServiceLocator.Functions.GetTimestamp(DateAndTime.Now));
                }
                WorldServiceLocator.WorldServer.CharacterDatabase
                    .Update(
                        $"UPDATE characters_quests SET quest_status = {tmpProgress} WHERE char_guid = {GUID} AND quest_id = {TalkQuests[QuestSlot].ID};");
        ...
        public void Teleport(float posX, float posY, float posZ, float posO, int map)
        {
            ...
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.INFORMATION,
                    "World: Player Teleport: X[{0}], Y[{1}], Z[{2}], O[{3}]",
                    posX,
                    posY,
                    posZ,
                    posO);
            charMovementFlags = 0;
            Packets.Packets.PacketClass packet = new(Opcodes.MSG_MOVE_TELEPORT_ACK);
            try
            {
                packet.AddPackGUID(GUID);
                packet.AddInt32(0);
            ...
            client.Character.ZoneID = WorldServiceLocator.WSMaps.AreaTable[
                WorldServiceLocator.WSMaps.GetAreaFlag(posX, posY, checked((int)client.Character.MapID))].Zone;
        }
        ...
        public void Transfer(float posX, float posY, float posZ, float ori, int map)
        {
            WorldServiceLocator.WorldServer.Log
                .WriteLine(
                    LogType.INFORMATION,
                    "World: Player Transfer: X[{0}], Y[{1}], Z[{2}], O[{3}], MAP[{4}]",
                    posX,
                    posY,
                    posZ,
                    ori,
                    map);
            Packets.Packets.PacketClass p = new(Opcodes.SMSG_TRANSFER_PENDING);
            checked
            {
                try
                {
                ...
                WorldServiceLocator.WorldServer.ClsWorldServer
                    .ClientTransfer(client.Index, posX, posY, posZ, ori, map);
            }
        }

        public void UnLearnSpell(int SpellID) { Spells.Remove(SpellID); }
            ...
            checked
            {
                ...
                do
                {
                    if((Item.ItemInfo.Spells[i].SpellID > 0) &&
                        WorldServiceLocator.WSSpells.SPELLs.ContainsKey(Item.ItemInfo.Spells[i].SpellID))
                    {
                        ...
                        {
                            Packets.Packets.PacketClass cooldown = new(Opcodes.SMSG_ITEM_COOLDOWN);
                            try
                            {
                ...
                if((ManaType == ManaTypes.TYPE_MANA) || (Class == Classes.CLASS_DRUID))
                {
                    UpdateManaRegen();
                }
        ...
        public void UpdateAuraDurations()
        {
            checked
            {
                ...
                for(var i = 0; i <= num; i++)
                {
                    if(ActiveSpells[i] != null)
                    {
                        Packets.Packets.PacketClass SMSG_UPDATE_AURA_DURATION = new(
                            Opcodes.SMSG_UPDATE_AURA_DURATION);
                        try
                        {
                ...
                for(var i = 0; i <= num; i++)
                {
                    ...
                    do
                    {
                        if(ActiveSpells[i].Aura_Info[j] != null)
                        {
                            ...
                            {
                                PowerRegenMP5 += GetStat((byte)ActiveSpells[i].Aura_Info[j].MiscValue) *
                                        ActiveSpells[i].Aura_Info[j].GetValue(Level, 0) /
                                    500f;
                            } else if((ActiveSpells[i].SpellID == 34074) &&
                                (ActiveSpells[i].Aura_Info[j].ApplyAuraIndex == 226))
                            {
                                PowerRegenMP5 = (float)(PowerRegenMP5 +
                                    ((ActiveSpells[i].Aura_Info[j].GetValue(Level, 0) * Intellect.Base / 500f) +
                                        (Level * 35 / 100.0)));
            ...
            checked
            {
                ...
                if(Item.ItemInfo.Delay > 0)
                {
                    switch(slot)
                    {
                        ...
                        case 15:
                            AttackTimeBase[0] = (short)((Class == Classes.CLASS_ROGUE) ? 1900 : 2000);
                            break;
        ...
        {
            if((SkillID != 0) &&
                (Skills[SkillID].Current < Skills[SkillID].Maximum) &&
                (((Skills[SkillID].Current / ((double)Skills[SkillID].Maximum)) - SpeedMod) <
                    WorldServiceLocator.WorldServer.Rnd.NextDouble()))
            {
        ...
        public void ZoneCheck()
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.WARNING, "Zone Flag {0} does not exist.", ZoneFlag);
                return;
            }
            ...
            {
                if(((cPlayerFlags & PlayerFlags.PLAYER_FLAGS_RESTING) == 0) &&
                    (Level < WorldServiceLocator.WSPlayerInitializator.DEFAULT_MAX_LEVEL))
                {
                ...
                checked
                {
                    switch(Class)
                    {
                        ...
                        case Classes.CLASS_DRUID:
                            ...
                            if(ShapeshiftForm is ShapeshiftForm.FORM_BEAR or ShapeshiftForm.FORM_DIREBEAR)
                            {
            ...
            get
            {
                checked
                {
                    switch(Class)
                    {
                        ...
                        case Classes.CLASS_DRUID:
                            if(ShapeshiftForm is ShapeshiftForm.FORM_CAT or ShapeshiftForm.FORM_BEAR or ShapeshiftForm.FORM_DIREBEAR or ShapeshiftForm.FORM_MOONKIN)
                            {
                                return 0;
                            }
                            return Agility.Base - 10;
        ...
        public int BaseRangedDamage => checked((int)Math.Round(
            (AttackPowerRanged + AttackPowerModsRanged) * 0.071428571428571425));

        public int BaseUnarmedDamage => checked((int)Math.Round(
            (AttackPower + AttackPowerMods) * 0.071428571428571425));

        public bool CanShootRanged => (AmmoID > 0) &&
            Items.ContainsKey(17) &&
            Items[17].IsRanged &&
            !Items[17].IsBroken() &&
            (ItemCOUNT(AmmoID) > 0);
        ...
        public float GetCriticalWithSpells => Class switch
        {
        ...
        public bool GM
        {
            ...
            set
            {
                if(value && (WorldServiceLocator.WorldServer.ClsWorldServer.Cluster != null))
                {
        ...
        public bool IsInCombat => (inCombatWith.Count > 0) ||
            (checked(WorldServiceLocator.NativeMethods.timeGetTime(string.Empty) - lastPvpAction) <
                WorldServiceLocator.GlobalConstants.DEFAULT_PVP_COMBAT_TIME);
        ...
        public bool IsMovingOrTurning => (WorldServiceLocator.GlobalConstants.movementOrTurningFlagsMask &
                charMovementFlags) !=
            0;
