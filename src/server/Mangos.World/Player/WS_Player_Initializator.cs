...
public class WS_Player_Initializator
{
    ...
    {
        return checked(((int)Math.Round((a3 * level * level * level) + (a2 * level * level) + (a1 * level) + a0)) -
            ((int)Math.Round(
                (a3 * (level - 1) * (level - 1) * (level - 1)) +
                    (a2 * (level - 1) * (level - 1)) +
                    (a1 * (level - 1)) +
                    a0)));
    }
        ...
        checked
        {
            switch(objCharacter.Class)
            {
                case Classes.CLASS_DRUID:
                    ...
                    objCharacter.Strength.Base += gainStat(
                        objCharacter.Level,
                        2.1E-05,
                        0.003009,
                        0.486493,
                        -0.400003);
                    objCharacter.Intellect.Base += gainStat(
                        objCharacter.Level,
                        3.8E-05,
                        0.005145,
                        0.871006,
                ...
                case Classes.CLASS_PALADIN:
                    ...
                    objCharacter.Stamina.Base += gainStat(
                        objCharacter.Level,
                        3.8E-05,
                        0.005145,
                        0.871006,
            ...
            objCharacter.Damage.Minimum++;
            objCharacter.RangedDamage.Minimum++;
            objCharacter.Damage.Maximum++;
            objCharacter.RangedDamage.Maximum++;
    ...
    public void InitializeReputations(ref WS_PlayerData.CharacterObject objCharacter)
    {
        ...
        do
        {
            objCharacter.Reputation[i] = new WS_PlayerHelper.TReputation { Value = 0, Flags = 0 };
            checked
            {
                foreach(var tmpFactionInfo in WorldServiceLocator.WSDBCDatabase.FactionInfo)
                {
                    ...
                    do
                    {
                        if(WorldServiceLocator.Functions
                            .HaveFlag((uint)tmpFactionInfo.Value.flags[j], (byte)(((int)objCharacter.Race) - 1)))
                        {
