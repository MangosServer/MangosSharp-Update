...
public class WS_PlayerHelper
{
    public void InitializeTalentSpells(WS_PlayerData.CharacterObject objCharacter)
    {
        ...
        {
            if(WorldServiceLocator.WSSpells.SPELLs.ContainsKey(Spell.Key) &&
                WorldServiceLocator.WSSpells.SPELLs[Spell.Key].IsPassive)
            {
                if(!objCharacter.HavePassiveAura(Spell.Key) &&
                    (WorldServiceLocator.WSSpells.SPELLs[Spell.Key].CanCast(ref objCharacter, t, FirstCheck: false) ==
                ...
                } else if(objCharacter.HavePassiveAura(Spell.Key) &&
                    (WorldServiceLocator.WSSpells.SPELLs[Spell.Key].CanCast(ref objCharacter, t, FirstCheck: false) !=
                        SpellFailedReason.SPELL_NO_ERROR))
                {
        ...
        ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass SMSG_SET_REST_START = new(Opcodes.SMSG_SET_REST_START);
        try
        {
    ...
    public void SendActionButtons(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_ACTION_BUTTONS);
        try
        {
            byte i = 0;
            do
    ...
    public void SendFactions(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_INITIALIZE_FACTIONS);
        try
        {
            packet.AddInt32(64);
            byte i = 0;
    ...
    public void SendInitialSpells(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_INITIAL_SPELLS);
        checked
        {
            try
            {
                ...
                foreach(var Cooldown in spellCooldowns)
                {
                    if(WorldServiceLocator.WSSpells.SPELLs.ContainsKey(Cooldown.Key))
                    {
                        ...
                        {
                            timeLeft = (int)(checked(Cooldown.Value.Key -
    ...
    public void SendInitWorldStates(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass packet = new(Opcodes.SMSG_INIT_WORLD_STATES);
        try
        {
    ...
    public void SendTutorialFlags(ref WS_Network.ClientClass client, ref WS_PlayerData.CharacterObject Character)
    {
        ...
        Packets.Packets.PacketClass SMSG_TUTORIAL_FLAGS = new(Opcodes.SMSG_TUTORIAL_FLAGS);
        try
        {
    ...
    public class TSkill
    {
        ...
        public void Increment(short Incrementator = 1)
        {
            checked
            {
                Current = (((short)unchecked(Current + Incrementator)) < Base)
                    ? ((short)unchecked(Current + Incrementator))
                    : Base;
            }
        }
    ...
    public class THonor
    {
        ...
        public void Save()
        {
            ...
            tmp = $"{tmp}, honor_hightestRank ={Conversions.ToString(HonorHighestRank)}";
