...
public sealed class NativeMethods
{
    ...
    public static int CloseHandle(IntPtr hObject, string dummy) { return 0 - (CloseHandle(hObject) ? 1 : 0); }
    ...
    public static int GetCurrentProcess(string dummy) { return GetCurrentProcess(); }

    public static UIntPtr GetProcAddress(IntPtr hModule, string lpProcName, string dummy)
    { return GetProcAddress(hModule, lpProcName); }

    public static int GlobalLock(int hMem, string dummy) { return GlobalLock(hMem); }

    public static int GlobalUnlock(int hMem, string dummy) { return GlobalUnlock(hMem); }

    public static int LoadLibrary(string lpLibFileName, string dummy) { return LoadLibrary(ref lpLibFileName); }

    public static int VirtualFree(int lpAddress, int dwSize, int dwFreeType, string dummy)
    { return VirtualFree(lpAddress, dwSize, dwFreeType); }

    public static int VirtualProtect(
        int lpAddress,
        int dwSize,
        int flNewProtect,
