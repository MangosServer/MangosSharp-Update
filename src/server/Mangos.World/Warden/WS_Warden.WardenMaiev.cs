...
[UnmanagedFunctionPointer(CallingConvention.ThisCall)]
        ...
        private bool InitModule()
        {
            checked
            {
                ...
                Console.WriteLine($"Initialize Function is mapped at 0x{InitPointer:X}");
                ...
                Console.WriteLine($"  SendPacket: 0x{myFunctionList.fpSendPacket:X}");
                Console.WriteLine($"  CheckModule: 0x{myFunctionList.fpCheckModule:X}");
                Console.WriteLine($"  LoadModule: 0x{myFunctionList.fpLoadModule:X}");
                Console.WriteLine($"  AllocateMemory: 0x{myFunctionList.fpAllocateMemory:X}");
                Console.WriteLine($"  ReleaseMemory: 0x{myFunctionList.fpReleaseMemory:X}");
                Console.WriteLine($"  SetRC4Data: 0x{myFunctionList.fpSetRC4Data:X}");
                Console.WriteLine($"  GetRC4Data: 0x{myFunctionList.fpGetRC4Data:X}");
                ...
                try
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.SUCCESS, "[WARDEN] Successfully Initialized Module.");
                    init = (InitializeModule)Marshal.GetDelegateForFunctionPointer(
                        new IntPtr(InitPointer),
                ...
                } catch(Exception ex2)
                {
                    ...
                    var ex = ex2;
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.CRITICAL, "[WARDEN] Failed to Initialize Module.");
                ...
                Console.WriteLine($"  GenerateRC4Keys: 0x{myWardenList.fpGenerateRC4Keys:X}");
                Console.WriteLine($"  Unload: 0x{myWardenList.fpUnload:X}");
                Console.WriteLine($"  PacketHandler: 0x{myWardenList.fpPacketHandler:X}");
                Console.WriteLine($"  Tick: 0x{myWardenList.fpTick:X}");
                GenerateRC4Keys = (GenerateRC4KeysDelegate)Marshal.GetDelegateForFunctionPointer(
                    new IntPtr(myWardenList.fpGenerateRC4Keys),
                    typeof(GenerateRC4KeysDelegate));
                UnloadModule = (UnloadModuleDelegate)Marshal.GetDelegateForFunctionPointer(
                    new IntPtr(myWardenList.fpUnload),
                    typeof(UnloadModuleDelegate));
                PacketHandler = (PacketHandlerDelegate)Marshal.GetDelegateForFunctionPointer(
                    new IntPtr(myWardenList.fpPacketHandler),
                    typeof(PacketHandlerDelegate));
                Tick = (TickDelegate)Marshal.GetDelegateForFunctionPointer(
                    new IntPtr(myWardenList.fpTick),
                    typeof(TickDelegate));
                return true;
            }
        }
        ...
        {
            Console.WriteLine($"Warden.ModuleLoad() Key={ptrRC4Key}, Mod={pModule}, size={dwModSize}");
            return 1;
        }
            ...
            checked
            {
                        ...
                        if(m_Mod != 0)
                        {
                            ...
                            while(dwRelocationIndex < Header.dwRelocationCount)
                            {
                                ...
                                if(dwValue < 0)
                                {
                                    ...
                                    dwValue = (dwValue +
                                            Marshal.ReadByte(new IntPtr(checked(pbRelocationTable + 1)))) <<
                                        8;
                                    dwValue = (dwValue +
                                            Marshal.ReadByte(new IntPtr(checked(pbRelocationTable + 2)))) <<
                                        8;
                            ...
                            WorldServiceLocator.WorldServer.Log
                                .WriteLine(LogType.DEBUG, "[WARDEN] Update: Updating API library references...");
                            var dwLibraryIndex = 0;
                            while(dwLibraryIndex < Header.dwLibraryCount)
                            {
                                var obj2 = Marshal.PtrToStructure(
                                    new IntPtr(m_Mod + Header.dwLibraryTable + (dwLibraryIndex * 8)),
                                    typeof(CLibraryEntry));
                                ...
                                WorldServiceLocator.WorldServer.Log
                                    .WriteLine(LogType.DEBUG, "    Library: {0}", procLib);
                                var hModule = NativeMethods.LoadLibrary(procLib, string.Empty);
                                if(hModule != 0)
                                {
                                    ...
                                    while(dwCurrent != 0)
                                    {
                                        ...
                                        if(dwCurrent <= 0)
                                        {
                                            ...
                                            procAddr = (int)(uint)NativeMethods.GetProcAddress(
                                                (IntPtr)hModule,
                                                Convert.ToString(new IntPtr(dwCurrent)),
                                                string.Empty);
                                            WorldServiceLocator.WorldServer.Log
                                                .WriteLine(LogType.DEBUG, "        Ordinary: 0x{0:X8}", dwCurrent);
                                        } else
                                        {
                                            ...
                                            } else
                                            {
                                                delegateCache.Add(
                                                    procFunc,
                                                    Delegate.CreateDelegate(procDelegate, procRedirector));
                                                procAddr = (int)Marshal.GetFunctionPointerForDelegate(
                                                    delegateCache[procFunc]);
                                                WorldServiceLocator.WorldServer.Log
                                                    .WriteLine(
                                                        LogType.DEBUG,
                                                        "        Function: {0} @ MY 0x{1:X8}",
                                                        procFunc,
                                                        procAddr);
                                            }
                            ...
                            for(var dwIndex = 0; dwIndex < Header.dwChunkCount; dwIndex++)
                            {
                                ...
                                if((unchecked((uint)flNewProtect) & 0xF0u) != 0)
                                {
                                    NativeMethods.FlushInstructionCache(
                                        NativeMethods.GetCurrentProcess(string.Empty),
                                        lpAddress,
                                        dwSize,
                                        string.Empty);
                                }
                            ...
                            if(Header.dwSizeOfCode < dwModuleSize)
                            {
                                ...
                                {
                                    NativeMethods.VirtualFree(
                                        m_Mod + dwOffset,
                                        dwModuleSize - dwOffset,
                                        16384,
                                        string.Empty);
                                }
                                bUnload = false;
                ...
                } catch(Exception ex2)
                {
                    ...
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(
                            LogType.CRITICAL,
                            "Failed to prepair module.{0}{1}",
                            Environment.NewLine,
                            ex.ToString());
                    const bool PrepairModule = false;
                    ProjectData.ClearProjectError();
        ...
        private void SendPacket(int ptrPacket, int dwSize)
        {
            if(dwSize is >= 1 and <= 5000)
            {
                ...
                Console.WriteLine($"Warden.SendPacket() ptrPacket={ptrPacket}, size={dwSize}");
            }
        }

        private int SetRC4Data(int lpKeys, int dwSize)
        {
            Console.WriteLine($"Warden.SetRC4Data() Keys={lpKeys}, Size={dwSize}");
            return 1;
        }

        private void Unload_Module() { WorldServiceLocator.WSWarden.Free(m_Mod); }
        ...
        {
            BigInteger power = new(new byte[4] { 1, 0, 1, 0 }, isUnsigned: true, isBigEndian: true);
            BigInteger pmod = new(
                new byte[256]
                {
                    107,
        ...
        public void InitWarden()
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.SUCCESS, "[WARDEN] Load of module, success [{0}]", ModuleName);
            } else
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.CRITICAL, "[WARDEN] Failed to load module [{0}]", ModuleName);
            }
        }
        ...
        public bool LoadModule(string Name, ref byte[] Data, byte[] Key)
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.CRITICAL, "[WARDEN] Failed to decrypt {0}, incorrect length.", Name);
                return false;
            }
            checked
            {
                ...
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.CRITICAL, "[WARDEN] Failed to decrypt {0}, sign missing.", Name);
                    return false;
                }
                dataPos += 4;
                ...
                {
                    WorldServiceLocator.WorldServer.Log
                        .WriteLine(LogType.CRITICAL, "[WARDEN] Signature fail on Warden Module.");
                    return false;
                }
                ...
                WorldServiceLocator.WorldServer.Log
                    .WriteLine(LogType.SUCCESS, "[WARDEN] Successfully prepaired Warden Module.");
                try
                {
                    if(!InitModule())
                    {
        ...
        public byte[] ReadPacket() { return m_PKT; }
