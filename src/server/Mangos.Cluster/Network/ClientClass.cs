...
public class ClientClass : ClientInfo
{
    ...
    public ClientClass(ClusterServiceLocator clusterServiceLocator, MangosConfiguration mangosConfiguration)
    {
    ...
    public void EnQueue(object state)
    {
        while(_clusterServiceLocator.WorldCluster.CharacteRs.Count > mangosConfiguration.Cluster.ServerPlayerLimit)
        {
            ...
            new PacketClass(Opcodes.SMSG_AUTH_RESPONSE).AddInt32(
                _clusterServiceLocator.WorldCluster.ClienTs.Count -
                    _clusterServiceLocator.WorldCluster.CharacteRs.Count);            // amount of players in queue
            Send(new PacketClass(Opcodes.SMSG_AUTH_RESPONSE));
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.INFORMATION, "[{1}:{2}] AUTH_WAIT_QUEUE: Server player limit reached!", IP, Port);
            Thread.Sleep(6000);
        }
    ...
    { return new() { Access = Access, Account = Account, Index = Index, IP = IP, Port = Port }; }
    ...
    public Task OnConnectAsync()
    {
        ...
        {
            if(DateTime.Now >
                _clusterServiceLocator.WcNetwork.LastConnections[_clusterServiceLocator.WcNetwork.Ip2Int(IP)])
            {
                _clusterServiceLocator.WcNetwork.LastConnections[_clusterServiceLocator.WcNetwork.Ip2Int(IP)] = DateTime.Now
                    .AddSeconds(5d);
            } else
            {
                Socket?.Close();
        ...
        {
            _clusterServiceLocator.WcNetwork.LastConnections
                .Add(_clusterServiceLocator.WcNetwork.Ip2Int(IP), DateTime.Now.AddSeconds(5d));
        }

        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "Incoming connection from [{0}:{1}]", IP, Port);
    ...
    public void OnPacket(PacketClass packet)
    {
        if(Socket is null)
        {
        ...
        if (!_clusterServiceLocator.WorldCluster.GetPacketHandlers().ContainsKey(packet.OpCode))
        {
            if((Character?.IsInWorld) != true)
            {
                Socket?.Dispose();
                Socket?.Close();
            ...
            } else
            {
                try
                {
                    Character.GetWorld?.ClientPacket(Index, packet.Data);
                } catch
                {
                    _clusterServiceLocator.WcNetwork.WorldServer
                        .Disconnect("NULL", new List<uint> { Character.Map });
                }
            }
        } else
        {
            try
            {
                _clusterServiceLocator.WorldCluster.GetPacketHandlers()[packet.OpCode]?.Invoke(packet, client);
            } catch(Exception e)
            {
                _clusterServiceLocator.WorldCluster.Log
                    .WriteLine(
                        LogType.FAILED,
                        "Opcode handler {2}:{2:X} caused an error: {1}{0}",
                        e.ToString(),
                        Environment.NewLine,
                        packet.OpCode);
            }
    ...
    public void Send(byte[] data)
    {
        ...
        } catch(Exception err)
        {
            ...
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(
                    LogType.CRITICAL,
                    "Connection from [{0}:{1}] caused an error {2}{3}",
                    IP,
                    Port,
                    err.ToString(),
                    Environment.NewLine);
