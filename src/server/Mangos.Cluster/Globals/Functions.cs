...
public class Functions
{
    ...
    public void Ban_Account(string name, string reason)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetAccountDatabase()
            .Query($"SELECT id, username FROM account WHERE username = {name};", ref account);
        if(account.Rows.Count > 0)
        {
            var accId = account.Rows[0].As<int>("id");
            _clusterServiceLocator.WorldCluster
                .GetAccountDatabase()
                .Query($"SELECT id, active FROM account_banned WHERE id = {accId};", ref bannedAccount);
            if(bannedAccount.Rows.Count > 0)
            {
                _clusterServiceLocator.WorldCluster
                    .GetAccountDatabase()
                    .Update($"UPDATE account_banned SET active = 1 WHERE id = '{accId}';");
            } else
            {
                ...
                _clusterServiceLocator.WorldCluster
                    .GetAccountDatabase()
                    .Update(
                        $"INSERT INTO `account_banned` VALUES ('{accId}', UNIX_TIMESTAMP('{tempBanDate}'), UNIX_TIMESTAMP('0000-00-00 00:00:00'), '{name}', '{reason}', active = 1);");
            }

            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.INFORMATION, "Account [{0}] banned by server. Reason: [{1}].", name, reason);
        } else
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.INFORMATION, "Account [{0}] NOT Found in Database.", name);
        }
    }
    ...
    public PacketClass BuildChatMessage(
        ulong senderGuid,
        string message,
        ChatMsg msgType,
        LANGUAGES msgLanguage,
        byte flag = 0,
        string msgChannel = "Global")
    {
            ...
            switch(msgType)
            {
                ...
                case var case20 when case20 == ChatMsg.CHAT_MSG_MONSTER_YELL:
                    _clusterServiceLocator.WorldCluster.Log
                        .WriteLine(
                            LogType.WARNING,
                            "Use Creature.SendChatMessage() for this message type - {0}!",
                            msgType);
                    break;

                default:
                    _clusterServiceLocator.WorldCluster.Log
                        .WriteLine(LogType.WARNING, "Unknown chat message type - {0}!", msgType);
                    break;
            }
        ...
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.FAILED, "failed chat message type - {0}!", msgType);
        }
    ...
    public string CapitalizeName(string name)
    {
        return (name.Length > 1)
            ? Strings.UCase(Strings.Left(name, 1)) + Strings.LCase(Strings.Right(name, name.Length - 1))
            : Strings.UCase(name);
    }
    ...
    public string EscapeString(string s) { return s.Replace("\"", string.Empty).Replace("'", string.Empty); }
    ...
    {
        return (Classes)classe switch
        {
            var @case when @case == Classes.CLASS_DRUID => "Druid",
            var case1 when case1 == Classes.CLASS_HUNTER => "Hunter",
            var case2 when case2 == Classes.CLASS_MAGE => "Mage",
            var case3 when case3 == Classes.CLASS_PALADIN => "Paladin",
            var case4 when case4 == Classes.CLASS_PRIEST => "Priest",
            var case5 when case5 == Classes.CLASS_ROGUE => "Rogue",
            var case6 when case6 == Classes.CLASS_SHAMAN => "Shaman",
            var case7 when case7 == Classes.CLASS_WARLOCK => "Warlock",
            var case8 when case8 == Classes.CLASS_WARRIOR => "Warrior",
            _ => "Unknown Class",
        };
    }
    ...
    public DateTime GetNextDate(int days, int hours = 0)
    {
        ...
        return nextDate.AddHours(hours);
    }
    ...
    public DateTime GetNextDay(DayOfWeek iDay, int hour = 0)
    {
        ...
        return nextFriday.AddHours(hour);
    }
    ...
    public int GetRaceModel(Races race, int gender)
    {
        return race switch
        {
            var @case when @case == Races.RACE_HUMAN => 49 + gender,
            var case1 when case1 == Races.RACE_ORC => 51 + gender,
            var case2 when case2 == Races.RACE_DWARF => 53 + gender,
            var case3 when case3 == Races.RACE_NIGHT_ELF => 55 + gender,
            var case4 when case4 == Races.RACE_UNDEAD => 57 + gender,
            var case5 when case5 == Races.RACE_TAUREN => 59 + gender,
            var case6 when case6 == Races.RACE_GNOME => 1563 + gender,
            var case7 when case7 == Races.RACE_TROLL => 1478 + gender,
            _ => 16358,// PinkPig? Lol
        };
    }
    ...
    {
        return (Races)race switch
        {
            var @case when @case == Races.RACE_DWARF => "Dwarf",
            var case1 when case1 == Races.RACE_GNOME => "Gnome",
            var case2 when case2 == Races.RACE_HUMAN => "Human",
            var case3 when case3 == Races.RACE_NIGHT_ELF => "Night Elf",
            var case4 when case4 == Races.RACE_ORC => "Orc",
            var case5 when case5 == Races.RACE_TAUREN => "Tauren",
            var case6 when case6 == Races.RACE_TROLL => "Troll",
            var case7 when case7 == Races.RACE_UNDEAD => "Undead",
            _ => "Unknown Race",
        };
    }
    ...
    {
        var startDate = DateTime.Parse("1970-01-01");
        var timeSpan = fromDateTime.Subtract(startDate);
    ...
    public bool HaveFlags(int value, int flags) { return (value & flags) == flags; }
    ...
    public void SendAccountMd5(ClientClass client, WcHandlerCharacter.CharacterObject character)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_ACCOUNT_DATA_MD5", client.IP, client.Port);
    }

    public void SendCorpseReclaimDelay(
        ClientClass client,
        WcHandlerCharacter.CharacterObject character,
        int seconds = 30)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_CORPSE_RECLAIM_DELAY [{2}s]", client.IP, client.Port, seconds);
    }
    ...
    public void SendGameTime(ClientClass client, WcHandlerCharacter.CharacterObject character)
    {
        ...
        try
        {
            ...
            smsgLoginSettimespeed.AddInt32(
                minute + (hour << 6) + (dayOfWeek << 11) + (day << 14) + (month << 20) + (year << 24));
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_LOGIN_SETTIMESPEED", client.IP, client.Port);
    }
    ...
    public void SendProficiency(ClientClass client, byte proficiencyType, int proficiencyFlags)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_SET_PROFICIENCY", client.IP, client.Port);
    }
    ...
    public void SendTriggerCinematic(ClientClass client, WcHandlerCharacter.CharacterObject character)
    {
        ...
        try
        {
            ...
            } else
            {
                _clusterServiceLocator.WorldCluster.Log
                    .WriteLine(
                        LogType.WARNING,
                        "[{0}:{1}] SMSG_TRIGGER_CINEMATIC [Error: RACE={2} CLASS={3}]",
                        client.IP,
                        client.Port,
                        character.Race,
                        character.Classe);
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_TRIGGER_CINEMATIC", client.IP, client.Port);
    }
    ...
    public string SetColor(string message, byte red, byte green, byte blue)
    {
        var setColorRet = "|cFF";
        setColorRet = (red < 16)
            ? $"{setColorRet}0{Conversion.Hex(red)}"
            : setColorRet + Conversion.Hex(red);
        setColorRet = (green < 16)
            ? $"{setColorRet}0{Conversion.Hex(green)}"
            : setColorRet + Conversion.Hex(green);
        setColorRet = (blue < 16)
            ? $"{setColorRet}0{Conversion.Hex(blue)}"
    ...
    public int ToInteger(bool value) { return value ? 1 : 0; }
    ...
    public enum PartyMemberStatsFlag : uint
    {
        ...
        GROUP_UPDATE_FULL = GROUP_UPDATE_FLAG_STATUS |
