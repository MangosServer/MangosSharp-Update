...
public class Packets
{
    ...
    public void DumpPacket(byte[] data, [Optional, DefaultParameterValue(null)] ClientClass client)
    {
        ...
        try
        {
            buffer = (client is null)
                ? buffer + $"DEBUG: Packet Dump{Constants.vbCrLf}"
                : buffer + $"[{client.IP}:{client.Port}] DEBUG: Packet Dump - Length={data.Length}{Constants.vbCrLf}";
            if((data.Length % 16) == 0)
            {
...
// #End If
catch(Exception e)
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.FAILED, "Error dumping packet: {0}{1}", Constants.vbCrLf, e.ToString());
        }
    }
    ...
    public void LogPacket(byte[] data, bool server, [Optional, DefaultParameterValue(null)] ClientClass client)
    {
        ...
        try
        {
            ...
            if(client is null)
            {
                buffer += $"{typeStr} Packet: (0x{opcode:X4}) {opcode} PacketSize = {data.Length - startAt}{Constants.vbCrLf}";
            } else
            {
                buffer += $"[{client.IP}:{client.Port}] {typeStr} Packet: (0x{opcode:X4}) {opcode} PacketSize = {data.Length - startAt}{Constants.vbCrLf}";
            }
        ...
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.WARNING, "Log Packet has thrown an Exception!", e);
        }
    }
}
