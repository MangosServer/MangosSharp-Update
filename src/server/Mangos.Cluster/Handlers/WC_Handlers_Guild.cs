...
public class WcHandlersGuild
{
    ...
    public void On_CMSG_GUILD_ACCEPT(PacketClass packet, ClientClass client)
    {
        ...
        {
            throw new ApplicationException("Character accepting guild invitation whithout being invited.");
        }
    ...
    public void On_CMSG_GUILD_ADD_RANK(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_ADD_RANK [{2}]", client.IP, client.Port, newRankName);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
        ...
        if(!_clusterServiceLocator.Functions.ValidateGuildName(newRankName))
        {
            _clusterServiceLocator.WcGuild
        ...
        for(var i = 0; i <= 9; i++)
        {
            if(string.IsNullOrEmpty(client.Character.Guild.Ranks[i]))
            {
                ...
                client.Character.Guild.RankRights[i] = (uint)(GuildRankRights.GR_RIGHT_GCHATLISTEN |
                    GuildRankRights.GR_RIGHT_GCHATSPEAK);
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"UPDATE guilds SET guild_rank{i} = '{newRankName}', guild_rank{i}_Rights = '{client.Character.Guild.RankRights[i]}' WHERE guild_id = {client.Character.Guild.Id};");
    ...
    public void On_CMSG_GUILD_CREATE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_CREATE [{2}]", client.IP, client.Port, guildName);
        if(client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"INSERT INTO guilds (guild_name, guild_leader, guild_cYear, guild_cMonth, guild_cDay) VALUES (\"{guildName}\", {client.Character.Guid}, {DateAndTime.Now.Year - 2006}, {DateAndTime.Now.Month}, {DateAndTime.Now.Day}); SELECT guild_id FROM guilds WHERE guild_name = \"{guildName}\";",
                ref mySqlQuery);
        _clusterServiceLocator.WcGuild
            .AddCharacterToGuild(client.Character, mySqlQuery.Rows[0].As<int>("guild_id"), 0);
    }

    public void On_CMSG_GUILD_DECLINE(PacketClass packet, ClientClass client)
    {
        client.Character.GuildInvited = 0U;
        if(_clusterServiceLocator.WorldCluster.CharacteRs
            .ContainsKey((ulong)Conversions.ToLong(client.Character.GuildInvitedBy)))
        {
            PacketClass response = new(Opcodes.SMSG_GUILD_DECLINE);
            response.AddString(client.Character.Name);
            _clusterServiceLocator.WorldCluster.CharacteRs[
                (ulong)Conversions.ToLong(client.Character.GuildInvitedBy)].Client
                .Send(response);
            response.Dispose();
        }
    }
    ...
    public void On_CMSG_GUILD_DEL_RANK(PacketClass packet, ClientClass client)
    {
        packet.GetInt16();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_DEL_RANK", client.IP, client.Port);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
        ...
        for(var i = 9; i >= 0; --i)
        {
            if(!string.IsNullOrEmpty(client.Character.Guild.Ranks[i]))
            {
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"UPDATE guilds SET guild_rank{i} = '{string.Empty}', guild_rank{i}_Rights = '{0}' WHERE guild_id = {client.Character.Guild.Id};");
                _clusterServiceLocator.WcGuild.SendGuildQuery(client, client.Character.Guild.Id);
                _clusterServiceLocator.WcGuild.SendGuildRoster(client.Character);
                return;
            }
    ...
    public void On_CMSG_GUILD_DEMOTE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_DEMOTE [{2}]", client.IP, client.Port, playerName);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query($"SELECT char_guid FROM characters WHERE char_name = '{playerName.Replace("'", "_")}';", ref q);
        ...
        if(objCharacter.Guild.Id != client.Character.Guild.Id)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(
                    client,
                    GuildCommand.GUILD_INVITE_S,
                    GuildError.GUILD_PLAYER_NOT_IN_GUILD_S,
                    playerName);
            return;
        }
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildRank = {objCharacter.GuildRank} WHERE char_guid = {objCharacter.Guid};");
    ...
    public void On_CMSG_GUILD_DISBAND(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_DISBAND", client.IP, client.Port);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
        ...
        foreach(var member in client.Character.Guild.Members.ToArray())
        {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update($"DELETE FROM guilds WHERE guild_id = {guildId};");
    }
    ...
    public void On_CMSG_GUILD_INFO(PacketClass packet, ClientClass client)
    {
        packet.GetInt16();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_INFO", client.IP, client.Port);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
    ...
    public void On_CMSG_GUILD_INVITE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_INVITE [{2}]", client.IP, client.Port, playerName);
        ...
        if(objCharacter.GuildInvited != 0L)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(
                    client,
                    GuildCommand.GUILD_INVITE_S,
                    GuildError.ALREADY_INVITED_TO_GUILD,
                    playerName);
            return;
        }
    ...
    public void On_CMSG_GUILD_LEADER(PacketClass packet, ClientClass client)
    {
        ...
        var playerName = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_LEADER [{2}]", client.IP, client.Port, playerName);
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT char_guid, char_guildId, char_guildrank FROM characters WHERE char_name = '{playerName}';",
                ref mySqlQuery);
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE guilds SET guild_leader = \"{playerGuid}\" WHERE guild_id = {client.Character.Guild.Id};");
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update($"UPDATE characters SET char_guildRank = {0} WHERE char_guid = {playerGuid};");
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildRank = {client.Character.GuildRank} WHERE char_guid = {client.Character.Guid};");
    ...
    public void On_CMSG_GUILD_LEAVE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_LEAVE", client.IP, client.Port);
        ...
        _clusterServiceLocator.WcGuild
            .SendGuildResult(
                client,
                GuildCommand.GUILD_QUIT_S,
                GuildError.GUILD_PLAYER_NO_MORE_IN_GUILD,
    ...
    public void On_CMSG_GUILD_MOTD(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_MOTD", client.IP, client.Port);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update($"UPDATE guilds SET guild_MOTD = '{motd}' WHERE guild_id = '{client.Character.Guild.Id}';");
        PacketClass response = new(Opcodes.SMSG_GUILD_EVENT);
    ...
    public void On_CMSG_GUILD_PROMOTE(PacketClass packet, ClientClass client)
    {
        ...
        var playerName = _clusterServiceLocator.Functions.CapitalizeName(packet.GetString());
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_PROMOTE [{2}]", client.IP, client.Port, playerName);
        if(!client.Character.IsInGuild)
        {
            _clusterServiceLocator.WcGuild
                .SendGuildResult(client, GuildCommand.GUILD_CREATE_S, GuildError.GUILD_PLAYER_NOT_IN_GUILD);
    ...
    public void On_CMSG_GUILD_QUERY(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_QUERY [{2}]", client.IP, client.Port, guildId);
    ...
    public void On_CMSG_GUILD_RANK(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE guilds SET guild_rank{rankId} = \"{rankName}\", guild_rank{rankId}_Rights = {rankRights} WHERE guild_id = {client.Character.Guild.Id};");
        _clusterServiceLocator.WcGuild.SendGuildQuery(client, client.Character.Guild.Id);
        _clusterServiceLocator.WcGuild.SendGuildRoster(client.Character);
    }
    ...
    public void On_CMSG_GUILD_REMOVE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_REMOVE [{2}]", client.IP, client.Port, playerName);
        if(playerName.Length < 2)
        {
            return;
        }
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query($"SELECT char_guid FROM characters WHERE char_name = '{playerName}';", ref q);
    ...
    public void On_CMSG_GUILD_ROSTER(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GUILD_ROSTER", client.IP, client.Port);
        _clusterServiceLocator.WcGuild.SendGuildRoster(client.Character);
    }
    ...
    public void On_CMSG_GUILD_SET_OFFICER_NOTE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_GUILD_SET_OFFICER_NOTE [{2}]",
                client.IP,
                client.Port,
                playerName);
        if(!client.Character.IsInGuild)
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildOffNote = \"{note.Replace("\"", "_").Replace("'", "_")}\" WHERE char_name = \"{playerName}\";");
        _clusterServiceLocator.WcGuild.SendGuildRoster(client.Character);
    }
    ...
    public void On_CMSG_GUILD_SET_PUBLIC_NOTE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_GUILD_SET_PUBLIC_NOTE [{2}]",
                client.IP,
                client.Port,
                playerName);
        if(!client.Character.IsInGuild)
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildPNote = \"{note.Replace("\"", "_").Replace("'", "_")}\" WHERE char_name = \"{playerName}\";");
        _clusterServiceLocator.WcGuild.SendGuildRoster(client.Character);
    }
    ...
    public void On_CMSG_TURN_IN_PETITION(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT * FROM petitions WHERE petition_itemGuid = {itemGuid - _clusterServiceLocator.GlobalConstants.GUID_ITEM} LIMIT 1;",
                ref q);
        if(q.Rows.Count == 0)
        {
            return;
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"INSERT INTO guilds (guild_name, guild_leader, guild_cYear, guild_cMonth, guild_cDay) VALUES ('{name}', {client.Character.Guid}, {DateAndTime.Now.Year - 2006}, {DateAndTime.Now.Month}, {DateAndTime.Now.Day}); SELECT guild_id FROM guilds WHERE guild_name = '{name}';",
                ref q2);
        ...
        {
            if(_clusterServiceLocator.WorldCluster.CharacteRs
                .ContainsKey(q.Rows[0].As<ulong>($"petition_signedMember{i}")))
            {
                var tmp = _clusterServiceLocator.WorldCluster.CharacteRs;
            ...
            } else
            {
                _clusterServiceLocator.WcGuild
                    .AddCharacterToGuild(
                        q.Rows[0].As<ulong>($"petition_signedMember{i}"),
                        q2.Rows[0].As<int>("guild_id"));
            }
        }
    ...
    public void On_MSG_SAVE_GUILD_EMBLEM(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] MSG_SAVE_GUILD_EMBLEM [{2},{3}] [{4}:{5}:{6}:{7}:{8}]",
                client.IP,
                client.Port,
                unk0,
                unk1,
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE guilds SET guild_tEmblemStyle = {tEmblemStyle}, guild_tEmblemColor = {tEmblemColor}, guild_tBorderStyle = {tBorderStyle}, guild_tBorderColor = {tBorderColor}, guild_tBackgroundColor = {tBackgroundColor} WHERE guild_id = {client.Character.Guild.Id};");
        _clusterServiceLocator.WcGuild.SendGuildQuery(client, client.Character.Guild.Id);
