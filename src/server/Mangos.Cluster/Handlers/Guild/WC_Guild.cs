...
public partial class WC_Guild
{
    ...
    public WC_Guild(ClusterServiceLocator clusterServiceLocator) { _clusterServiceLocator = clusterServiceLocator; }
    ...
    public void AddCharacterToGuild(WcHandlerCharacter.CharacterObject objCharacter, int guildId, int guildRank = 4)
    {
        _clusterServiceLocator.WorldCluster
            ...
            .Update(
                $"UPDATE characters SET char_guildId = {guildId}, char_guildRank = {guildRank}, char_guildOffNote = '', char_guildPNote = '' WHERE char_guid = {objCharacter.Guid};");
        if(!GuilDs.ContainsKey((uint)guildId))
        {
    ...
    public void AddCharacterToGuild(ulong guid, int guildId, int guildRank = 4)
    {
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildId = {guildId}, char_guildRank = {guildRank}, char_guildOffNote = '', char_guildPNote = '' WHERE char_guid = {guid};");
    }

    public void BroadcastChatMessageGuild(
        ...
        int guildId)
    {
        ...
        var packet = _clusterServiceLocator.Functions
            .BuildChatMessage(sender.Guid, message, ChatMsg.CHAT_MSG_GUILD, language, (byte)sender.ChatFlag);

        // DONE: Send message to everyone
        foreach(var member in sender.Guild.Members.ToArray())
        {
            if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(member) &&
                _clusterServiceLocator.WorldCluster.CharacteRs[member].IsGuildRightSet(
                    GuildRankRights.GR_RIGHT_GCHATLISTEN))
            {
    ...
    public void BroadcastChatMessageOfficer(
        WcHandlerCharacter.CharacterObject sender,
        string message,
        LANGUAGES language,
        int guildId)
    {
        ...
        // DONE: Build packet
        var packet = _clusterServiceLocator.Functions
            .BuildChatMessage(sender.Guid, message, ChatMsg.CHAT_MSG_OFFICER, language, (byte)sender.ChatFlag);
        ...
        foreach(var member in sender.Guild.Members.ToArray())
        {
            if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(member) &&
                _clusterServiceLocator.WorldCluster.CharacteRs[member].IsGuildRightSet(
                    GuildRankRights.GR_RIGHT_OFFCHATLISTEN))
            {
                _clusterServiceLocator.WorldCluster.CharacteRs[member].Client.SendMultiplyPackets(packet);
            }
        }
    ...
    public void BroadcastToGuild(
        PacketClass packet,
        Guild guild,
        [Optional, DefaultParameterValue(0UL)] ulong notTo)
    {
    ...
    public void RemoveCharacterFromGuild(WcHandlerCharacter.CharacterObject objCharacter)
    {
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildId = {0}, char_guildRank = 0, char_guildOffNote = '', char_guildPNote = '' WHERE char_guid = {objCharacter.Guid};");
    ...
    public void RemoveCharacterFromGuild(ulong guid)
    {
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters SET char_guildId = {0}, char_guildRank = 0, char_guildOffNote = '', char_guildPNote = '' WHERE char_guid = {guid};");
    }

    // Members Options
    public void SendGuildMotd(WcHandlerCharacter.CharacterObject objCharacter)
    {
        if(objCharacter.IsInGuild && !string.IsNullOrEmpty(objCharacter.Guild.Motd))
        {
    ...
    public void SendGuildRoster(WcHandlerCharacter.CharacterObject objCharacter)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT char_online, char_guid, char_name, char_class, char_level, char_zone_id, char_logouttime, char_guildRank, char_guildPNote, char_guildOffNote FROM characters WHERE char_guildId = {objCharacter.Guild.Id};",
                ref members);
        ...
        for(int i = 0, loopTo = members.Rows.Count - 1; i <= loopTo; i++)
        {
            ...
            } else
            {
                ...
                var daysOffline = (float)((_clusterServiceLocator.Functions.GetTimestamp(DateAndTime.Now) -
                        members.Rows[i].As<uint>("char_logouttime")) /
                    ((double)DateInterval.Day));
