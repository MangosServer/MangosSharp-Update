...
public class WsHandlerChannels
{
    ...
    private long GetNexyChatChannelId() { return Interlocked.Increment(ref _chatChanneLsCounter); }
    ...
    public class ChatChannelClass : IDisposable
    {
        ...
        public ChatChannelClass(string name, ClusterServiceLocator clusterServiceLocator)
        {
            ...
            {
                if ((chatChannel.Value.Name.Replace('s', sZone).ToUpperInvariant() ?? string.Empty) ==
                    (name.ToUpper() ?? string.Empty))
                {
            ...
            if(clusterServiceLocator.WsDbcDatabase.ChatChannelsInfo.ContainsKey(ChannelIndex))
            {
                ...
                if((((ChatChannelsFlags)withBlock.Flags) & ChatChannelsFlags.FLAG_TRADE) ==
                    ChatChannelsFlags.FLAG_TRADE)
                {
                    ChannelFlags = (byte)(ChannelFlags | ((byte)CHANNEL_FLAG.CHANNEL_FLAG_TRADE));
                }

                if((((ChatChannelsFlags)withBlock.Flags) & ChatChannelsFlags.FLAG_CITY_ONLY2) ==
                    ChatChannelsFlags.FLAG_CITY_ONLY2)
                {
                    ChannelFlags = (byte)(ChannelFlags | ((byte)CHANNEL_FLAG.CHANNEL_FLAG_CITY));
                }

                ChannelFlags = ((((ChatChannelsFlags)withBlock.Flags) & ChatChannelsFlags.FLAG_LFG) ==
                        ChatChannelsFlags.FLAG_LFG)
            ...
            switch(notify)
            {
                ...
                default:
                    _clusterServiceLocator.WorldCluster.Log
                        .WriteLine(
                            LogType.WARNING,
                            "Probably wrong channel function used for SendChannelNotify({0})",
                            notify);
                    break;
            }
        ...
        public virtual void Ban(WcHandlerCharacter.CharacterObject character, string name)
        {
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_ON,
                    character.Guid,
                    default,
                    default);
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_MODERATOR,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(packet);
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_ON_FOR_NAME,
                    character.Guid,
                    default,
                    name);
                character.Client.Send(packet);
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_PLAYER_INVITE_BANNED,
                    character.Guid,
                    default,
                    name);
                character.Client.Send(packet);
            ...
            } else
            {
                ...
                var packet2 = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_BANNED,
                    victimGuid,
                    character.Guid,
                    default);
                Broadcast(packet2);
                ...
                _clusterServiceLocator.WorldCluster.CharacteRs[victimGuid].JoinedChannels
                    .Remove(ChannelName.ToUpper());

                // DONE: You Left channel
                var packet1 = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_YOU_LEFT,
                    character.Guid,
                    default,
                    default);
        ...
        public bool CanSetOwner(WcHandlerCharacter.CharacterObject character, string name)
        {
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_OWNER,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(packet);
            ...
            foreach(var guid in Joined.ToArray())
            {
                if((_clusterServiceLocator.WorldCluster.CharacteRs[guid].Name.ToUpper() ?? string.Empty) ==
                    (name.ToUpper() ?? string.Empty))
                {
                    return true;
                }
        ...
        public void GetOwner(WcHandlerCharacter.CharacterObject character)
        {
            ...
            } else
            {
                p = (Owner > 0m)
                    ? BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_WHO_OWNER,
                        character.Guid,
                        default,
                        _clusterServiceLocator.WorldCluster.CharacteRs[Owner].Name)
        ...
        public void Invite(WcHandlerCharacter.CharacterObject character, string name)
        {
            ...
            } else
            {
                ...
                if(!_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid))
                {
                    ...
                    _clusterServiceLocator.Functions.GetCharacterSide((byte)character.Race))
                {
                    var packet = BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_INVITED_WRONG_FACTION,
                        character.Guid,
                        default,
                        default);
                    character.Client.Send(packet);
                ...
                {
                    var packet1 = BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_PLAYER_INVITED,
                        character.Guid,
                        default,
                        _clusterServiceLocator.WorldCluster.CharacteRs[guid].Name);
                    character.Client.Send(packet1);
                    packet1.Dispose();
                    var packet2 = BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_INVITED,
                        character.Guid,
                        default,
                        default);
        ...
        public virtual void Join(WcHandlerCharacter.CharacterObject character, string clientPassword)
        {
            ...
            if(Joined.Contains(character.Guid) && character.JoinedChannels.Contains(0.ToString()))
            {
                var p = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_ALREADY_ON,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(p);
            ...
            {
                var p = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_YOU_ARE_BANNED,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(p);
            ...
            if(!string.IsNullOrEmpty(Password) && ((Password ?? string.Empty) != (clientPassword ?? string.Empty)))
            {
                var p = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_WRONG_PASS,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(p);
            ...
            if(Announce)
            {
                var response = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_JOINED,
                    character.Guid,
                    default,
                    default);
                Broadcast(response);
            ...
            var response2 = BuildChannelNotify(
                CHANNEL_NOTIFY_FLAGS.CHANNEL_YOU_JOINED,
                character.Guid,
                default,
                default);
            ...
            if(!JoinedMode.ContainsKey(character.Guid))
            {
                JoinedMode.Add(character.Guid, (byte)CHANNEL_USER_FLAG.CHANNEL_FLAG_NONE);
            }

            if(!character.JoinedChannels.Contains(ChannelName))
            {
                character.JoinedChannels.Add(ChannelName);
            }

            // DONE: If new channel, set owner
            if(_clusterServiceLocator.Functions.HaveFlags(ChannelFlags, (byte)CHANNEL_FLAG.CHANNEL_FLAG_CUSTOM) &&
                (Owner == 0m))
            {
                SetOwner(character);
            }
        ...
        public virtual void Kick(WcHandlerCharacter.CharacterObject character, string name)
        {
            ...
            } else
            {
                ...
                var packet2 = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_KICKED,
                    victimGuid,
                    character.Guid,
                    default);
                Broadcast(packet2);
        ...
        public virtual void Part(WcHandlerCharacter.CharacterObject character)
        {
            ...
            {
                var response = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_LEFT,
                    character.Guid,
                    default,
                    default);
                Broadcast(response);
        ...
        public void Say(string message, int msgLang, WcHandlerCharacter.CharacterObject character)
        {
            ...
            {
                var p = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_YOUCANTSPEAK,
                    character.Guid,
                    default,
                    default);
                character.Client.Send(p);
            ...
            } else
            {
                ...
                _clusterServiceLocator.WorldCluster.Log
                    .WriteLine(
                        LogType.USER,
                        "[{0}:{1}] SMSG_MESSAGECHAT [{2}: <{3}> {4}]",
                        character.Client.IP,
                        character.Client.Port,
                        ChannelName,
                        character.Name,
                        message);
            }
        ...
        public void SetAnnouncements(WcHandlerCharacter.CharacterObject character)
        {
            ...
            } else
            {
                ...
                var packet = Announce
                    ? BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_ENABLE_ANNOUNCE,
                        character.Guid,
                        default,
                        default)
                    : BuildChannelNotify(
                        CHANNEL_NOTIFY_FLAGS.CHANNEL_DISABLE_ANNOUNCE,
                        character.Guid,
                        default,
                        default);
                Broadcast(packet);
            ...
            } else
            {
                foreach(var guid in Joined.ToArray())
                {
                        ...
                        (name.ToUpper() ?? string.Empty))
                    {
                        ...
                        JoinedMode[guid] = (byte)(((CHANNEL_USER_FLAG)JoinedMode[guid]) |
                            CHANNEL_USER_FLAG.CHANNEL_FLAG_MODERATOR);
                        ...
                        var response = BuildChannelNotify(
                            CHANNEL_NOTIFY_FLAGS.CHANNEL_MODE_CHANGE,
                            guid,
                            flags,
                            default);
                        Broadcast(response);
            ...
            } else
            {
                foreach(var guid in Joined.ToArray())
                {
                        ...
                        (name.ToUpper() ?? string.Empty))
                    {
                        var flags = JoinedMode[guid];
                        JoinedMode[guid] = (byte)(((CHANNEL_USER_FLAG)JoinedMode[guid]) |
                            CHANNEL_USER_FLAG.CHANNEL_FLAG_MUTED);
        ...
        public void SetOwner(WcHandlerCharacter.CharacterObject character)
        {
            ...
            {
                JoinedMode[Owner] = (byte)(((CHANNEL_USER_FLAG)JoinedMode[Owner]) ^
                    CHANNEL_USER_FLAG.CHANNEL_FLAG_OWNER);
            }

            JoinedMode[character.Guid] = (byte)(JoinedMode[character.Guid] |
                ((byte)CHANNEL_USER_FLAG.CHANNEL_FLAG_OWNER));
            Owner = character.Guid;
            ...
            var p = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_CHANGE_OWNER, character.Guid, default, default);
            Broadcast(p);
            p.Dispose();
        }
        ...
        public void SetPassword(WcHandlerCharacter.CharacterObject character, string newPassword)
        {
            ...
            } else
            {
                ...
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_SET_PASSWORD,
                    character.Guid,
                    default,
                    default);
                Broadcast(packet);
        ...
        public virtual void UnBan(WcHandlerCharacter.CharacterObject character, string name)
        {
            ...
            {
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_BANNED,
                    character.Guid,
                    default,
                    name);
                character.Client.Send(packet);
            ...
            } else
            {
                ...
                var packet = BuildChannelNotify(
                    CHANNEL_NOTIFY_FLAGS.CHANNEL_UNBANNED,
                    victimGuid,
                    character.Guid,
                    default);
                Broadcast(packet);
