...
public class WcHandlersTickets
{
    ...
    public void On_CMSG_BUG(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_BUG [2]", client.IP, client.Port, suggestion);
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.INFORMATION,
                $"Bug report [{{0}}:{{1}} Lengths:{{2}}, {{3}}] {cString}{Constants.vbCrLf}{tString}",
                cLength.ToString(),
                tLength.ToString());
    }
    ...
    public void On_CMSG_GMTICKET_CREATE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster
            ...
            .Query($"SELECT * FROM characters_tickets WHERE char_guid = {client.Character.Guid};", ref mySqlResult);
        PacketClass smsgGmticketCreate = new(Opcodes.SMSG_GMTICKET_CREATE);
        if(mySqlResult.Rows.Count > 0)
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GMTICKET_CREATE", client.IP, client.Port);
        ...
        } else
        {
            ...
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update(
                    $"INSERT INTO characters_tickets (char_guid, ticket_text, ticket_x, ticket_y, ticket_z, ticket_map) VALUES ({client.Character.Guid} , \"{ticketText}\", {Strings.Trim(Conversion.Str(ticketX))}, {Strings.Trim(Conversion.Str(ticketY))}, {Strings.Trim(Conversion.Str(ticketZ))}, {ticketMap});");
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GMTICKET_DELETETICKET", client.IP, client.Port);
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update($"DELETE FROM characters_tickets WHERE char_guid = {client.Character.Guid};");
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GMTICKET_GETTICKET", client.IP, client.Port);
        ...
        smsgQueryTimeResponse.AddInt32(_clusterServiceLocator.NativeMethods.timeGetTime(string.Empty)); // GetTimestamp(Now))
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GMTICKET_SYSTEMSTATUS", client.IP, client.Port);
    ...
    public void On_CMSG_GMTICKET_UPDATETEXT(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_GMTICKET_UPDATETEXT [{2}]",
                client.IP,
                client.Port,
                ticketText);
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Update(
                $"UPDATE characters_tickets SET char_guid={client.Character.Guid}, ticket_text=\"{ticketText}\";");
    }
    ...
    public void On_CMSG_WHOIS(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_WHOIS [{2}]", client.IP, client.Port, name);
