...
public class WcHandlersSocial
{
    ...
    public void LoadIgnoreList(WcHandlerCharacter.CharacterObject objCharacter)
    {
        ...
        _clusterServiceLocator.WorldCluster
            ...
            .Query(
                $"SELECT * FROM character_social WHERE guid = {objCharacter.Guid} AND flags = {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_IGNORED)};",
                ref q);
    ...
    public void NotifyFriendStatus(WcHandlerCharacter.CharacterObject objCharacter, FriendStatus s)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT guid FROM character_social WHERE friend = {objCharacter.Guid} AND (flags & {Conversions.ToInteger(SocialFlag.SOCIAL_FLAG_FRIEND)}) > 0;",
                ref q);
        ...
        foreach(DataRow row in q.Rows)
        {
            ...
            if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid) &&
                (_clusterServiceLocator.WorldCluster.CharacteRs[guid].Client is not null))
            {
    ...
    public void On_CMSG_ADD_FRIEND(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ADD_FRIEND [{2}]", client.IP, client.Port, name);

        // DONE: Get GUID from DB
        DataTable q = new();
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query($"SELECT char_guid, char_race FROM characters WHERE char_name = \"{name}\";", ref q);
        if(q.Rows.Count > 0)
        {
            ...
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT flags FROM character_social WHERE flags = {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_FRIEND)}",
                    ref q);
            ...
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT flags FROM character_social WHERE guid = {client.Character.Guid} AND friend = {guid} AND flags = {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_FRIEND)};",
                    ref q);
            ...
            } else if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid))
            {
                ...
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"INSERT INTO character_social (guid, friend, flags) VALUES ({client.Character.Guid}, {guid}, {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_FRIEND)});");
            } else
            {
        ...
        q.Dispose();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_FRIEND_STATUS", client.IP, client.Port);
    }
    ...
    public void On_CMSG_ADD_IGNORE(PacketClass packet, ClientClass client)
    {
        ...
        var guid = 0UL;
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ADD_IGNORE [{2}]", client.IP, client.Port, name);

        // DONE: Get GUID from DB
        DataTable q = new();
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query($"SELECT char_guid FROM characters WHERE char_name = \"{name}\";", ref q);
        if(q.Rows.Count > 0)
        {
            guid = (ulong)q.Rows[0].As<long>("char_guid");
            q.Clear();
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT flags FROM character_social WHERE flags = {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_IGNORED)}",
                    ref q);
            var numberOfFriends = q.Rows.Count;
            q.Clear();
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT * FROM character_social WHERE guid = {client.Character.Guid} AND friend = {guid} AND flags = {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_IGNORED)};",
                    ref q);
            if(guid == client.Character.Guid)
            {
            ...
            } else
            {
                ...
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"INSERT INTO character_social (guid, friend, flags) VALUES ({client.Character.Guid}, {guid}, {Conversions.ToByte(SocialFlag.SOCIAL_FLAG_IGNORED)});");
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_DEL_FRIEND", client.IP, client.Port);
        ...
        try
        {
            ...
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT flags FROM character_social WHERE guid = {client.Character.Guid} AND friend = {guid};",
                    ref q);
            if(q.Rows.Count > 0)
            {
                ...
                if((newFlags & (SocialFlag.SOCIAL_FLAG_FRIEND | SocialFlag.SOCIAL_FLAG_IGNORED)) == 0)
                {
                    _clusterServiceLocator.WorldCluster
                        .GetCharacterDatabase()
                        .Update(
                            $"DELETE FROM character_social WHERE friend = {guid} AND guid = {client.Character.Guid};");
                } else
                {
                    _clusterServiceLocator.WorldCluster
                        .GetCharacterDatabase()
                        .Update(
                            $"UPDATE character_social SET flags = {newFlags} WHERE friend = {guid} AND guid = {client.Character.Guid};");
                }
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_DEL_IGNORE", client.IP, client.Port);
        if((packet.Data.Length - 1) < 13)
        {
            return;
        }
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_FRIEND_LIST", client.IP, client.Port);
    ...
    public void On_CMSG_WHO(PacketClass packet, ClientClass client)
    {
        ...
        {
            strings.Add(
                _clusterServiceLocator.CommonFunctions
                    .UppercaseFirstLetter(_clusterServiceLocator.Functions.EscapeString(packet.GetString())));
        }

        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_WHO [P:'{2}' G:'{3}' L:{4}-{5} C:{6:X} R:{7:X}]",
                client.IP,
                client.Port,
                namePlayer,
                nameGuild,
        ...
        foreach(var objCharacter in _clusterServiceLocator.WorldCluster.CharacteRs)
        {
            ...
            if((_clusterServiceLocator.Functions.GetCharacterSide((byte)objCharacter.Value.Race) !=
                    _clusterServiceLocator.Functions.GetCharacterSide((byte)client.Character.Race)) &&
                (client.Character.Access < AccessLevel.GameMaster))
            {
                continue;
            }
            ...
            if(!string.IsNullOrEmpty(namePlayer) &&
                !_clusterServiceLocator.CommonFunctions
                    .UppercaseFirstLetter(objCharacter.Value.Name)
                    .Contains(_clusterServiceLocator.CommonFunctions.UppercaseFirstLetter(namePlayer)))
            {
                continue;
            }

            if(!string.IsNullOrEmpty(nameGuild) &&
                ((objCharacter.Value.Guild is null) ||
                    !_clusterServiceLocator.CommonFunctions
                        .UppercaseFirstLetter(objCharacter.Value.Guild.Name)
                        .Contains(_clusterServiceLocator.CommonFunctions.UppercaseFirstLetter(nameGuild))))
            {
                continue;
            }
            ...
            if((zonesCount > 0L) && (!zones.Contains(objCharacter.Value.Zone)))
            {
                continue;
            }
            ...
            if(stringsCount > 0L)
            {
                ...
                {
                    if(_clusterServiceLocator.CommonFunctions
                            .UppercaseFirstLetter(objCharacter.Value.Name)
                            .IndexOf(stringValue, StringComparison.Ordinal) !=
                        (-1))
                    {
                        continue;
                    }
                    ...
                    if(_clusterServiceLocator.CommonFunctions
                            .UppercaseFirstLetter(
                                _clusterServiceLocator.Functions.GetRaceName((int)objCharacter.Value.Race)) ==
                        stringValue)
                    {
                        continue;
                    }
                    ...
                    if(_clusterServiceLocator.CommonFunctions
                            .UppercaseFirstLetter(
                                _clusterServiceLocator.Functions.GetClassName((int)objCharacter.Value.Classe)) ==
                        stringValue)
                    {
                        continue;
                    }

                    if((objCharacter.Value.Guild is not null) &&
                        (_clusterServiceLocator.CommonFunctions
                                .UppercaseFirstLetter(objCharacter.Value.Guild.Name)
                                .IndexOf(stringValue, StringComparison.Ordinal) !=
                            (-1)))
        ...
        foreach(var guid in results)
        {
            ...
            {
                response.AddString(string.Empty);
            }                          // Guild Name
    ...
    public void SendFriendList(ClientClass client, WcHandlerCharacter.CharacterObject character)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT * FROM character_social WHERE guid = {character.Guid} AND (flags & {Conversions.ToInteger(SocialFlag.SOCIAL_FLAG_FRIEND)}) > 0;",
                ref q);
        ...
        if(q.Rows.Count > 0)
        {
            ...
            foreach(DataRow row in q.Rows)
            {
                ...
                if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid) &&
                    _clusterServiceLocator.WorldCluster.CharacteRs[guid].IsInWorld)
                {
                    if(_clusterServiceLocator.WorldCluster.CharacteRs[guid].Dnd)
                    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_FRIEND_LIST", client.IP, client.Port);
    }
    ...
    public void SendIgnoreList(ClientClass client, WcHandlerCharacter.CharacterObject character)
    {
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query(
                $"SELECT * FROM character_social WHERE guid = {character.Guid} AND (flags & {Conversions.ToInteger(SocialFlag.SOCIAL_FLAG_IGNORED)}) > 0;",
                ref q);
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_IGNORE_LIST", client.IP, client.Port);
    }
}
