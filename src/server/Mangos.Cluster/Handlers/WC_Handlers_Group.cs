...
public class WcHandlersGroup
{
    ...
    public void On_CMSG_GROUP_ACCEPT(PacketClass packet, ClientClass client)
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_ACCEPT", client.IP, client.Port);
        ...
        } else
        {
            SendPartyResult(
                ...
                PartyCommand.PARTY_OP_INVITE,
                PartyCommandResult.INVITE_PARTY_FULL);
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_CANCEL", client.IP, client.Port);
    }
    ...
    public void On_CMSG_GROUP_CHANGE_SUB_GROUP(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_GROUP_CHANGE_SUB_GROUP [{2}:{3}]",
                client.IP,
                client.Port,
                name,
                subGroup);
        if(client.Character.IsInGroup)
        {
            ...
            {
                if((client.Character.Group.Members[i] is not null) &&
                    ((client.Character.Group.Members[i].Name ?? string.Empty) == (name ?? string.Empty)))
                {
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_DECLINE", client.IP, client.Port);
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_DISBAND", client.IP, client.Port);
        if(client.Character.IsInGroup)
        {
    ...
    public void On_CMSG_GROUP_INVITE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_INVITE [{2}]", client.IP, client.Port, name);
        var guid = 0UL;
        ...
        {
            if(_clusterServiceLocator.CommonFunctions.UppercaseFirstLetter(character.Value.Name) ==
        ...
        } else if(!_clusterServiceLocator.WorldCluster.CharacteRs[guid].IsInWorld)
        {
            errCode = PartyCommandResult.INVITE_NOT_FOUND;
        } else if(_clusterServiceLocator.Functions
                .GetCharacterSide((byte)_clusterServiceLocator.WorldCluster.CharacteRs[guid].Race) !=
            _clusterServiceLocator.Functions.GetCharacterSide((byte)client.Character.Race))
        {
        ...
        } else if(!client.Character.IsInGroup)
        {
            ...
            _clusterServiceLocator.WorldCluster.CharacteRs[guid].Group = (new(
                client.Character,
        ...
        } else if((!client.Character.IsGroupLeader) && (!client.Character.GroupAssistant))
        {
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_RAID_CONVERT", client.IP, client.Port);
        if(client.Character.IsInGroup)
        {
            SendPartyResult(client, string.Empty, PartyCommand.PARTY_OP_INVITE, PartyCommandResult.INVITE_OK);
    ...
    public void On_CMSG_GROUP_SET_LEADER(PacketClass packet, ClientClass client)
    {
        ...
        var name = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_SET_LEADER [Name={2}]", client.IP, client.Port, name);
        var guid = _clusterServiceLocator.WcHandlerCharacter.GetCharacterGuidByName(name);
        if(guid == 0m)
        {
            SendPartyResult(client, string.Empty, PartyCommand.PARTY_OP_INVITE, PartyCommandResult.INVITE_NOT_FOUND);
        ...
        } else if(!client.Character.IsGroupLeader)
        {
            SendPartyResult(
                ...
                client.Character.Name,
                PartyCommand.PARTY_OP_INVITE,
                PartyCommandResult.INVITE_NOT_LEADER);
        } else
        {
    ...
    public void On_CMSG_GROUP_SWAP_SUB_GROUP(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_GROUP_SWAP_SUB_GROUP [{2}:{3}]",
                client.IP,
                client.Port,
                name1,
                name2);
        if(client.Character.IsInGroup)
        {
            ...
            for(j = 0; j <= loopTo; j++)
            {
                if((client.Character.Group.Members[j] is not null) &&
                    ((client.Character.Group.Members[j].Name ?? string.Empty) == (name2 ?? string.Empty)))
                {
                    break;
                }
            }
            ...
            {
                if((client.Character.Group.Members[i] is not null) &&
                    ((client.Character.Group.Members[i].Name ?? string.Empty) == (name1 ?? string.Empty)))
                {
                    (client.Character.Group.Members[i], client.Character.Group.Members[j]) = (client.Character.Group.Members[
                        j], client.Character.Group.Members[i]);
                    if(client.Character.Group.Leader == i)
                    {
                        client.Character.Group.Leader = (byte)j;
    ...
    public void On_CMSG_GROUP_UNINVITE(PacketClass packet, ClientClass client)
    {
        ...
        var name = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_UNINVITE [{2}]", client.IP, client.Port, name);
        var guid = 0UL;
        _clusterServiceLocator.WorldCluster.CharacteRsLock
            .AcquireReaderLock(_clusterServiceLocator.GlobalConstants.DEFAULT_LOCK_TIMEOUT);
        foreach(var character in _clusterServiceLocator.WorldCluster.CharacteRs)
        ...
        {
            SendPartyResult(client, string.Empty, PartyCommand.PARTY_OP_LEAVE, PartyCommandResult.INVITE_NOT_LEADER);
        } else
        {
            var tmp = _clusterServiceLocator.WorldCluster.CharacteRs;
            var argobjCharacter = tmp[guid];
    ...
    public void On_CMSG_GROUP_UNINVITE_GUID(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GROUP_UNINVITE_GUID [0x{2:X}]", client.IP, client.Port, guid);

        // TODO: InBattlegrounds: INVITE_RESTRICTED
        if(guid == 0m)
        {
            SendPartyResult(client, string.Empty, PartyCommand.PARTY_OP_LEAVE, PartyCommandResult.INVITE_NOT_FOUND);
        } else if(!_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid))
        {
            SendPartyResult(client, string.Empty, PartyCommand.PARTY_OP_LEAVE, PartyCommandResult.INVITE_NOT_FOUND);
        } else if(!client.Character.IsGroupLeader)
    ...
    public void On_CMSG_LOOT_METHOD(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_LOOT_METHOD [Method={2}, Master=0x{3:X}, Threshold={4}]",
                client.IP,
                client.Port,
                method,
                master,
    ...
    public void On_CMSG_REQUEST_PARTY_MEMBER_STATS(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_REQUEST_PARTY_MEMBER_STATS [{2:X}]",
                client.IP,
                client.Port,
                guid);
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_REQUEST_RAID_INFO", client.IP, client.Port);
        DataTable q = new();
        if(client.Character is not null)
        {
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query($"SELECT * FROM characters_instances WHERE char_guid = {client.Character.Guid};", ref q);
        }
        ...
        foreach(DataRow r in q.Rows)
        {
            ...
            response.AddUInt32(
                (uint)(Conversions.ToInteger(r["expire"]) -
                    _clusterServiceLocator.Functions.GetTimestamp(DateAndTime.Now)));  // TimeLeft
    ...
    public void On_MSG_MINIMAP_PING(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] MSG_MINIMAP_PING [{2}:{3}]", client.IP, client.Port, x, y);
        if(client.Character.IsInGroup)
        {
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] MSG_RAID_READY_CHECK", client.IP, client.Port);
        if(client.Character.IsGroupLeader)
        {
    ...
    public void On_MSG_RANDOM_ROLL(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] MSG_RANDOM_ROLL [min={2} max={3}]",
                client.IP,
                client.Port,
                minRoll,
                maxRoll);
    ...
    public void SendPartyResult(
        ClientClass objCharacter,
        string name,
        PartyCommand operation,
    ...
    public class Group : IDisposable
    {
        ...
        public void BroadcastToOther(PacketClass packet, WcHandlerCharacter.CharacterObject objCharacter)
        {
            ...
            {
                if((Members[i] is not null) &&
                    !ReferenceEquals(Members[i], objCharacter) &&
                    (Members[i].Client is not null))
                {
                    Members[i].Client.SendMultiplyPackets(packet);
                }
            }
        ...
        public void BroadcastToOutOfRange(PacketClass packet, WcHandlerCharacter.CharacterObject objCharacter)
        {
            for(byte i = 0, loopTo = (byte)(Members.Length - 1); i <= loopTo; i++)
            {
                if((Members[i] is not null) &&
                    ...
                    (Members[i].Client is not null))
                {
                    if((objCharacter.Map != Members[i].Map) ||
                        (Math.Sqrt(
                                Math.Pow(objCharacter.PositionX - Members[i].PositionX, 2d) +
                                    Math.Pow(objCharacter.PositionY - Members[i].PositionY, 2d)) >
                            _clusterServiceLocator.GlobalConstants.DEFAULT_DISTANCE_VISIBLE))
                    {
                        Members[i].Client.SendMultiplyPackets(packet);
                    }
                }
        ...
        public void ConvertToRaid()
        {
            ...
            for(int i = _clusterServiceLocator.GlobalConstants.GROUP_SIZE + 1, loopTo = _clusterServiceLocator.GlobalConstants.GROUP_RAIDSIZE; i <=
                loopTo; i++)
            {
                Members[i] = null;
            }
        ...
        public WcHandlerCharacter.CharacterObject GetLeader() { return Members[Leader]; }

        public WcHandlerCharacter.CharacterObject GetLootMaster() { return Members[Leader]; }
        ...
        public void SendChatMessage(
            WcHandlerCharacter.CharacterObject sender,
            string message,
            LANGUAGES language,
            ChatMsg thisType)
        {
            var packet = _clusterServiceLocator.Functions
                .BuildChatMessage(sender.Guid, message, thisType, language, (byte)sender.ChatFlag);
            Broadcast(packet);
            packet.Dispose();
        }
