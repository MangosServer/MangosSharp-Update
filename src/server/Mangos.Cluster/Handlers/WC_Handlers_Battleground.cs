...
public class WC_Handlers_Battleground
{
    ...
    private int GetBattleGrounMapIdByTypeId(BattleGroundTypeId bgTypeId)
    {
        ...
        {
            var @case when @case == BattleGroundTypeId.BATTLEGROUND_AV => 30,
            var case1 when case1 == BattleGroundTypeId.BATTLEGROUND_WS => 489,
            var case2 when case2 == BattleGroundTypeId.BATTLEGROUND_AB => 529,
            _ => 0,
        };
    }
    ...
    public void On_CMSG_BATTLEFIELD_PORT(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_BATTLEFIELD_PORT [Action: {1}, ID: {2}]",
                client.IP,
                client.Port,
                action,
                id);
    ...
    public void On_CMSG_BATTLEMASTER_JOIN(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_BATTLEMASTER_JOIN [MapType: {2}, Instance: {3}, Group: {4}, GUID: {5}]",
                client.IP,
                client.Port,
                mapType,
                instance,
    ...
    public void On_CMSG_LEAVE_BATTLEFIELD(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_LEAVE_BATTLEFIELD [MapType: {2}, Unk1: {3}, Unk2: {4}, ID: {5}]",
                client.IP,
                client.Port,
                mapType,
                unk1,
    ...
    public class Battlefield : IDisposable
    {
        ...
        public void Join(WcHandlerCharacter.CharacterObject objCharacter)
        {
            if(_invitedTeam1.Contains(objCharacter) || _invitedTeam2.Contains(objCharacter))
            {
                ...
                SendBattlegroundStatus(objCharacter, 0);
                var withBlock = _clusterServiceLocator.WsDbcDatabase.WorldSafeLocs[
                    _clusterServiceLocator.WsDbcDatabase.Battlegrounds[(byte)MapType].AllianceStartLoc];
                ...
                objCharacter.Transfer(
                    ...
                    withBlock.Y,
                    withBlock.Z,
                    _clusterServiceLocator.WsDbcDatabase.Battlegrounds[(byte)MapType].AllianceStartO,
                    (int)withBlock.Map);
            }
        }
