...
public class WC_Handlers_Chat
{
    ...
    public void On_CMSG_CHANNEL_ANNOUNCEMENTS(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_CHANNEL_ANNOUNCEMENTS [{2}]",
                client.IP,
                client.Port,
                channelName);
    ...
    public void On_CMSG_CHANNEL_LIST(PacketClass packet, ClientClass client)
    {
        ...
        var channelName = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHANNEL_LIST [{2}]", client.IP, client.Port, channelName);
    ...
    public void On_CMSG_CHANNEL_MODERATE(PacketClass packet, ClientClass client)
    {
        ...
        var channelName = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHANNEL_MODERATE [{2}]", client.IP, client.Port, channelName);
    ...
    public void On_CMSG_CHANNEL_MODERATOR(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_CHANNEL_MODERATOR [{2}, {3}]",
                client.IP,
                client.Port,
                channelName,
                channelUser);
        ...
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].SetModerator(
                client.Character,
                channelUser);
        }
    }
    ...
    public void On_CMSG_CHANNEL_MUTE(PacketClass packet, ClientClass client)
    {
        ...
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].SetMute(
                client.Character,
                channelUser);
        }
    }
    ...
    public void On_CMSG_CHANNEL_OWNER(PacketClass packet, ClientClass client)
    {
        ...
        var channelName = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHANNEL_OWNER [{2}]", client.IP, client.Port, channelName);
    ...
    public void On_CMSG_CHANNEL_PASSWORD(PacketClass packet, ClientClass client)
    {
        ...
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].SetPassword(
                client.Character,
                channelNewPassword);
        }
    }
    ...
    public void On_CMSG_CHANNEL_SET_OWNER(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_CHANNEL_SET_OWNER [{2}, {3}]",
                client.IP,
                client.Port,
                channelName,
                channelNewOwner);
        ...
        // ChannelName = ChannelName.ToUpper
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName) &&
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].CanSetOwner(
                client.Character,
                channelNewOwner))
        {
            foreach(var guid in _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].Joined.ToArray())
            {
                if((_clusterServiceLocator.WorldCluster.CharacteRs[guid].Name.ToUpper() ?? string.Empty) ==
                    (channelNewOwner.ToUpper() ?? string.Empty))
                {
    ...
    public void On_CMSG_CHANNEL_UNMODERATOR(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_CHANNEL_UNMODERATOR [{2}, {3}]",
                client.IP,
                client.Port,
                channelName,
                channelUser);
        ...
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].SetUnModerator(
                client.Character,
                channelUser);
        }
    }
    ...
    public void On_CMSG_CHANNEL_UNMUTE(PacketClass packet, ClientClass client)
    {
        ...
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channelName].SetUnMute(
                client.Character,
                channelUser);
        }
    }
    ...
    public void On_CMSG_CHAT_IGNORED(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAT_IGNORED [0x{2}]", client.IP, client.Port, guid);
        if(_clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid))
        {
            var response = _clusterServiceLocator.Functions
                .BuildChatMessage(
                    client.Character.Guid,
                    string.Empty,
                    ChatMsg.CHAT_MSG_IGNORED,
    ...
    public void On_CMSG_JOIN_CHANNEL(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_JOIN_CHANNEL [{2}]", client.IP, client.Port, channelName);
        if(!_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
            // The New does a an add to the .Containskey collection above
            WsHandlerChannels.ChatChannelClass ChatChannelClass = new(channelName, _clusterServiceLocator);
        }
    ...
    public void On_CMSG_LEAVE_CHANNEL(PacketClass packet, ClientClass client)
    {
        ...
        var channelName = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_LEAVE_CHANNEL [{2}]", client.IP, client.Port, channelName);
        if(_clusterServiceLocator.WsHandlerChannels.ChatChanneLs.ContainsKey(channelName))
        {
    ...
    public void On_CMSG_MESSAGECHAT(PacketClass packet, ClientClass client)
    {
        ...
        switch(msgType)
        {
            case var @case when @case == ChatMsg.CHAT_MSG_CHANNEL:
            {
                ...
                {
                    _clusterServiceLocator.WsHandlerChannels.ChatChanneLs[channel].Say(
                        message,
                        (int)msgLanguage,
                        client.Character);
                }
            ...
            case var case1 when case1 == ChatMsg.CHAT_MSG_WHISPER:
            {
                ...
                foreach(var character in _clusterServiceLocator.WorldCluster.CharacteRs)
                {
                    if(_clusterServiceLocator.CommonFunctions.UppercaseFirstLetter(character.Value.Name) ==
                        _clusterServiceLocator.CommonFunctions.UppercaseFirstLetter(toUser))
                    {
                ...
                if((guid > 0m) && _clusterServiceLocator.WorldCluster.CharacteRs.ContainsKey(guid))
                {
                    ...
                    if(_clusterServiceLocator.WorldCluster.CharacteRs[guid].IgnoreList
                            .Contains(client.Character.Guid) &&
                    ...
                    } else
                    {
                        ...
                        client.Character
                            .SendChatMessage(
                                guid,
                                message,
                                ChatMsg.CHAT_MSG_WHISPER_INFORM,
                                (int)msgLanguage,
                                string.Empty);
                        ...
                        {
                            // From message
                            _clusterServiceLocator.WorldCluster.CharacteRs[guid].SendChatMessage(
                                client.Character.Guid,
                                message,
                        ...
                        } else
                        {
                            ...
                            client.Character
                                .SendChatMessage(
                                    guid,
                                    _clusterServiceLocator.WorldCluster.CharacteRs[guid].AfkMessage,
                                    ChatMsg.CHAT_MSG_DND,
                                    (int)msgLanguage,
                                    string.Empty);
                        }
            ...
            case var case6 when case6 == ChatMsg.CHAT_MSG_AFK:
            {
                ...
                if(string.IsNullOrEmpty(message) || (!client.Character.Afk))
                {
            ...
            {
                var message = packet.GetString();
                if(string.IsNullOrEmpty(message) || (!client.Character.Dnd))
                {
            ...
            case var case11 when case11 == ChatMsg.CHAT_MSG_GUILD:
            {
                ...
                _clusterServiceLocator.WcGuild
                    .BroadcastChatMessageGuild(
                        client.Character,
                        message,
                        msgLanguage,
                        (int)client.Character.Guild.Id);
                break;
            }
            ...
            default:
                _clusterServiceLocator.WorldCluster.Log
                    .WriteLine(
                        LogType.FAILED,
                        "[{0}:{1}] Unknown chat message [msgType={2}, msgLanguage={3}]",
                        client.IP,
                        client.Port,
                        msgType,
                        msgLanguage);
