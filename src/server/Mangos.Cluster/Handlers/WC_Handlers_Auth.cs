...
public class WC_Handlers_Auth
{
    ...
    public WC_Handlers_Auth(ClusterServiceLocator clusterServiceLocator, MangosConfiguration mangosConfiguration)
    {
    ...
    public void On_CMSG_AUTH_SESSION(PacketClass packet, ClientClass client)
    {
        ...
        {
            if((tmpClientEntry.Value is not null) && (tmpClientEntry.Value.Account == tmp))
            {
        ...
        var query = $"SELECT sessionkey, gmlevel FROM account WHERE username = '{client.Account}';";
        ...
        } else
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(
                    LogType.USER,
                    "[{0}:{1}] AUTH_UNKNOWN_ACCOUNT: Account not in DB!",
                    client.IP,
                    client.Port);
        ...
        if((_clusterServiceLocator.WorldCluster.ClienTs.Count > mangosConfiguration.Cluster.ServerPlayerLimit) &&
            (client.Access <= AccessLevel.Player))
        {
        ...
        {
            if(File.Exists($@"interface\{addOnsNames[i]}.pub") && (addOnsHashes[i] != 0x1C776D01U))
            {
                // We have hash data
                addOnsEnable.AddInt8(2);                    // AddOn Type [1-enabled, 0-banned, 2-blizzard]
                addOnsEnable.AddInt8(1);                    // Unk
                FileStream fs = new(
                    $@"interface\{addOnsNames[i]}.pub",
                    FileMode.Open,
                    FileAccess.Read,
                    FileShare.Read,
                    258,
    ...
    public void On_CMSG_CHAR_CREATE(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_CREATE [{2}]", client.IP, client.Port, name);
        ...
        try
        {
            ...
            {
                result = _clusterServiceLocator.WcNetwork.WorldServer.Worlds[0U].ClientCreateCharacter(
                    client.Account,
                    name,
                    race,
                    classe,
            ...
            {
                result = _clusterServiceLocator.WcNetwork.WorldServer.Worlds[1U].ClientCreateCharacter(
                    client.Account,
                    name,
                    race,
                    classe,
        ...
        } catch(Exception ex)
        {
            ...
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(
                    LogType.FAILED,
                    "[{0}:{1}] Character creation failed!{2}{3}",
                    client.IP,
                    client.Port,
                    Constants.vbCrLf,
                    ex.ToString());
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_DELETE", client.IP, client.Port);
        ...
        try
        {
            ...
            _clusterServiceLocator.WorldCluster
                .GetAccountDatabase()
                .Query($"SELECT id FROM account WHERE username = \"{client.Account}\";", ref q);
            if(q.Rows.Count == 0)
            {
                return;
            }
            ...
            _clusterServiceLocator.WorldCluster
                ...
                .Query(
                    $"SELECT char_guid FROM characters WHERE account_id = \"{q.Rows[0]["id"]}\" AND char_guid = \"{guid}\";",
                    ref q);
            if(q.Rows.Count == 0)
            {
            ...
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query($"SELECT item_guid FROM characters_inventory WHERE item_bag = {guid};", ref q);
            foreach(DataRow row in q.Rows)
            {
                ...
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"DELETE FROM characters_inventory WHERE item_guid = \"{row.As<string>("item_guid")}\";");
                // DONE: Delete items in bags
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"DELETE FROM characters_inventory WHERE item_bag = \"{row.As<ulong>("item_guid") + _clusterServiceLocator.GlobalConstants.GUID_ITEM}\";");
            }

            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query($"SELECT item_guid FROM characters_inventory WHERE item_owner = {guid};", ref q);
            q.Clear();
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query($"SELECT mail_id FROM characters_mail WHERE mail_receiver = \"{guid}\";", ref q);
            foreach(DataRow row in q.Rows)
            {
                // TODO: Return mails?
                // DONE: Delete mails
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update($"DELETE FROM characters_mail WHERE mail_id = \"{row.As<string>("mail_id")}\";");
                // DONE: Delete mail items
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update($"DELETE FROM mail_items WHERE mail_id = \"{row.As<string>("mail_id")}\";");
            }

            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM characters WHERE char_guid = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM characters_honor WHERE char_guid = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM characters_quests WHERE char_guid = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM character_social WHERE guid = '{guid}' OR friend = '{guid}';");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM characters_spells WHERE guid = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM petitions WHERE petition_owner = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM auctionhouse WHERE auction_owner = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM characters_tickets WHERE char_guid = \"{guid}\";");
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update($"DELETE FROM corpse WHERE guid = \"{guid}\";");
            q.Clear();
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query($"SELECT guild_id FROM guilds WHERE guild_leader = \"{guid}\";", ref q);
            if(q.Rows.Count > 0)
            {
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update(
                        $"UPDATE characters SET char_guildid = 0, char_guildrank = 0, char_guildpnote = '', charguildoffnote = '' WHERE char_guildid = \"{q.Rows[0]["guild_id"]}\";");
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Update($"DELETE FROM guild WHERE guild_id = \"{q.Rows[0]["guild_id"]}\";");
            }
        ...
        client.Send(response);
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_CHAR_DELETE [{2:X}]", client.IP, client.Port, guid);
    }

    public void On_CMSG_CHAR_ENUM(PacketClass packet, ClientClass client)
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_ENUM", client.IP, client.Port);
        ...
        try
        {
            _clusterServiceLocator.WorldCluster
                .GetAccountDatabase()
                .Query($"SELECT id FROM account WHERE username = '{client.Account}';", ref mySqlQuery);
            accountId = mySqlQuery.Rows[0].As<int>("id");
            mySqlQuery.Clear();
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Query(
                    $"SELECT * FROM characters WHERE account_id = '{accountId}' ORDER BY char_guid;",
                    ref mySqlQuery);
            ...
            for(int i = 0, loopTo = mySqlQuery.Rows.Count - 1; i <= loopTo; i++)
            {
                ...
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Query(
                        $"SELECT COUNT(*) FROM corpse WHERE player = {mySqlQuery.Rows[i]["char_guid"]};",
                        ref deadMySqlQuery);
                ...
                _clusterServiceLocator.WorldCluster
                    .GetCharacterDatabase()
                    .Query(
                        $"SELECT modelid, level, entry FROM character_pet WHERE owner = '{mySqlQuery.Rows[i]["char_guid"]}';",
                        ref petQuery);
                ...
                if(petQuery.Rows.Count > 0)
                {
                    ...
                    _clusterServiceLocator.WorldCluster
                        .GetWorldDatabase()
                        .Query(
                            $"SELECT family FROM creature_template WHERE entry = '{petQuery.Rows[0]["entry"]}'",
                            ref petFamilyQuery);
                ...
                _clusterServiceLocator.WorldCluster
                    ...
                    .Query(
                        string.Format(
                            $"SELECT item_slot, displayid, inventorytype FROM {characterDb}.characters_inventory, {worldDb}.item_template WHERE item_bag = {{0}} AND item_slot <> 255 AND entry = item_id  ORDER BY item_slot;",
                            guid),
                        ref itemsMySqlQuery);
                ...
                for(byte slot = 0, loopTo1 = (byte)EquipmentSlots.EQUIPMENT_SLOT_END; slot <= loopTo1; slot++) // - 1
                {
                    ...
                    } else
                    {
                        ...
                        if((((forceRestrictions & ((uint)ForceRestrictionFlags.RESTRICT_HIDECLOAK)) != 0) &&
                                (((EquipmentSlots)row.As<byte>("item_slot")) == EquipmentSlots.EQUIPMENT_SLOT_BACK)) ||
                            (((forceRestrictions & ((uint)ForceRestrictionFlags.RESTRICT_HIDEHELM)) != 0) &&
                                (((EquipmentSlots)row.As<byte>("item_slot")) == EquipmentSlots.EQUIPMENT_SLOT_HEAD)))
                        {
                            response.AddInt32(0); // Item Model
                            response.AddInt8(0);  // Item Slot
                        } else
        ...
        } catch(Exception e)
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(
                    LogType.FAILED,
                    "[{0}:{1}] Unable to enum characters. [{2}]",
                    client.IP,
                    client.Port,
                    e.Message);
        ...
        client.Send(response);
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_CHAR_ENUM", client.IP, client.Port);
    }
    ...
    public void On_CMSG_CHAR_RENAME(PacketClass packet, ClientClass client)
    {
        ...
        var name = packet.GetString();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_RENAME [{2}:{3}]", client.IP, client.Port, guid, name);
        ...
        _clusterServiceLocator.WorldCluster
            .GetCharacterDatabase()
            .Query($"SELECT char_name FROM characters WHERE char_name LIKE \"{name}\";", ref q);
        if(q.Rows.Count > 0)
        {
        ...
        if(errCode == ((byte)ATLoginFlags.AT_LOGIN_RENAME))
        {
            _clusterServiceLocator.WorldCluster
                .GetCharacterDatabase()
                .Update(
                    $"UPDATE characters SET char_name = \"{name}\", force_restrictions = 0 WHERE char_guid = {guid};");
        }
    ...
    public void On_CMSG_PLAYER_LOGIN(PacketClass packet, ClientClass client)
    {
        ...
        var guid = packet.GetUInt64();
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_PLAYER_LOGIN [0x{2:X}]", client.IP, client.Port, guid);
        ...
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.FAILED, "[{0:000000}] Unable to login: WORLD SERVER DOWN", client.Index);
            client.Character.Dispose();
            ...
            {
                _clusterServiceLocator.WorldCluster.Log
                    .WriteLine(LogType.FAILED, "[{0:000000}] Unable to login: {1}", client.Index, ex.ToString());
                client.Character.Dispose();
                client.Character = null;
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_PLAYER_LOGOUT", client.IP, client.Port);
    ...
    public void On_CMSG_REQUEST_ACCOUNT_DATA(PacketClass packet, ClientClass client)
    {
        ...
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(
                LogType.DEBUG,
                "[{0}:{1}] CMSG_REQUEST_ACCOUNT_DATA [ID={2}]",
                client.IP,
                client.Port,
                dataId);
        if(dataId > 7L)
    ...
    public void On_CMSG_UPDATE_ACCOUNT_DATA(PacketClass packet, ClientClass client)
    {
        try
        {
            ...
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(
                    LogType.DEBUG,
                    "[{0}:{1}] CMSG_UPDATE_ACCOUNT_DATA [ID={2} Size={3}]",
                    client.IP,
                    client.Port,
                    dataId,
                    uncompressedSize);
...
// If UncompressedSize > ReceivedPacketSize Then
        ...
        {
            _clusterServiceLocator.WorldCluster.Log
                .WriteLine(LogType.FAILED, "Error while updating account data.{0}", $"{Constants.vbCrLf}{e}");
        }
    }
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] MSG_MOVE_WORLDPORT_ACK", client.IP, client.Port);
        try
        {
            if(!_clusterServiceLocator.WcNetwork.WorldServer.InstanceCheck(client, client.Character.Map))
            {
    ...
    {
        _clusterServiceLocator.WorldCluster.Log
            .WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_AUTH_SESSION [{2}]", client.IP, client.Port, client.Account);
        Thread.Sleep(500);
